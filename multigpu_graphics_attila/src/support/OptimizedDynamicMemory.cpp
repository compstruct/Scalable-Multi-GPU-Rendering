...
void OptimizedDynamicMemory::Bucket::init(u32bit maxObjectSize, u32bit maxObjects, u32bit numGPUs)
{
    _maxObjects = maxObjects * numGPUs * 0.75;
...
void OptimizedDynamicMemory::initialize( u32bit maxObjectSize1, u32bit capacity1, u32bit maxObjectSize2, u32bit capacity2,
    u32bit maxObjectSize3, u32bit capacity3, u32bit numGPUs )
{
    // integrity test
    GPU_ASSERT(
    ...
    bucket[0].init(maxObjectSize1, capacity1, numGPUs);
    bucket[1].init(maxObjectSize2, capacity2, numGPUs);
    bucket[2].init(maxObjectSize3, capacity3, numGPUs);
...
void OptimizedDynamicMemory::setTag(std::string tag)
{
#ifdef FAST_NEW_DELETE
        return ;
#endif
...
return;
    ...
    ((u64bit *) this)[(bucket[b].ChunkSize() >> 3) - 2] = *((u64bit *) (tag.c_str()));
