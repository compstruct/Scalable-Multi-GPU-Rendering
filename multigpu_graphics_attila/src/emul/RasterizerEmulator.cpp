...
RasterizerEmulator::RasterizerEmulator(u32bit gpuId, u32bit numGPUs, u32bit numActiveTriangles,
    u32bit attrPerFragment, u32bit scanW, u32bit scanH, u32bit displayW, u32bit displayH,
    u32bit overW, u32bit overH, u32bit genW, u32bit genH,
    bool useBBOpt, u32bit subPxBits) :

    gpuId(gpuId), numGPUs(numGPUs),
    ...
    genTileFragments(genW * genH),
    displayTileWidth(displayW * scanW),
    displayTileHeight(displayH * scanH),
    useBBOptimization(useBBOpt),
...
u32bit RasterizerEmulator::preBound(QuadFloat *vAttr1, QuadFloat *vAttr2, QuadFloat *vAttr3)
{
    ...
    triangle = new SetupTriangle(vAttr1, vAttr2, vAttr3, -1, -1, -1);
...
u32bit RasterizerEmulator::setup(QuadFloat *vAttr1, QuadFloat *vAttr2, QuadFloat *vAttr3,
		                 s32bit vIndex1, s32bit vIndex2, s32bit vIndex3)
{
    f64bit edge1[3];
    f64bit edge2[3];
    f64bit edge3[3];
    ...
    /*  Create the new setup triangle.  */
    triangle = new SetupTriangle(vAttr1, vAttr2, vAttr3, vIndex1, vIndex2, vIndex3);
...
bool RasterizerEmulator::multiGPUDisplayTest(u32bit hRes, u32bit vRes, u32bit triangle, u32bit multiGPUExecMode) {
    // Get triangle bounding box
    s32bit minX, minY, maxX, maxY;
    setupTriangles[triangle]->getBoundingBox(minX, minY, maxX, maxY);

    u32bit numDisplayTilesW;
    
    u32bit minDisplayTileIdW, minDisplayTileIdH;
    u32bit maxDisplayTileIdW, maxDisplayTileIdH;

    u32bit numOfGPUs;
    u32bit numScanTilesW, numScanTilesH;
    u32bit scanTilesInDisplayTileW, scanTilesInDisplayTileH;
    u32bit minScanTileIdW, minScanTileIdH;
    u32bit maxScanTileIdW, maxScanTileIdH;

    switch (multiGPUExecMode) {
        case FRAME_DUP:
	case GPUPD:
            numDisplayTilesW = (u32bit)ceil(f32bit(hRes)/f32bit(displayTileWidth));
            minDisplayTileIdW = minX/displayTileWidth;
            minDisplayTileIdH = minY/displayTileHeight;
            maxDisplayTileIdW = maxX/displayTileWidth;
            maxDisplayTileIdH = maxY/displayTileHeight;

	    break;

	case SORT_LAST:
            numScanTilesW = (u32bit)ceil(f32bit(hRes)/f32bit(scanTileWidth));
            numScanTilesH = (u32bit)ceil(f32bit(vRes)/f32bit(scanTileHeight));
	    
	    numDisplayTilesW = 1;
	    scanTilesInDisplayTileW = numScanTilesW;
	    scanTilesInDisplayTileH = numScanTilesH;
	    ...
	    while (numOfGPUs >= 2) {
	        ...
	        numOfGPUs = numOfGPUs / 2;
	    }

            minScanTileIdW = minX/scanTileWidth;
            minScanTileIdH = minY/scanTileHeight;
            maxScanTileIdW = maxX/scanTileWidth;
            maxScanTileIdH = maxY/scanTileHeight;

            minDisplayTileIdW = minScanTileIdW/scanTilesInDisplayTileW;
            minDisplayTileIdH = minScanTileIdH/scanTilesInDisplayTileH;
            maxDisplayTileIdW = maxScanTileIdW/scanTilesInDisplayTileW;
            maxDisplayTileIdH = maxScanTileIdH/scanTilesInDisplayTileH;

	    break;

        default:
	    panic("RasterizerEmulator", "multiGPUDisplayTest", "Invalid execution mode of multi-GPU architecture.");
    }

    u32bit displayTileId, displayTileIdW, displayTileIdH;
    for (displayTileIdH = minDisplayTileIdH; displayTileIdH <= maxDisplayTileIdH; displayTileIdH++) {
        for (displayTileIdW = minDisplayTileIdW; displayTileIdW <= maxDisplayTileIdW; displayTileIdW++) {
	    displayTileId = displayTileIdH * numDisplayTilesW + displayTileIdW;
