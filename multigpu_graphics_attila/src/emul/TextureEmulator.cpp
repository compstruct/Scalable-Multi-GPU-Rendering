...
TextureEmulator::TextureEmulator(u32bit unitID_, u32bit stampFrags, u32bit blockDim, u32bit superBlockDim, u32bit anisoAlg,
		    ...
		    u32bit displayScanWidth, u32bit displayScanHeight,
                    ...
                    u32bit genWidth, u32bit genHeight)
{
    ...
    displayScanTileWidth = displayScanWidth;
    displayScanTileHeight = displayScanHeight;
    ...
    for(u32bit tu = 0; tu < MAX_TEXTURES; tu++)
        texPixelMapper[tu].setupDisplay(1024, 1024, STAMP_WIDTH, STAMP_HEIGHT,
                                        ...
                                        displayScanTileWidth, displayScanTileHeight,
                                        1, 1);
...
TextureAccess *TextureEmulator::textureOperation(u32bit id, TextureOperation texOp, QuadFloat *stampCoord, f32bit *stampParameter, u32bit textUnit,
		                                 bool *debug, u32bit *threadID, u32bit *batchCounter, u32bit *frameCounter)
{
    ...
    if (texOp == ATTRIBUTE_READ)
    {
        ...
        textAccess = new TextureAccess(id, texOp, stampCoord, stampParameter, textUnit,
			               debug, threadID, batchCounter, frameCounter);
...
void TextureEmulator::calculateAddress(TextureAccess *textAccess)
{
    ...
    for(frag = 0; frag < STAMP_FRAGMENTS; frag++)
    {
        ...
        switch(textAccess->texOperation)
        {
			...
			if (textAccess->debug[frag]) {
			    char filename[32];
			    sprintf(filename, "debugVSFInput%d.txt", unitID);
			    FILE* debugfile = fopen(filename, "a+");
			    fprintf(debugfile, "thread %d textEmu::calculateAddress::op_%d fileter_%d [%d] %llx textureAddr 0x%x textUnit %d mode %d d1 %d cubemap %d (%d, %d, %d)\n",
				    textAccess->threadID[frag], textAccess->texOperation, textAccess->filter[frag], 0,
				    textAccess->trilinear[trilinearAccess]->address[frag][0],
				    textureAddress[textUnit][d1][textAccess->cubemap],
				    textUnit, textureMode[textUnit], d1, textAccess->cubemap, i, j, k);
			    fflush(debugfile);
			    fclose(debugfile);
			}
...
//printf("TU > N_M_L > d1 %d i %d j %d k %d\n", d1, i, j, k);
			    ...
			    if (textAccess->debug[frag]) {
			        ...
			        sprintf(filename, "debugVSFInput%d.txt", unitID);
			        FILE* debugfile = fopen(filename, "a+");
			        fprintf(debugfile, "thread %d textEmu::calculateAddress::op_%d fileter_%d [%d] %llx\n",
			                textAccess->threadID[frag], textAccess->texOperation, textAccess->filter[frag], 0,
			        	textAccess->trilinear[trilinearAccess]->address[frag][0]);
			        fflush(debugfile);
			        fclose(debugfile);
...
//printf("TU > N_M_L > d2 %d i %d j %d k %d\n", d2, i, j, k);
			    ...
			    if (textAccess->debug[frag]) {
			        ...
			        sprintf(filename, "debugVSFInput%d.txt", unitID);
			        FILE* debugfile = fopen(filename, "a+");
			        fprintf(debugfile, "threadID %d textEmu::calculateAddress::op_%d fileter_%d [%d] %llx\n",
			                textAccess->threadID[frag], textAccess->texOperation, textAccess->filter[frag], 0,
			        	textAccess->trilinear[trilinearAccess]->address[frag][0]);
			        fflush(debugfile);
			        fclose(debugfile);
    ...
    u32bit frag, u32bit level, u32bit i, u32bit j, u32bit k, u32bit l)
{
    ...
    switch(textureMode[textUnit])
    {
	    ...
	    if (textAccess.debug[frag]) {
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
	        FILE* debugfile = fopen(filename, "a+");
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 2*l,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][2*l]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 2*l + 1,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][2*l + 1]);
	        fflush(debugfile);
	        fclose(debugfile);
	    }
	    ...
	    if (textAccess.debug[frag]) {
	        ...
	        FILE* debugfile = fopen(filename, "a+");
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 4*l,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][4*l]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 4*l + 1,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][4*l + 1]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 4*l + 2,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][4*l + 2]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 4*l + 3,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][4*l + 3]);
	        fflush(debugfile);
	        fclose(debugfile);
	    }
	    ...
	    if (textAccess.debug[frag]) {
	        ...
	        FILE* debugfile = fopen(filename, "a+");
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 8*l,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][8*l]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 8*l + 1,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][8*l + 1]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 8*l + 2,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][8*l + 2]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 8*l + 3,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][8*l + 3]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 8*l + 4,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][8*l + 4]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 8*l + 5,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][8*l + 5]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 8*l + 6,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][8*l + 6]);
	        fprintf(debugfile, "thread %d textEmu::genBilinearTexels::op_%d mode_%d [%d] %llx\n",
	    	        textAccess.threadID[frag], textAccess.texOperation, textureMode[textUnit], 8*l + 7,
	    	        textAccess.trilinear[trilinearAccess]->address[frag][8*l + 7]);
	        fflush(debugfile);
	        fclose(debugfile);
	    }
...
void TextureEmulator::filter(TextureAccess &textAccess, u32bit nextTrilinearFilter)
{
    ...
    if (textAccess.texOperation == ATTRIBUTE_READ)
    {
		...
		if (textAccess.debug[frag]) {
	            ...
	            sprintf(filename, "debugVSFInput%d.txt", unitID);
	            FILE* debugfile = fopen(filename, "a+");
	            fprintf(debugfile, "thread %d textEmu::filter0::op_%d stream_%d sample[%d] = %f\n",
			    textAccess.threadID[frag], textAccess.texOperation, stream,
			    0, textAccess.sample[frag][0]);
	            fprintf(debugfile, "thread %d textEmu::filter0::op_%d stream_%d sample[%d] = %f\n",
			    textAccess.threadID[frag], textAccess.texOperation, stream,
			    1, textAccess.sample[frag][1]);
	            fprintf(debugfile, "thread %d textEmu::filter0::op_%d stream_%d sample[%d] = %f\n",
			    textAccess.threadID[frag], textAccess.texOperation, stream,
			    2, textAccess.sample[frag][2]);
	            fprintf(debugfile, "thread %d textEmu::filter0::op_%d stream_%d sample[%d] = %f\n",
			    textAccess.threadID[frag], textAccess.texOperation, stream,
			    3, textAccess.sample[frag][3]);
	            fflush(debugfile);
	            fclose(debugfile);
                }		
            }
    ...
    if (textAccess.trilinearToFilter == 0)
    {
	    ...
	    if (textAccess.debug[frag]) {
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
	        FILE* debugfile = fopen(filename, "a+");
	        fprintf(debugfile, "thread %d textEmu::filter1::op_%d sample[%d] = %f\n",
	    	    textAccess.threadID[frag], textAccess.texOperation,
	    	    0, textAccess.sample[frag][0]);
	        fprintf(debugfile, "thread %d textEmu::filter1::op_%d sample[%d] = %f\n",
	    	    textAccess.threadID[frag], textAccess.texOperation,
	    	    1, textAccess.sample[frag][1]);
...
void TextureEmulator::filterTrilinear(TextureAccess &textAccess, u32bit trilinearAccess)
{
    ...
    for(i = 0; i < stampFragments; i++)
    {
        ...
        if (textAccess.debug[i]) {
	    char filename[32];
	    sprintf(filename, "debugVSFInput%d.txt", unitID);
	    FILE* debugfile = fopen(filename, "a+");
	    fprintf(debugfile, "thread %d textEmu::filterTrilinear::op_%d filter_%d sample[%d] = %f\n",
		    textAccess.threadID[i], textAccess.texOperation, textAccess.filter[i],
		    0, textAccess.trilinear[trilinearAccess]->sample[i][0]);
	    fprintf(debugfile, "thread %d textEmu::filterTrilinear::op_%d filter_%d sample[%d] = %f\n",
		    textAccess.threadID[i], textAccess.texOperation, textAccess.filter[i],
		    1, textAccess.trilinear[trilinearAccess]->sample[i][1]);
	    fprintf(debugfile, "thread %d textEmu::filterTrilinear::op_%d filter_%d sample[%d] = %f\n",
		    textAccess.threadID[i], textAccess.texOperation, textAccess.filter[i],
		    2, textAccess.trilinear[trilinearAccess]->sample[i][2]);
	    fprintf(debugfile, "thread %d textEmu::filterTrilinear::op_%d filter_%d sample[%d] = %f\n",
		    textAccess.threadID[i], textAccess.texOperation, textAccess.filter[i],
		    3, textAccess.trilinear[trilinearAccess]->sample[i][3]);
	    fflush(debugfile);
	    fclose(debugfile);
        }		
    }
...
//printf("TxEmu::convertFormat => ATTRIBUTE_READ -> (1) copying %d bytes from %d\n", readSize, readOffset);
	    ...
	    if (textAccess.debug[frag]) {
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
	        FILE* debugfile = fopen(filename, "a+");
	        fprintf(debugfile, "thread %d textEmu::convertFormat0::op_%d tex_%d ",
	        	textAccess.threadID[frag], textAccess.texOperation, tex);
	        for(u32bit b = 0; b < readSize; b++) {
	            fprintf(debugfile, "[%d] = %d ", b, textAccess.trilinear[trilinearAccess]->attrData[frag][b]);
	        }
	        fprintf(debugfile, "\n");
	        fflush(debugfile);
	        fclose(debugfile);
...
//printf("TxEmu::convertFormat => ATTRIBUTE_READ -> (2) copying %d bytes to %d\n", readSize, dataOffset);
	    ...
	    if (textAccess.debug[frag]) {
	        ...
	        fprintf(debugfile, "thread %d textEmu::convertFormat0::op_%d tex_%d ",
	        	textAccess.threadID[frag], textAccess.texOperation, tex);
	        for(u32bit b = 0; b < readSize; b++) {
	            fprintf(debugfile, "[%d] = %d ", b, textAccess.trilinear[trilinearAccess]->attrData[frag][b + dataOffset]);
	        }
	        fprintf(debugfile, "\n");
	        fflush(debugfile);
	        fclose(debugfile);
    ...
    if (textAccess.debug[frag]) {
        ...
        sprintf(filename, "debugVSFInput%d.txt", unitID);
        FILE* debugfile = fopen(filename, "a+");
        fprintf(debugfile, "thread %d textEmu::convertFormat1::op_%d tex_%d format_%d trilinearAccess_%d texel[%d] = %f\n",
        	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
        	trilinearAccess, 0, textAccess.trilinear[trilinearAccess]->texel[frag][tex][0]);
        fprintf(debugfile, "thread %d textEmu::convertFormat1::op_%d tex_%d format_%d trilinearAccess_%d texel[%d] = %f\n",
        	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
        	trilinearAccess, 1, textAccess.trilinear[trilinearAccess]->texel[frag][tex][1]);
        fprintf(debugfile, "thread %d textEmu::convertFormat1::op_%d tex_%d format_%d trilinearAccess_%d texel[%d] = %f\n",
        	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
        	trilinearAccess, 2, textAccess.trilinear[trilinearAccess]->texel[frag][tex][2]);
        fprintf(debugfile, "thread %d textEmu::convertFormat1::op_%d tex_%d format_%d trilinearAccess_%d texel[%d] = %f\n",
        	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
        	trilinearAccess, 3, textAccess.trilinear[trilinearAccess]->texel[frag][tex][3]);
        fflush(debugfile);
        fclose(debugfile);
    }
	...
	if (textAccess.debug[frag]) {
            ...
            sprintf(filename, "debugVSFInput%d.txt", unitID);
            FILE* debugfile = fopen(filename, "a+");
            fprintf(debugfile, "thread %d textEmu::convertFormat2::op_%d tex_%d format_%d texel[%d] = %f\n",
            	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
            	0, textAccess.trilinear[trilinearAccess]->texel[frag][tex][0]);
            fprintf(debugfile, "thread %d textEmu::convertFormat2::op_%d tex_%d format_%d texel[%d] = %f\n",
            	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
            	1, textAccess.trilinear[trilinearAccess]->texel[frag][tex][1]);
            fprintf(debugfile, "thread %d textEmu::convertFormat2::op_%d tex_%d format_%d texel[%d] = %f\n",
            	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
            	2, textAccess.trilinear[trilinearAccess]->texel[frag][tex][2]);
            fprintf(debugfile, "thread %d textEmu::convertFormat2::op_%d tex_%d format_%d texel[%d] = %f\n",
            	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
            	3, textAccess.trilinear[trilinearAccess]->texel[frag][tex][3]);
            fflush(debugfile);
            fclose(debugfile);
        }
    }
	...
	if (textAccess.debug[frag]) {
            ...
            sprintf(filename, "debugVSFInput%d.txt", unitID);
            FILE* debugfile = fopen(filename, "a+");
            fprintf(debugfile, "thread %d textEmu::convertFormat3::op_%d tex_%d format_%d texel[%d] = %f\n",
            	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
            	0, textAccess.trilinear[trilinearAccess]->texel[frag][tex][0]);
            fprintf(debugfile, "thread %d textEmu::convertFormat3::op_%d tex_%d format_%d texel[%d] = %f\n",
            	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
	...
	if (textAccess.debug[frag]) {
            ...
            sprintf(filename, "debugVSFInput%d.txt", unitID);
            FILE* debugfile = fopen(filename, "a+");
            fprintf(debugfile, "thread %d textEmu::convertFormat4::op_%d tex_%d format_%d texel[%d] = %f\n",
            	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
            	0, textAccess.trilinear[trilinearAccess]->texel[frag][tex][0]);
            fprintf(debugfile, "thread %d textEmu::convertFormat4::op_%d tex_%d format_%d texel[%d] = %f\n",
            	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
	...
	if (textAccess.debug[frag]) {
            ...
            sprintf(filename, "debugVSFInput%d.txt", unitID);
            FILE* debugfile = fopen(filename, "a+");
            fprintf(debugfile, "thread %d textEmu::convertFormat5::op_%d tex_%d format_%d texel[%d] = %f\n",
            	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
            	0, textAccess.trilinear[trilinearAccess]->texel[frag][tex][0]);
            fprintf(debugfile, "thread %d textEmu::convertFormat5::op_%d tex_%d format_%d texel[%d] = %f\n",
            	textAccess.threadID[frag], textAccess.texOperation, tex, textureFormat[textAccess.textUnit],
...
void TextureEmulator::decompressDXT1RGB(u8bit *input, u8bit *output, u32bit size, bool debug, u32bit debugThread, u32bit unitID_)
{
    u32bit i;
    u32bit numBlocks;
    ...
    if (debug) {
        char filename[32];
        sprintf(filename, "debugVSFInput%d.txt", unitID_);
        debugfile = fopen(filename, "a+");
        fprintf(debugfile, "thread %d textEmu::decompressDXT1RGB size %d numBlocks %d\n",
                debugThread, size, numBlocks);
    }
	...
	if (debug) {
	    for (u32bit j = 0; j < (1 << S3TC_DXT1_BLOCK_SIZE_SHIFT); j++) {
	        fprintf(debugfile, "input[%d] = %d ", j, input[(i << S3TC_DXT1_BLOCK_SIZE_SHIFT) + j]);
	    }
	    fprintf(debugfile, "\n");
	    for (u32bit j = 0; j < (1 << (S3TC_DXT1_BLOCK_SIZE_SHIFT + DXT1_SPACE_SHIFT)); j++) {
	        fprintf(debugfile, "output[%d] = %d ", j, input[(i << (S3TC_DXT1_BLOCK_SIZE_SHIFT + DXT1_SPACE_SHIFT)) + j]);
	    }
	}
    }
...
void TextureEmulator::decompressDXT1RGBA(u8bit *input, u8bit *output, u32bit size, bool debug, u32bit debugThread, u32bit unitID_)
{
    u32bit i;
    u32bit numBlocks;
    ...
    if (debug) {
        ...
        sprintf(filename, "debugVSFInput%d.txt", unitID_);
        debugfile = fopen(filename, "a+");
        fprintf(debugfile, "thread %d textEmu::decompressDXT1RGBA size %d numBlocks %d\n",
                debugThread, size, numBlocks);
    }
...
void TextureEmulator::decompressDXT3RGBA(u8bit *input, u8bit *output, u32bit size, bool debug, u32bit debugThread, u32bit unitID_)
{
    u32bit i;
    u32bit numBlocks;
    ...
    if (debug) {
        ...
        sprintf(filename, "debugVSFInput%d.txt", unitID_);
        debugfile = fopen(filename, "a+");
        fprintf(debugfile, "thread %d textEmu::decompressDXT3RGBA size %d numBlocks %d\n",
                debugThread, size, numBlocks);
    }
	...
	if (debug) {
	    for (u32bit j = 0; j < (1 << S3TC_DXT3_DXT5_BLOCK_SIZE_SHIFT); j++) {
	        fprintf(debugfile, "input[%d] = %d ", j, input[(i << S3TC_DXT3_DXT5_BLOCK_SIZE_SHIFT) + j]);
	    }
	    fprintf(debugfile, "\n");
	    for (u32bit j = 0; j < (1 << (S3TC_DXT3_DXT5_BLOCK_SIZE_SHIFT + DXT3_DXT5_SPACE_SHIFT)); j++) {
	        fprintf(debugfile, "output[%d] = %d ", j, input[(i << (S3TC_DXT3_DXT5_BLOCK_SIZE_SHIFT + DXT3_DXT5_SPACE_SHIFT)) + j]);
	    }
	}
    }
...
void TextureEmulator::decompressDXT5RGBA(u8bit *input, u8bit *output, u32bit size, bool debug, u32bit debugThread, u32bit unitID_)
{
    u32bit i;
    u32bit numBlocks;
    ...
    if (debug) {
        ...
        sprintf(filename, "debugVSFInput%d.txt", unitID_);
        debugfile = fopen(filename, "a+");
        fprintf(debugfile, "thread %d textEmu::decompressDXT5RGBA size %d numBlocks %d\n",
                debugThread, size, numBlocks);
    }
...
void TextureEmulator::decompressLATC1(u8bit *input, u8bit *output, u32bit size, bool debug, u32bit debugThread, u32bit unitID_)
{
    u32bit i;
    u32bit numBlocks;
    ...
    if (debug) {
        ...
        sprintf(filename, "debugVSFInput%d.txt", unitID_);
        debugfile = fopen(filename, "a+");
        fprintf(debugfile, "thread %d textEmu::decompressLATC1 size %d numBlocks %d\n",
                debugThread, size, numBlocks);
    }
	...
	if (debug) {
	    for (u32bit j = 0; j < (1 << LATC1_BLOCK_SIZE_SHIFT); j++) {
	        fprintf(debugfile, "input[%d] = %d ", j, input[(i << LATC1_BLOCK_SIZE_SHIFT) + j]);
	    }
	    fprintf(debugfile, "\n");
	    for (u32bit j = 0; j < (1 << (LATC1_BLOCK_SIZE_SHIFT + LATC1_LATC2_SPACE_SHIFT)); j++) {
	        fprintf(debugfile, "output[%d] = %d ", j, input[(i << (LATC1_BLOCK_SIZE_SHIFT + LATC1_LATC2_SPACE_SHIFT)) + j]);
	    }
	}
    }
...
void TextureEmulator::decompressLATC1Signed(u8bit *input, u8bit *output, u32bit size, bool debug, u32bit debugThread, u32bit unitID_)
{
    u32bit i;
    u32bit numBlocks;
    ...
    if (debug) {
        ...
        sprintf(filename, "debugVSFInput%d.txt", unitID_);
        debugfile = fopen(filename, "a+");
        fprintf(debugfile, "thread %d textEmu::decompressLATC1Signed size %d numBlocks %d\n",
                debugThread, size, numBlocks);
    }
...
void TextureEmulator::decompressLATC2(u8bit *input, u8bit *output, u32bit size, bool debug, u32bit debugThread, u32bit unitID_)
{
    u32bit i;
    u32bit numBlocks;
    ...
    if (debug) {
        ...
        sprintf(filename, "debugVSFInput%d.txt", unitID_);
        debugfile = fopen(filename, "a+");
        fprintf(debugfile, "thread %d textEmu::decompressLATC2 size %d numBlocks %d\n",
                debugThread, size, numBlocks);
    }
	...
	if (debug) {
	    for (u32bit j = 0; j < (1 << LATC2_BLOCK_SIZE_SHIFT); j++) {
	        fprintf(debugfile, "input[%d] = %d ", j, input[(i << LATC2_BLOCK_SIZE_SHIFT) + j]);
	    }
	    fprintf(debugfile, "\n");
	    for (u32bit j = 0; j < (1 << (LATC2_BLOCK_SIZE_SHIFT + LATC1_LATC2_SPACE_SHIFT)); j++) {
	        fprintf(debugfile, "output[%d] = %d ", j, input[(i << (LATC2_BLOCK_SIZE_SHIFT + LATC1_LATC2_SPACE_SHIFT)) + j]);
	    }
	}
    }
...
void TextureEmulator::decompressLATC2Signed(u8bit *input, u8bit *output, u32bit size, bool debug, u32bit debugThread, u32bit unitID_)
{
    u32bit i;
    u32bit numBlocks;
    ...
    if (debug) {
        ...
        sprintf(filename, "debugVSFInput%d.txt", unitID_);
        debugfile = fopen(filename, "a+");
        fprintf(debugfile, "thread %d textEmu::decompressLATC2Signed size %d numBlocks %d\n",
                debugThread, size, numBlocks);
    }
...
void TextureEmulator::index2address(u32bit attribute, TextureAccess &textAccess, u32bit trilinearAccess, u32bit frag, u32bit index)
{
...
//printf("TxEmu::index2address => the attribute is inactive setting address to %x\n", BLACK_TEXEL_ADDRESS); 
        ...
        textAccess.trilinear[trilinearAccess]->address[frag][0] = BLACK_TEXEL_ADDRESS;

	if (textAccess.debug[frag]) {
	    char filename[32];
	    sprintf(filename, "debugVSFInput%d.txt", unitID);
	    FILE* debugfile = fopen(filename, "a+");
	    fprintf(debugfile, "thread %d textEmu::index2address::op_%d stream_%d [%d] %llx\n",
		    textAccess.threadID[frag], textAccess.texOperation, stream, 0,
		    textAccess.trilinear[trilinearAccess]->address[frag][0]);
	    fflush(debugfile);
	    fclose(debugfile);
	}
        ...
        textAccess.trilinear[trilinearAccess]->attrFirstOffset[frag] = 0;
        textAccess.trilinear[trilinearAccess]->attrFirstSize[frag] = streamDataSize[stream];
        textAccess.trilinear[trilinearAccess]->attrSecondSize[frag] = 0;
        ...
        textAccess.texelSize[frag] = 4;

//printf("TxEmu::index2address => loops = %d | texels per loop = %d | texelSize = %d\n",
//textAccess.trilinear[trilinearAccess]->loops[frag],
//textAccess.trilinear[trilinearAccess]->texelsLoop[frag],
//textAccess.texelSize[frag]);
...
//startAddress, endAddress, (textCacheBlockDim * 2), sameLine ? "Si" : "No");
    ...
    if (sameLine)
    {
        ...
        //  Set address to read.
        textAccess.trilinear[trilinearAccess]->address[frag][0] = u64bit(startAddress);
        
	if (textAccess.debug[frag]) {
	    char filename[32];
	    sprintf(filename, "debugVSFInput%d.txt", unitID);
	    FILE* debugfile = fopen(filename, "a+");
	    fprintf(debugfile, "thread %d textEmu::index2address::op_%d stream_%d sameLine %s [%d] %llx\n",
		    textAccess.threadID[frag], textAccess.texOperation, stream, sameLine ? "T" : "F", 0,
		    textAccess.trilinear[trilinearAccess]->address[frag][0]);
	    fflush(debugfile);
	    fclose(debugfile);
	}
...
//printf("TxEmu::index2address => address0 = %x\n", textAccess.trilinear[trilinearAccess]->address[frag][0]);
        ...
        //  Set amount of data to read.
        textAccess.texelSize[frag] = streamDataSize[stream];
    }
    else
    {
        ...
        textAccess.trilinear[trilinearAccess]->address[frag][1] = u64bit(((startAddress >> (textCacheBlockDim * 2)) + 1) << (textCacheBlockDim * 2));

	if (textAccess.debug[frag]) {
	    char filename[32];
	    sprintf(filename, "debugVSFInput%d.txt", unitID);
	    FILE* debugfile = fopen(filename, "a+");
	    fprintf(debugfile, "thread %d textEmu::index2address::op_%d stream_%d sameLine_%s [%d] %llx\n",
		    textAccess.threadID[frag], textAccess.texOperation, stream, sameLine? "T" : "F", 0,
		    textAccess.trilinear[trilinearAccess]->address[frag][0]);
	    fprintf(debugfile, "thread %d textEmu::index2ddress::op_%d stream_%d sameLine_%s [%d] %llx\n",
		    textAccess.threadID[frag], textAccess.texOperation, stream, sameLine? "T" : "F", 1,
		    textAccess.trilinear[trilinearAccess]->address[frag][1]);
	    fflush(debugfile);
	    fclose(debugfile);
	}
...
//printf("TxEmu::index2address => address1 = %x\n", textAccess.trilinear[trilinearAccess]->address[frag][1]);
        ...
        //  Set offset and bytes to read for first cache read.
        textAccess.trilinear[trilinearAccess]->attrFirstOffset[frag] = readSize - firstLineSize;
        textAccess.trilinear[trilinearAccess]->attrFirstSize[frag] = firstLineSize;
        textAccess.trilinear[trilinearAccess]->attrSecondSize[frag] = secondLineSize;
        ...
        //  Set amount of data to read.
        textAccess.texelSize[frag] = readSize;
    }
...
void TextureEmulator::loadAttribute(TextureAccess &textAccess, u32bit trilinearAccess, u32bit frag)
{
    //  Get the attribute identifier for which to convert/fill data.
    u32bit attr = textAccess.textUnit;
    
    //  Get the pointer to the data read for the attribute.
    u8bit *data = textAccess.trilinear[trilinearAccess]->attrData[frag];
...
//attr, stream, format, elemSize, streamElements[stream]);
    ...
    switch(streamElements[stream])
    {
        ...
        case 1:
            //  1-element attribute.
            textAccess.sample[frag][0] = attributeDataConvert(format, &data[offset]);

            //  Load the default value in the undefined elements of the attribute.
            textAccess.sample[frag][1] = attrDefValue[attr][1];
            textAccess.sample[frag][2] = attrDefValue[attr][2];
            textAccess.sample[frag][3] = attrDefValue[attr][3];
        ...
        case 2:
            //  2-element attribute.
            textAccess.sample[frag][0] = attributeDataConvert(format, &data[offset]);
            textAccess.sample[frag][1] = attributeDataConvert(format, &data[offset + elemSize]);
        ...
        case 3:
            ...
            textAccess.sample[frag][0] = attributeDataConvert(format, &data[offset]);
            textAccess.sample[frag][1] = attributeDataConvert(format, &data[offset + elemSize]);
            textAccess.sample[frag][2] = attributeDataConvert(format, &data[offset + elemSize * 2]);
        ...
        case 4:
            ...
            textAccess.sample[frag][1] = attributeDataConvert(format, &data[offset + elemSize]);
            textAccess.sample[frag][2] = attributeDataConvert(format, &data[offset + elemSize * 2]);
            textAccess.sample[frag][3] = attributeDataConvert(format, &data[offset + elemSize * 3]);
            break;
    }
    ...
    if (textAccess.debug[frag]) {
        ...
        sprintf(filename, "debugVSFInput%d.txt", unitID);
        FILE* debugfile = fopen(filename, "a+");
        fprintf(debugfile, "thread %d texEmu::loadAttribute::op_%d stream_%d elements_%d sample[%d] = %f\n",
    	    textAccess.threadID[frag], textAccess.texOperation, stream, streamElements[stream],
    	    0, textAccess.sample[frag][0]);
        fprintf(debugfile, "thread %d texEmu::loadAttribute::op_%d stream_%d elements_%d sample[%d] = %f\n",
    	    textAccess.threadID[frag], textAccess.texOperation, stream, streamElements[stream],
    	    1, textAccess.sample[frag][1]);
...
//textAccess.sample[frag][0],
//textAccess.sample[frag][1],
//textAccess.sample[frag][2],
//textAccess.sample[frag][3]);
    ...
    if (d3d9ColorStream[stream])
    {
        ...
        f32bit red = textAccess.sample[frag][2];
        f32bit green = textAccess.sample[frag][1];
        f32bit blue= textAccess.sample[frag][0];
        f32bit alpha = textAccess.sample[frag][3];

        textAccess.sample[frag][0] = red;
        textAccess.sample[frag][1] = green;
        textAccess.sample[frag][2] = blue;
        textAccess.sample[frag][3] = alpha;
        
	if (textAccess.debug[frag]) {
            char filename[32];
            sprintf(filename, "debugVSFInput%d.txt", unitID);
            FILE* debugfile = fopen(filename, "a+");
            fprintf(debugfile, "thread %d texEmu::loadAttribute::d3d9ColorStream op_%d stream_%d elements_%d sample[%d] = %f\n",
        	    textAccess.threadID[frag], textAccess.texOperation, stream, streamElements[stream],
        	    0, textAccess.sample[frag][0]);
            fprintf(debugfile, "thread %d texEmu::loadAttribute::d3d9ColorStream op_%d stream_%d elements_%d sample[%d] = %f\n",
        	    textAccess.threadID[frag], textAccess.texOperation, stream, streamElements[stream],
...
TextureAccess::TextureAccess(u32bit id, TextureOperation texOp, QuadFloat *coord, f32bit *param, u32bit unit,
		             bool* debug_, u32bit* threadID_, u32bit* batchCounter_, u32bit* frameCounter_)
{
    u32bit i;
	...
	debug[i] = debug_[i];
	threadID[i] = threadID_[i];

	batchCounter[i] = batchCounter_[i];
	frameCounter[i] = frameCounter_[i];
    }
}
