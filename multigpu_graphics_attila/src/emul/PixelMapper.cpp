    ...
    u32bit samplesP, u32bit bytesSampleP)
{
    ...
    displayTileWidth = displayW;
    displayTileHeight = displayH;
    ...
    scanTileRowWidth = (u32bit)ceil((f32bit)hRes / (f32bit)scanTilePixelWidth);
    scanTileColumnHeight = (u32bit)ceil((f32bit)vRes / (f32bit)scanTilePixelHeight);

    displayTilePixelWidth = displayW * scanW * genW * stampW;
    displayTilePixelHeight = displayH * scanH * genH * stampH;
    displayTileRowWidth = (u32bit)ceil((f32bit)hRes / (f32bit)displayTilePixelWidth);
...
void PixelMapper::getResolution(u32bit &resWidth, u32bit &resHeight)
{
    resWidth = hRes;
    resHeight = vRes;
}

void PixelMapper::getOverTileInfo(u32bit &otRowWidth, u32bit &otRows)
{
    otRowWidth = overTileRowWidth;
    otRows = overTileRows;
}
...
// Map pixel (position) to display tile.
u32bit PixelMapper::mapToDisplayTile(u32bit x, u32bit y, u32bit multiGPUExecMode, u32bit numGPUs)
{
    ...
    u32bit xDisplayTile, yDisplayTile;
    u32bit xScanTile, yScanTile;
    u32bit numDisplayTilesW;
    u32bit scanTilesInDisplayTileW, scanTilesInDisplayTileH;

    u32bit displayTile;

    switch (multiGPUExecMode) {
	case FRAME_DUP:
        case GPUPD:
            xDisplayTile = xDevice / displayTilePixelWidth;
            yDisplayTile = yDevice / displayTilePixelHeight;
            displayTile = yDisplayTile * displayTileRowWidth + xDisplayTile;
	    
	    break;

	case SORT_LAST:
	    xScanTile = xDevice / scanTilePixelWidth;
	    yScanTile = yDevice / scanTilePixelHeight;
	   
	    numDisplayTilesW = 1;
	    scanTilesInDisplayTileW = scanTileRowWidth;
	    scanTilesInDisplayTileH = scanTileColumnHeight;

	    while (numGPUs >= 2) {
	        if (scanTilesInDisplayTileW >= scanTilesInDisplayTileH && scanTilesInDisplayTileW >= 2) {
		    scanTilesInDisplayTileW = scanTilesInDisplayTileW / 2;
		    numDisplayTilesW = numDisplayTilesW * 2;
		} else if (scanTilesInDisplayTileH >= 2) {
		    scanTilesInDisplayTileH = scanTilesInDisplayTileH / 2;
		}
		numGPUs = numGPUs / 2;
	    }
	    
	    xDisplayTile = xScanTile / scanTilesInDisplayTileW;
	    yDisplayTile = yScanTile / scanTilesInDisplayTileH;
	    displayTile = yDisplayTile * numDisplayTilesW + xDisplayTile;
	    
	    break;

	default:
            panic("PixelMapper", "mapToDisplayTile", "Invlaid execution mode of multi-GPU architecture.");
    }
...
// Map pixel (address) to display tile.
u32bit PixelMapper::mapToDisplayTile(u32bit address, u32bit multiGPUExecMode, u32bit numGPUs)
{
    // Scan sub tile ID in the over scan tile
    u32bit scanSubTile = address / scanSubTileBytes;
    u32bit scanTileIdInOverTile = GPU_MOD(scanSubTile, overTileSize);

    GPU_ASSERT(
        if (GPU_MOD(overTileWidth, 2) != 0)
            panic("PixelMapper", "mapToDisplayTile", "overTileWidth has to be an even number in this function.");	
        if (overTileWidth > 4)
            panic("PixelMapper", "mapToDisplayTile", "current implementation cannot decode morton table with width bigger than 4.");	
    )

    // The number of morton tiles (2X2) in a row of over scan tile
    u32bit numMortonTilesInOverTileRow = overTileWidth / 2;
    // Morton tile ID in the over scan tile
    u32bit mortonTileIdInOverTile = scanTileIdInOverTile / 4;
    // Morton tile position in the over scan tile
    u32bit xMortonTileInOverTile = GPU_MOD(mortonTileIdInOverTile, numMortonTilesInOverTileRow);
    u32bit yMortonTileInOverTile = mortonTileIdInOverTile / numMortonTilesInOverTileRow;
    // Scan tile position in the morton tile
    u32bit scanTileIdInMortonTile = GPU_MOD(scanTileIdInOverTile, 4);
    u32bit xScanTileInMortonTile = GPU_MOD(scanTileIdInMortonTile, 2);
    u32bit yScanTileInMortonTile = scanTileIdInMortonTile / 2;
    // Scan tile position in the over scan tile
    u32bit xScanTileInOverTile = xScanTileInMortonTile + 2 * xMortonTileInOverTile;
    u32bit yScanTileInOverTile = yScanTileInMortonTile + 2 * yMortonTileInOverTile;

    // Over scan tile position in the screen
    u32bit overTileId = address / (overTilePixelSize * samples * bytesSample);
    u32bit xOverTile = GPU_MOD(overTileId, overTileRowWidth);
    u32bit yOverTile = overTileId / overTileRowWidth;

    // Scan tile position in the screen
    u32bit xScanTile = xOverTile * overTileWidth + xScanTileInOverTile;
    u32bit yScanTile = yOverTile * overTileHeight + yScanTileInOverTile;
    ...
    // Display tile Id in screen
    switch (multiGPUExecMode) {
	case FRAME_DUP:
        case GPUPD:
            xDisplayTile = xScanTile / displayTileWidth;
            yDisplayTile = yScanTile / displayTileHeight;
            displayTile = yDisplayTile * displayTileRowWidth + xDisplayTile;
...
u32bit PixelMapper::computeNumOfScanTiles()
{
    return overTileRowWidth * overTileRows * overTileWidth * overTileHeight;
}
