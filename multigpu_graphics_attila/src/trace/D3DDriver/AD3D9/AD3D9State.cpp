...
#define SOFTWARE_INSTANCING

AD3D9State* AD3D9State::inst = NULL;
...
void AD3D9State::initialize(AIDeviceImp9 *device, UINT width, UINT height)
{
    ...
    numGPUs = acdDev->getNumGPUs();
    multiGPUExecMode = acdDev->getMultiGPUExecMode();
    execRound = acdDev->getExecRound();
...
void AD3D9State::draw(D3DPRIMITIVETYPE type, UINT start, UINT count)
{
    ...
    //std::vector<acdlib::acd_uint> starts, counts;
    //u32bit gpuIndex, primitive_per_gpu, residual_primitives, next_primitive;
    //switch(multiGPUExecMode) {
    //    case gpu3d::FRAME_DUP :
    //        starts.resize(numGPUs, start);
    //        counts.resize(numGPUs, get_vertex_count(type, count));
    //        break;
    //    case gpu3d::GPUPD :
    //        if (execRound == 1) {
    //            primitive_per_gpu = count/numGPUs;
    //            residual_primitives = GPU_MOD(count, numGPUs);
    //            next_primitive = 0;
    //            starts.resize(numGPUs, 0);
    //            counts.resize(numGPUs, 0);
    //            for (gpuIndex = 0; gpuIndex < numGPUs; gpuIndex++) {
    //                starts[gpuIndex] = get_start_vertex(type, start, next_primitive);
    //                if (residual_primitives > 0) {
    //                    counts[gpuIndex] = get_vertex_count(type, primitive_per_gpu + 1);	
    //                    residual_primitives--;
    //                    next_primitive += (primitive_per_gpu + 1);
    //                } else if (primitive_per_gpu > 0) {
    //                    counts[gpuIndex] = get_vertex_count(type, primitive_per_gpu);
    //                    next_primitive += primitive_per_gpu;
    //                }
    //            }
    //        } else {
    //            panic("AD3D9State", "draw", "Unsupported execution round for GPUpd.");
    //        }
    //        break;
    //    default:
    //        panic("AD3D9State", "draw", "Unsupported execution mode for multi-GPU architecture.");
    //        break;
    //}
