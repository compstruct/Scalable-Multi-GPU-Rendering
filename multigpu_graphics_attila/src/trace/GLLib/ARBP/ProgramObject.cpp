...
std::string ProgramObject::shOpcode2Str[] =
{
...
/*  Mask Mode to string.  */
std::string ProgramObject::maskMode2Str[] =
{
...
std::string ProgramObject::ccMode2Str[] =
{
    "EQ",     /*  CC Mode 0  */
    "GE",     /*  CC Mode 1  */
    "GT",     /*  CC Mode 2  */
...
/*  Bank to string.  */
std::string ProgramObject::bank2Str[] =
{
   "i",         /*  Bank 0h  */
   "o",         /*  Bank 1h  */
   "c",         /*  Bank 2h  */
...
void ProgramObject::_computeASMCode() const
{
    ...
    while(binary_pointer < binSize)
    {
        ...
        sprintf(instruction, "%s%s", instruction, shOpcode2Str[binary[binary_pointer]].c_str());
            ...
            if (((binary[binary_pointer+12] & 0x70) >> 4) != 0x06)
            {
                if (((binary[binary_pointer+1] & 0x01) == 0x01))
                    sprintf(instruction, "%s [a%d.%c + (%d)] (%s%s)", instruction, (binary[binary_pointer+14] & 0x03),
                        ...
                        ccMode2Str[((binary[binary_pointer+12] & 0x70) >> 4)].c_str(), buffer);
                else
                    sprintf(instruction, "%s %d (%s%s)", instruction, binary[binary_pointer + 15],ccMode2Str[((binary[binary_pointer+12] & 0x70) >> 4)].c_str(), buffer);
            }
            else
            {
                if (((binary[binary_pointer+1] & 0x01) == 0x01))
        ...
        else if (binary[binary_pointer] == 0x1A)
        {
            ...
            if (((binary[binary_pointer+12] & 0x70) >> 4) != 0x06)
            {
                sprintf(instruction, "%s (%s%s)", instruction, ccMode2Str[((binary[binary_pointer+12] & 0x70) >> 4)].c_str(), buffer);
            }
        ...
        else
        {
            ...
            */
            ...
            sprintf(instruction, "%s %s%d%s", instruction, bank2Str[((binary[binary_pointer+7] & 0x38) >> 3)].c_str(), binary[binary_pointer+5], maskMode2Str[(binary[binary_pointer+12] & 0x0F)].c_str());
            ...
            if (numOperands(binary[binary_pointer]) > 0)
            {
                ...
                if (((binary[binary_pointer+8] & 0x08) == 0x08))
                {
                    if (((binary[binary_pointer+1] & 0x01) == 0x01) && ((binary[binary_pointer + 6] & 0x07) == 0x02))
                        ...
                        sprintf(instruction, "%s|%s%d%s|", instruction, bank2Str[(binary[binary_pointer + 6] & 0x07)].c_str(), binary[binary_pointer+2], buffer);
                }
                else
                {
                    if (((binary[binary_pointer+1] & 0x01) == 0x01) && ((binary[binary_pointer + 6] & 0x07) == 0x02))
            ...
            if (numOperands(binary[binary_pointer]) > 1)
            {
                ...
                if (((binary[binary_pointer+8] & 0x10) == 0x10))
                {
                    if (((binary[binary_pointer+1] & 0x01) == 0x01) && ((binary[binary_pointer + 6] & 0x038) == 0x02))
                        ...
                        sprintf(instruction, "%s|%s%d%s|", instruction, bank2Str[((binary[binary_pointer + 6] & 0x038) >> 3)].c_str(), binary[binary_pointer+3], buffer);
                }
                else
                {
                    if (((binary[binary_pointer+1] & 0x01) == 0x01) && ((binary[binary_pointer + 6] & 0x038) == 0x02))
            ...
            if (numOperands(binary[binary_pointer]) > 2)
            {
                ...
                if (((binary[binary_pointer+8] & 0x20) == 0x20))
                {
                    if (((binary[binary_pointer+1] & 0x01) == 0x01) && ((binary[binary_pointer + 7] & 0x07) == 0x02))
                        ...
                        sprintf(instruction, "%s|%s%d%s|", instruction, bank2Str[(binary[binary_pointer + 7] & 0x07)].c_str(), binary[binary_pointer+4], buffer);
                }
                else
                {
                    if (((binary[binary_pointer+1] & 0x01) == 0x01) && ((binary[binary_pointer + 7] & 0x07) == 0x02))
