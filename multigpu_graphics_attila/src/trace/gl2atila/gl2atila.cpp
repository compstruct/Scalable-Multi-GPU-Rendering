...
int main(int argc, char *argv[])
{
    ...
    cl = new ConfigLoader(strdup(std::string("bGPU.ini").c_str()));
    ...
    GPU_ASSERT(
        if (simP.numGPUs != 1)
            panic("gl2atila", "main", "Only support single-GPU architecture.");
    )
    ...
    while(!end)
    {
        ...
        nextAGPTransaction = trDriver->nextAGPTransaction(0);
