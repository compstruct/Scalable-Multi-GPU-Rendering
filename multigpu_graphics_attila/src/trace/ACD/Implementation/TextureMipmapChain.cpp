...
void TextureMipmapChain::setSamplerID(acd_int id) {
    map<acd_uint, TextureMipmap*>::iterator it = _mips.begin();
    for ( ; it != _mips.end(); ++it )
        it->second->setSamplerID(id);
}

acd_int TextureMipmapChain::getSamplerID(acd_uint mipLevel) const {
    const TextureMipmap* mip = find(mipLevel);
    if (mip == 0)
	panic("TextureMipmapChain", "getSamplerID", "cannot find TextureMipmap");

    return mip->getSamplerID();
}
