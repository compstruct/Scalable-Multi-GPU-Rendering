...
void ACDStreamImp::sync(bool forcePreload)
{
    ...
    if ( buf != 0 ) {
        _device->allocator().syncGPU(buf, forcePreload); // syncronize buffer contents
    ...
    if (_STREAM_ID < gpu3d::MAX_STREAM_BUFFERS) { 
...
void ACDStreamImp::sync(u32bit gpuId, bool forcePreload)
{
    gpu3d::GPURegData data;
    ...
    if ( buf != 0 ) {
        _device->allocator().syncGPU(gpuId, buf, forcePreload); // syncronize buffer contents

        //if ( _buffer.changed() || (_buffer.initial()->getUID()!=buf->getUID()) || _offset.changed() || _gpuMemTrack != buf->trackRealloc(0) || _requiredSync ) {
            // Stream address requires to be recomputed
            acd_uint md = _device->allocator().md(buf);
            _driver->writeGPUAddrRegister(gpuId, gpu3d::GPU_STREAM_ADDRESS, _STREAM_ID, md, _offset);
            // reset IF conditions
            _buffer.restart();
            _offset.restart();
            _gpuMemTrack = buf->trackRealloc(0); // restart buffer tracking
