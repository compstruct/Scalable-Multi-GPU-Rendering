...
void MemoryObject::defineRegion(acd_uint region)
{
	...
	info.locked = false;
	info.preload = false;
	info.stateBeforeSync = MOS_NotFound;
...
void MemoryObject::syncGPU(acd_uint region, acd_uint gpuId) {
    map<acd_uint, MemoryObjectRegion>::iterator it = mor.find(region);
    if ( it == mor.end() )
        panic("MemoryObject", "syncGPU", "Region not found/defined");

    it->second.syncedGPUIds.insert(gpuId);
}

bool MemoryObject::isSynced(acd_uint region, acd_uint gpuId) {
    map<acd_uint, MemoryObjectRegion>::iterator it = mor.find(region);
    if ( it == mor.end() )
        panic("MemoryObject", "isSynced", "Region not found/defined");

    return it->second.syncedGPUIds.count(gpuId) > 0;
}

bool MemoryObject::anyGPUSynced(acd_uint region) {
    map<acd_uint, MemoryObjectRegion>::iterator it = mor.find(region);
    if ( it == mor.end() )
        panic("MemoryObject", "anyGPUSynced", "Region not found/defined");

    return it->second.syncedGPUIds.size() > 0;
}

void MemoryObject::clearSyncedGPUIds(acd_uint region) {
    map<acd_uint, MemoryObjectRegion>::iterator it = mor.find(region);
    if ( it == mor.end() )
        panic("MemoryObject", "clearSyncedGPUIds", "Region not found/defined");

    it->second.syncedGPUIds.clear();
}

MemoryObjectState MemoryObject::getStateBeforeSync(acd_uint region) {
    map<acd_uint, MemoryObjectRegion>::iterator it = mor.find(region);
    if ( it == mor.end() )
        panic("MemoryObject", "getStateBeforeSync", "Region not found/defined");

    return it->second.stateBeforeSync;
}

void MemoryObject::setStateBeforeSync(acd_uint region, MemoryObjectState stateBeforeSync) {
    map<acd_uint, MemoryObjectRegion>::iterator it = mor.find(region);
    if ( it == mor.end() )
        panic("MemoryObject", "setStateBeforeSync", "Region not found/defined");

    it->second.stateBeforeSync = stateBeforeSync;
}
