...
void ACDSamplerImp::setTexture(ACDTexture* texture)
{
    ...
    _texture = texture;
    _texture->setSamplerID(_SAMPLER_ID);
...
void ACDSamplerImp::_syncTexture(bool forcePreload)
{
    using namespace gpu3d;
    ...
    for ( acd_uint face = 0; face < totalFaces; ++face ) 
    {
        for ( acd_uint level = _baseLevel; level <= _maxLevel; ++level ) 
        {
            ...
            if (texture2D)
            {
                moa.syncGPU(texture2D, mipRegion, ((!_enabled && !_requiredSync) || forcePreload));
                md = moa.md(texture2D, mipRegion);

#ifdef ACD_DUMP_SAMPLERS
                acd_ubyte filename[30];
                sprintf((char*)filename, "Frame_%d_Batch_%d_Sampler%d_Mipmap%d_Format_%d.ppm",
			_device->getCurrentFrame(), _device->getCurrentBatch(),
			_SAMPLER_ID, level, _getFormatString(_textureFormat));
                texture2D->dumpMipmap(mipRegion, filename);
#endif
	        //FILE* debugfile = fopen("debugTexAddr.txt", "a+");
		//fprintf(debugfile, "ACDSamplerImp::_syncTexture: texture2D md %d samplerUnitOffset %d level %d face %d mipRegion %d state %d\n",
		//	md, samplerUnitOffset, level, face, mipRegion, texture2D->getState(mipRegion));
		//fflush(debugfile);
		//fclose(debugfile);
            }
            else if (textureCM)
            {
                moa.syncGPU(textureCM, mipRegion, ((!_enabled && !_requiredSync) || forcePreload));
                md = moa.md(textureCM, mipRegion);

#ifdef ACD_DUMP_SAMPLERS
                acd_ubyte filename[30];
                sprintf((char*)filename, "Frame_%d_Batch_%d_Sampler%d_Face%d_Mipmap%d_Format_%d.ppm",
		        _device->getCurrentFrame(), _device->getCurrentBatch(), _SAMPLER_ID,
		        face, level, _getFormatString(_textureFormat));
                textureCM->dumpMipmap((ACD_CUBEMAP_FACE)face, level, filename);
#endif
	        //FILE* debugfile = fopen("debugTexAddr.txt", "a+");
		//fprintf(debugfile, "ACDSamplerImp::_syncTexture: textureCM md %d samplerUnitOffset %d level %d face %d mipRegion %d state %d\n",
		//	md, samplerUnitOffset, level, face, mipRegion, textureCM->getState(mipRegion));
		//fflush(debugfile);
		//fclose(debugfile);
            }
	    else
	    {
                moa.syncGPU(texture3D, mipRegion, ((!_enabled && !_requiredSync) || forcePreload));
                md = moa.md(texture3D, mipRegion);
	        
		//FILE* debugfile = fopen("debugTexAddr.txt", "a+");
		//fprintf(debugfile, "ACDSamplerImp::_syncTexture: texture3D md %d samplerUnitOffset %d level %d face %d mipRegion %d state %d\n",
		//	md, samplerUnitOffset, level, face, mipRegion, texture3D->getState(mipRegion));
		//fflush(debugfile);
		//fclose(debugfile);
	    }
...
void ACDSamplerImp::_syncTexture(acd_uint gpuId, bool forcePreload)
{
    using namespace gpu3d;
    ...
    for ( acd_uint face = 0; face < totalFaces; ++face ) 
    {
        for ( acd_uint level = _baseLevel; level <= _maxLevel; ++level ) 
        {
            ...
            if (texture2D)
            {
                moa.syncGPU(gpuId, texture2D, mipRegion, ((!_enabled && !_requiredSync) || forcePreload));
                md = moa.md(texture2D, mipRegion);
		...
		//fclose(debugfile);
            ...
            {
                moa.syncGPU(gpuId, textureCM, mipRegion, ((!_enabled && !_requiredSync) || forcePreload));
                md = moa.md(textureCM, mipRegion);
	    ...
	    {
                moa.syncGPU(gpuId, texture3D, mipRegion, ((!_enabled && !_requiredSync) || forcePreload));
                md = moa.md(texture3D, mipRegion);
...
void ACDSamplerImp::performBlitOperation2D(acd_uint xoffset, acd_uint yoffset, acd_uint x, acd_uint y, acd_uint width, acd_uint height, acd_uint textureWidth, ACD_FORMAT internalFormat, ACDTexture2D* texture, acd_uint level)
{
    ...
    if (!_requiredSync)
    {
        ...
        MemoryObjectAllocator& moa = _device->allocator();
	if (_driver->getNumGPUs() > 1 && _driver->getMultiGPUExecMode() == gpu3d::GPUPD && _driver->getExecRound() == 1) {
            moa.syncGPU(tex2D, level, true);
	} else {
...
void ACDSamplerImp::sync(bool forcePreload)
{
    ACD_ASSERT(
    ...
    if ( !_enabled && !_requiredSync && !(_driver->getMultiGPUExecMode() == gpu3d::SORT_LAST && _texture))
        return;
...
void ACDSamplerImp::sync(acd_uint gpuId, bool forcePreload)
{
    ACD_ASSERT(
        // Check texture completeness before synchronize texture in GPU/system memory
        if ( _enabled && _texture && !_texture->wellDefined() )
    ...
    /// Synchronize texture unit state with the current bound texture
    if ( _texture )
        _syncTexture(gpuId, forcePreload);
