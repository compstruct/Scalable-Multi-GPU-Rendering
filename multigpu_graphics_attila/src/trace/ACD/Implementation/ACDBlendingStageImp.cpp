...
acd_bool ACDBlendingStageImp::getEnable(acd_uint renderTargetID) const
{
    GLOBALPROFILER_ENTERREGION("ACD", "", "")
    ACD_ASSERT(
        if ( renderTargetID >= MAX_RENDER_TARGETS )
            panic("ACDBlendingStageImp", "getEnabled", "Render target value greater than the maximum allowed id");
    )

    return _enabled[renderTargetID];
    GLOBALPROFILER_EXITREGION()
}
...
void ACDBlendingStageImp::setColorMask(acd_uint renderTargetID, acd_bool red, acd_bool green, acd_bool blue, acd_bool alpha)
{
    ...
    ACD_ASSERT(
        if ( renderTargetID >= MAX_RENDER_TARGETS )
            panic("ACDBlendingStageImp", "setColorMask", "Render target value greater than the maximum allowed id");
    )
...
acd_bool ACDBlendingStageImp::getColorEnable(acd_uint renderTargetID)
{
    ACD_ASSERT(
        if ( renderTargetID >= MAX_RENDER_TARGETS )
            panic("ACDBlendingStageImp", "getColorEnable", "Render target value greater than the maximum allowed id");
    )
...
void ACDBlendingStageImp::enableColorWrite(acd_uint renderTargetID)
{
    ACD_ASSERT(
        if ( renderTargetID >= MAX_RENDER_TARGETS )
            panic("ACDBlendingStageImp", "enableColorWrite", "Render target value greater than the maximum allowed id");
    )
...
void ACDBlendingStageImp::disableColorWrite(acd_uint renderTargetID)
{
    ACD_ASSERT(
        if ( renderTargetID >= MAX_RENDER_TARGETS )
            panic("ACDBlendingStageImp", "disableColorWrite", "Render target value greater than the maximum allowed id");
    )

    _colorWriteEnabled[renderTargetID] = false;
}

void ACDBlendingStageImp::getBlendOptions(acd_uint rt, ACD_BLEND_OPTION &srcRGB, ACD_BLEND_OPTION &destRGB,
		                          ACD_BLEND_OPTION &srcAlpha, ACD_BLEND_OPTION &destAlpha)
{
    srcRGB = _srcBlend[rt];
    destRGB = _destBlend[rt];
    srcAlpha = _srcBlendAlpha[rt];
    destAlpha = _destBlendAlpha[rt];
}
...
void ACDBlendingStageImp::sync(bool forcePreload)
{
