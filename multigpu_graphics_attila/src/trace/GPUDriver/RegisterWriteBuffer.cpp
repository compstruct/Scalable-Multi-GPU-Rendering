...
RegisterWriteBuffer::RegisterWriteBuffer(GPUDriver* driver, WritePolicy wp) :
    driver(driver), writePolicy(wp),
...
{
    RegisterIdentifier auxri(reg, index, numGPUs);
	...
	for (u32bit gpuId = 0; gpuId < numGPUs; gpuId++) {
            RegisterIdentifier auxriPerGPU(reg, index, gpuId);
            WriteBufferIt auxPerGPU = registerStatus.find(auxriPerGPU);
	    if (auxPerGPU != registerStatus.end()) {
	        registerChanged = registerChanged || (auxPerGPU->second.data != data) || (auxPerGPU->second.md != md);
	    }
	}
    ...
    if (writePolicy == WaitUntilFlush)
    {
        ...
        if (registerChanged)
        {
            RegisterIdentifier ri(reg, index, numGPUs);
	...
	// Inmediate forwarding
        driver->_sendAGPTransaction(new AGPTransaction(reg, index, data, md));
    }
}

void RegisterWriteBuffer::writeRegister(u32bit gpuId, GPURegister reg, u32bit index,
                                        const GPURegData& data, u32bit md)
{
    RegisterIdentifier auxri(reg, index, gpuId);
    WriteBufferIt aux = registerStatus.find(auxri);
    ...
    if (aux != registerStatus.end())
    {
        ...
        RegisterIdentifier auxriAll(reg, index, numGPUs);
        WriteBufferIt auxAll = registerStatus.find(auxriAll);
	if (auxAll != registerStatus.end()) {
	    registerChanged = registerChanged || (auxAll->second.data != data) || (auxAll->second.md != md);
	}
	...
	std::cout << reg << std::endl;
        panic("RegisterWriteBuffer", "writeRegisterGPUId", "GPU Register not registered.");
    }
    ...
    if (writePolicy == WaitUntilFlush)
    {
        ...
        if (registerChanged)
        {
            RegisterIdentifier ri(reg, index, gpuId);
            WriteBufferIt it = writeBuffer.find(ri);
	...
	// Inmediate forwarding
        driver->_sendAGPTransaction(gpuId, new AGPTransaction(reg, index, data, md));
    }
}
...
void RegisterWriteBuffer::unbufferedWriteRegister(GPURegister reg, u32bit index, const GPURegData& data, u32bit md)
{
    ...
    if (aux != registerStatus.end())
    {
        ...
        panic("RegisterWriteBuffer", "unbufferedWriteRegister", "GPU Register not registered.");
...
void RegisterWriteBuffer::readRegister(u32bit gpuId, gpu3d::GPURegister reg, u32bit index, gpu3d::GPURegData &data)
{
    RegisterIdentifier auxri(reg, index, gpuId);
    WriteBufferIt aux = registerStatus.find(auxri);
...
void RegisterWriteBuffer::flush()
{
    ...
    RegisterIdentifier ri(GPU_INDEX_STREAM, 0, numGPUs);
    WriteBufferIt it = writeBuffer.find(ri);
	...
	u32bit gpuId = it->first.gpuId;
	if (gpuId == numGPUs)
            driver->_sendAGPTransaction(new AGPTransaction(reg, index, it->second.data, it->second.md));
	else 
            driver->_sendAGPTransaction(gpuId, new AGPTransaction(reg, index, it->second.data, it->second.md));
    }
...
void RegisterWriteBuffer::initAllRegisterStatus ()
{
    ...
    for (int i = 0; i < MAX_STREAM_BUFFERS; i++) //MAX_STREAM_BUFFERS
    {
	    ...
	    for (u32bit gpuId = 0; gpuId < numGPUs; gpuId++) {
                initRegisterStatus( gpuId, GPU_STREAM_ADDRESS, i, data, j); //0
	    }
	}

        initRegisterStatus( GPU_STREAM_STRIDE, i, data, 0); //0
        for (u32bit gpuId = 0; gpuId < numGPUs; gpuId++) {
            initRegisterStatus( gpuId, GPU_STREAM_STRIDE, i, data, 0); //0
	}

        data.streamData = SD_U32BIT;
        initRegisterStatus( GPU_STREAM_DATA, i, data, 0); //SD_U32BIT
        for (u32bit gpuId = 0; gpuId < numGPUs; gpuId++) {
            initRegisterStatus( gpuId, GPU_STREAM_DATA, i, data, 0); //SD_U32BIT
	}
        ...
        for (u32bit gpuId = 0; gpuId < numGPUs; gpuId++) {
            initRegisterStatus(gpuId, GPU_STREAM_ELEMENTS, i, data, 0); //0
            initRegisterStatus(gpuId, GPU_STREAM_FREQUENCY, i, data, 0);  // 0
	}
        
        data.booleanVal = false;
        initRegisterStatus( GPU_D3D9_COLOR_STREAM, i, data, 0); //false
        for (u32bit gpuId = 0; gpuId < numGPUs; gpuId++) {
            initRegisterStatus( gpuId, GPU_D3D9_COLOR_STREAM, i, data, 0); //false
	}
    }
   
    if (multiGPUExecMode == GPUPD && execRound == 1 && numGPUs > 1) {
        data.uintVal = 0;
        for (int j = 0; j < 100; j++) {
            for (int gpuId = 0; gpuId < numGPUs; gpuId++) {
                for (int i = 0; i < cpdDrawCommBatchSize; i++) {
                    initRegisterStatus(gpuId, GPU_STREAM_ADDRESS,
				       MAX_STREAM_BUFFERS + gpuId * cpdDrawCommBatchSize + i,
				       data, j);
                }
            }
            for (int gpuId = 0; gpuId < numGPUs; gpuId++) {
                for (int i = 0; i < cpdDrawCommBatchSize; i++) {
                    initRegisterStatus(gpuId, GPU_STREAM_ADDRESS,
				       MAX_STREAM_BUFFERS + (gpuId + numGPUs) * cpdDrawCommBatchSize + i,
				       data, j);
                }
            }
	    ...
	    if (runaheadInGPUpd) {
                for (int gpuId = 0; gpuId < numGPUs; gpuId++) {
                    for (int i = 0; i < cpdDrawCommBatchSize; i++) {
                        initRegisterStatus(gpuId, GPU_STREAM_ADDRESS,
	            		           MAX_STREAM_BUFFERS + (gpuId + 2*numGPUs) * cpdDrawCommBatchSize + i,
	            		           data, j);
                    }
                }
                for (int gpuId = 0; gpuId < numGPUs; gpuId++) {
                    for (int i = 0; i < cpdDrawCommBatchSize; i++) {
                        initRegisterStatus(gpuId, GPU_STREAM_ADDRESS,
	            		           MAX_STREAM_BUFFERS + (gpuId + 3*numGPUs) * cpdDrawCommBatchSize + i,
	            		           data, j);
                    }
                }
	    }
    ...
    initRegisterStatus( GPU_STREAM_START_OF_FAN, 0, data, 0); //0
    for (u32bit gpuId = 0; gpuId < numGPUs; gpuId++) {
        initRegisterStatus( gpuId, GPU_STREAM_START, 0, data, 0); //0
        initRegisterStatus( gpuId, GPU_STREAM_COUNT, 0, data, 0); //0
        initRegisterStatus( gpuId, GPU_STREAM_START_OF_FAN, 0, data, 0); //0
    }
    ...
    data.booleanVal = false;
    initRegisterStatus( GPU_FRAME_DUP_IN_SORT_LAST, 0, data, 0); // true
    ...
    initRegisterStatus( GPU_COMP_ZSTENCILBUFFER_ADDR, 0, data, 0);
	...
	for (u32bit gpuId = 0; gpuId < numGPUs; gpuId++)
            initRegisterStatus( gpuId, GPU_TEXTURE_ADDRESS, t, data, 0); //Special
    }
    ...
    for (u32bit gpuId = 0; gpuId < numGPUs; gpuId++)
    {
        ...
        {
            data.booleanVal = false;
            initRegisterStatus( gpuId, GPU_TEXTURE_ENABLE, i, data, 0); //false

            data.txMode = GPU_TEXTURE2D;
            initRegisterStatus( gpuId, GPU_TEXTURE_MODE, i, data, 0); //GPU_TEXTURE2D

            data.uintVal = 0;
            initRegisterStatus( gpuId, GPU_TEXTURE_WIDTH, i, data, 0); //0
            initRegisterStatus( gpuId, GPU_TEXTURE_HEIGHT, i, data, 0); //0
            initRegisterStatus( gpuId, GPU_TEXTURE_DEPTH, i, data, 0); //0
            initRegisterStatus( gpuId, GPU_TEXTURE_WIDTH2, i, data, 0); //0
            initRegisterStatus( gpuId, GPU_TEXTURE_HEIGHT2, i, data, 0); //0
            initRegisterStatus( gpuId, GPU_TEXTURE_DEPTH2, i, data, 0); //0
            initRegisterStatus( gpuId, GPU_TEXTURE_BORDER, i, data, 0); //0

            data.txFormat = GPU_RGBA8888;
            initRegisterStatus( gpuId, GPU_TEXTURE_FORMAT, i, data, 0); //GPU_RGBA8888

            data.booleanVal = false;
            initRegisterStatus( gpuId, GPU_TEXTURE_REVERSE, i, data, 0); //false
            initRegisterStatus( gpuId, GPU_TEXTURE_D3D9_COLOR_CONV, i, data, 0); //false
            initRegisterStatus( gpuId, GPU_TEXTURE_D3D9_V_INV, i, data, 0); //false

            data.txCompression = GPU_NO_TEXTURE_COMPRESSION;
            initRegisterStatus( gpuId, GPU_TEXTURE_COMPRESSION, i, data, 0); //GPU_NO_TEXTURE_COMPRESSION

            data.txBlocking = GPU_TXBLOCK_TEXTURE;
            initRegisterStatus( gpuId, GPU_TEXTURE_BLOCKING, i, data, 0); //GPU_TXBLOCK_TEXTURE

            data.qfVal[0] = 0;
            data.qfVal[1] = 0;
            data.qfVal[2] = 0;
            data.qfVal[3] = 1;
            initRegisterStatus( gpuId, GPU_TEXTURE_BORDER_COLOR, i, data, 0); // QUADD  0 0 0 1
            
            data.txClamp = GPU_TEXT_CLAMP;
            initRegisterStatus( gpuId, GPU_TEXTURE_WRAP_S, i, data, 0); // GPU_TEXT_CLAMP
            initRegisterStatus( gpuId, GPU_TEXTURE_WRAP_T, i, data, 0); // GPU_TEXT_CLAMP
            initRegisterStatus( gpuId, GPU_TEXTURE_WRAP_R, i, data, 0); // GPU_TEXT_CLAMP

            data.booleanVal = false;
            initRegisterStatus( gpuId, GPU_TEXTURE_NON_NORMALIZED, i, data, 0);  // FALSE
            
            data.txFilter = GPU_NEAREST;
            initRegisterStatus( gpuId, GPU_TEXTURE_MIN_FILTER, i, data, 0); //GPU_NEAREST
            initRegisterStatus( gpuId, GPU_TEXTURE_MAG_FILTER, i, data, 0); //GPU_NEAREST


            data.booleanVal = false;
            initRegisterStatus( gpuId, GPU_TEXTURE_ENABLE_COMPARISON, i, data, 0);  // FALSE
            
            data.compare = GPU_LEQUAL;
            initRegisterStatus( gpuId, GPU_TEXTURE_COMPARISON_FUNCTION, i, data, 0);  // GPU_LEQUAL
            
            data.booleanVal = false;
            initRegisterStatus( gpuId, GPU_TEXTURE_SRGB, i, data, 0);  // FALSE

            data.f32Val = 0.0f;
            initRegisterStatus( gpuId, GPU_TEXTURE_MIN_LOD, i, data, 0); //0.0f;

            data.f32Val = 12.0f;
            initRegisterStatus( gpuId, GPU_TEXTURE_MAX_LOD, i, data, 0); //12.0f

            data.f32Val = 0.0f;
            initRegisterStatus( gpuId, GPU_TEXTURE_LOD_BIAS, i, data, 0); //0.0f;

            data.uintVal = 0;
            initRegisterStatus( gpuId, GPU_TEXTURE_MIN_LEVEL, i, data, 0); //0

            data.uintVal = 12;
            initRegisterStatus( gpuId, GPU_TEXTURE_MAX_LEVEL, i, data, 0); //12

            data.f32Val = 0.0f;
            initRegisterStatus( gpuId, GPU_TEXT_UNIT_LOD_BIAS, i, data, 0); //0.0f

            data.uintVal = 1;
            initRegisterStatus( gpuId, GPU_TEXTURE_MAX_ANISOTROPY, i, data, 0); //1
        }
    }
    ...
    initRegisterStatus( GPU_COMP_Z_BUFFER_CLEAR, 0, data, 0); //0x00ffffff

    data.uintVal = 0;
    initRegisterStatus( GPU_ZSTENCIL_STATE_BUFFER_MEM_ADDR, 0, data, 0);

    data.booleanVal = false;
    initRegisterStatus( GPU_ZSTENCILBUFFER_DEFINED, 0, data, 0); //false
    ...
    for (int i = 0; i < MAX_RENDER_TARGETS; i++) //MAX_RENDER_TARGETS
    {
        ...
        initRegisterStatus( GPU_COMP_RENDER_TARGET_ADDRESS, i, data, 0); //0x600000 + 0x100000 * i
    ...
    data.qfVal[2] = 0.0f;
    data.qfVal[3] = 0.0f;
    initRegisterStatus( GPU_COMP_COLOR_BUFFER_CLEAR, 0, data, 0); // QUAD 0 0 0 0
...
void RegisterWriteBuffer::initRegisterStatus (u32bit gpuId, GPURegister reg, u32bit index,
                                              const GPURegData& data, u32bit md)
{
...
void RegisterWriteBuffer::dumpRegisterInfo(ofstream& out, GPURegister reg, u32bit index,const GPURegData& data, u32bit md)
{
    ...
    switch (reg)
    {
        ...
        case GPU_STREAM_START_OF_FAN: out << "GPU_STREAM_START_OF_FAN" << "(Index: " <<index<<", Data: "<<data.uintVal<<", MD:"<< md; break; 
        case GPU_INDEX_MODE: out << "GPU_INDEX_MODE" << "(Index: " <<index<<", Data: "<<data.booleanVal<<", MD:"<< md; break; 
        case GPU_FRAME_DUP_IN_SORT_LAST: out << "GPU_FRAME_DUP_IN_SORT_LAST" << "(Index: " <<index<<", Data: "<<data.booleanVal<<", MD:"<< md; break; 
        ...
        case GPU_COMP_ZSTENCILBUFFER_ADDR: out << "GPU_COMP_ZSTENCILBUFFER_ADDR" << "(Index: " <<index<<", Data: "<<data.uintVal<<", MD:"<< md; break; 
        ...
        case GPU_COMP_Z_BUFFER_CLEAR: out << "GPU_COMP_Z_BUFFER_CLEAR" << "(Index: " <<index<<", Data: "<<data.uintVal<<", MD:"<< md; break;
        case GPU_ZSTENCIL_STATE_BUFFER_MEM_ADDR: out << "GPU_ZSTENCIL_STATE_BUFFER_MEM_ADDR" << "(Index: " <<index<<", Data: "<<data.uintVal<<", MD:"<< md; break;
        case GPU_ZSTENCILBUFFER_DEFINED: out << "GPU_ZSTENCILBUFFER_DEFINED" << "(Index: " <<index<<", Data: "<<data.booleanVal<<", MD:"<< md; break; 
        ...
        case GPU_COMP_RENDER_TARGET_ADDRESS: out << "GPU_COMP_RENDER_TARGET_ADDRESS" << "(Index: " <<index<<", Data: "<<data.uintVal<<", MD:"<< md; break;
        ...
        case GPU_COMP_COLOR_BUFFER_CLEAR: out << "GPU_COMP_COLOR_BUFFER_CLEAR" << "(Index: " <<index<<", Data: "<<data.qfVal[0] <<" "<<data.qfVal[1] <<" "<<data.qfVal[2] <<" "<<data.qfVal[3] <<", MD:"<< md; break;
