...
struct BatchRunningInfoEntry {
    std::vector<u32bit> numTriangles;
    std::vector<u32bit> numVertexes;
    std::vector<u32bit> numFragments;

    std::vector<s64bit> geoProcessStartCycle;
    std::vector<s64bit> fragProcessStartCycle;

    std::vector<s64bit> scheduledTrisPerGPU;
    std::vector<s64bit> processedTrisPerGPU;
    std::vector<s64bit> remainedTrisPerGPU;

    BatchRunningInfoEntry() {}
    BatchRunningInfoEntry(u32bit numGPUs) {
	numTriangles.resize(numGPUs, 0);
	numVertexes.resize(numGPUs, 0);
	numFragments.resize(numGPUs, 0);
	geoProcessStartCycle.resize(numGPUs, 0);
	fragProcessStartCycle.resize(numGPUs, 0);
	scheduledTrisPerGPU.resize(numGPUs, 0);
	processedTrisPerGPU.resize(numGPUs, 0);
	remainedTrisPerGPU.resize(numGPUs, 0);
    }
};
typedef std::pair<u32bit, u32bit> BatchIdentifier;
typedef std::map<BatchIdentifier, BatchRunningInfoEntry> BatchRunningInfo;

struct ScheduleTableEntry {
    s64bit totScheduledTris;
    s64bit totProcessedTris;
    s64bit numProcessedTris;
...
typedef std::vector<ScheduleTableEntry> ScheduleTable;

class GPUSchedInfo {
private:
    u32bit numGPUs;
    bool roundRobin;
    u32bit processedTrisUpdateThreshold;
    u32bit timesOfProcessedTrisUpdate;

    BatchRunningInfo batchRunningInfo;
    ScheduleTable scheduleTable;
    std::set<u32bit> compGPUs;

    u32bit totTriangles;
    s64bit totRenderingCycles;
    s64bit triangleRate;
    u32bit totVertexes;
    s64bit totGeoProcessCycles;
    s64bit vertexRate;
    u32bit totFragments;
    s64bit totFragProcessCycles;
    s64bit fragmentRate;

    bool scheduleParamCalculated;
    u32bit lastScheduledGPUId;

    std::vector<std::vector<u32bit> > fragsPerGPU;
    std::vector<std::vector<u64bit> > renderingStartCyclePerGPU;
    std::vector<std::vector<u64bit> > renderingStopCyclePerGPU;
    std::vector<std::vector<u64bit> > renderingCyclesPerGPU;
    std::vector<std::vector<u64bit> > unoverlappedRenderingCyclesPerGPU;

public:
    GPUSchedInfo(u32bit numGPUs);
    void setRoundRobin(bool roundRobin_) { roundRobin = roundRobin_; }
    void setProcessedTrisUpdateThreshold(u32bit thres_) { processedTrisUpdateThreshold = thres_; }
   
    bool isScheduleParamCalculated() const { return scheduleParamCalculated; } 
    void scheduleBatchToGPU (u32bit frameId, u32bit batchId, std::vector<u32bit> numTrisPerGPU);
    u32bit isScheduledToThisGPU(u32bit frameId, u32bit batchId, u32bit gpuId);
    u32bit findNextScheduleGPUId();
    void incNumProcessedVertexes(u32bit frameId, u32bit batchId, u32bit gpuId);
    void incNumProcessedFragments(u32bit frameId, u32bit batchId, u32bit gpuId);
    void recordGeoProcessStartCycle(u32bit frameId, u32bit batchId, u32bit gpuId, u64bit cycle);
    void recordGeoProcessCycles(u32bit frameId, u32bit batchId, u32bit gpuId, u64bit cycle);
    void recordFragProcessStartCycle(u32bit frameId, u32bit batchId, u32bit gpuId, u64bit cycle);
    void recordFragProcessCycles(u32bit frameId, u32bit batchId, u32bit gpuId, u64bit cycle);
    void resetScheduleTable();
    void getRemainingTris(std::vector<std::pair<u32bit, u32bit> > &remainingTris);
    void clearNumProcessedTris(u32bit gpuId);

    void insertCompGPU(u32bit gpuId) { compGPUs.insert(gpuId); }
    void removeCompGPU(u32bit gpuId) { compGPUs.erase(gpuId); }
    
    void allocProfilingEntry();
    void profileProcessedFragments(u32bit batchId, u32bit gpuId);
    void profileRenderingStartCycle(u32bit batchId, u32bit gpuId, u64bit cycle);
    void profileRenderingCycles(u32bit batchId, u32bit gpuId, u64bit cycle);
    void dumpProfilingResults();
};

struct CompScheduleTableEntry {
    u32bit numGPUs;
    bool anyRtBlend;
    bool ready;
    u32bit compPhaseId;
    ...
    CompScheduleTableEntry(u32bit numGPUs_) {
	numGPUs = numGPUs_;
	reset();
    }
...
private:
    ...
    unsigned numGPUs;
    std::vector<CompScheduleTableEntry> compScheduleTable;

public:

    CompScheduler(u32bit numGPUs_);
    void clearAfterComp(u32bit gpuId);
    void setAnyRtBlend(u32bit gpuId, bool anyRtBlend);
    void setReady(u32bit gpuId);
    void resetReceiving(u32bit gpuId);
    bool isSending(u32bit gpuId);
    void resetSending(u32bit gpuId);
    u32bit getSendToGPU(u32bit gpuId);
    void setSendToGPU(s32bit gpuId);
    bool compWithCompBuffer(u32bit srcGPUId, u32bit dstGPUId);
};
...
private:
    ...
    std::vector<u32bit> agpTransactionsGeneratedPerGPU;
    ...
    bool _sendAGPTransaction( u32bit gpuId, gpu3d::AGPTransaction* agpt );
    ...
    // HACK: number of GPUs in the multi-GPU architecture
    u32bit numGPUs;
    gpu3d::MultiGPUExecMode multiGPUExecMode;
    ...
    GPUSchedInfo* gpuSchedInfo;
    CompScheduler* compSched;
    std::map<BatchIdentifier, u32bit> numTrisPerBatch;
    bool gpupdBatchEnabled;
    u32bit gpupdBatchThreshold;

    /**
     * Outstanding AGPTRansactions
     * HACK: change scalar to vector for multi-GPU architecture
     */
    //std::vector<gpu3d::AGPTransaction**> agpBuffers;
    std::vector<std::list<gpu3d::AGPTransaction*> > agpBuffers;
    ...
    u32bit displayScanWidth;
    u32bit displayScanHeight;
    u32bit fetchRate;
    bool doubleBuffer;
    bool forceMSAA;
...
public:
    ...
    void setGPUParameters(u32bit numGpus, u32bit multiGPUExecMode,
	u32bit cpdDrawCommBatchSize, bool runaheadInGPUpd, u32bit execRound, u32bit minKDTreeInSortLast,
	u32bit gpuMemSz, u32bit sysMemSz, u32bit blocksz, u32bit sblocksz,
   ...
   // u32bit getGPUBlockSize() const;
    ...
    bool writeMemoryToGPU( u32bit gpuId, u32bit md, const u8bit* data, u32bit dataSize, bool isLocked = false );
    ...
    bool writeMemoryToGPU( u32bit gpuId, u32bit md, u32bit offset, const u8bit* data, u32bit dataSize, bool isLocked = false );
    ...
    bool writeMemoryPreloadToGPU(u32bit gpuId, u32bit md, u32bit offset, const u8bit* data, u32bit dataSize);
    ...
    void writeGPURegister( u32bit gpuId, gpu3d::GPURegister regId, gpu3d::GPURegData data, u32bit md = 0 );
    ...
    void writeGPURegister( u32bit gpuId, gpu3d::GPURegister regId, u32bit index, gpu3d::GPURegData data, u32bit md = 0);
    ...
    void writeGPUAddrRegister( u32bit gpuId, gpu3d::GPURegister regId, u32bit index, u32bit md, u32bit offset = 0);
     ...
     */
    gpu3d::AGPTransaction* nextAGPTransaction(u32bit gpuId);

    u32bit getNumGPUs() const { return numGPUs; }
    gpu3d::MultiGPUExecMode getMultiGPUExecMode() const { return multiGPUExecMode; }
    u32bit getCpdDrawCommBatchSize() const { return cpdDrawCommBatchSize; }
    bool isRunaheadInGPUpd() const { return runaheadInGPUpd; }
    u32bit getExecRound() const  { return execRound; }
    gpu3d::exchangedIndexesInGPUpd_t *getExchangedIndexesInGPUpd() const { return exchangedIndexesInGPUpd; }
    void setPtrOfExchangedIndexesInGPUpd(gpu3d::exchangedIndexesInGPUpd_t *ptr) { exchangedIndexesInGPUpd = ptr; }
    gpu3d::kdTreeIndexesInSortLast_t *getKDTreeIndexesInSortLast() const { return kdTreeIndexesInSortLast; }
    void setPtrOfKDTreeIndexesInSortLast(gpu3d::kdTreeIndexesInSortLast_t *ptr) { kdTreeIndexesInSortLast = ptr; }
    u32bit getMinKDTreeInSortLast() const { return minKDTreeInSortLast; }
    GPUSchedInfo* getGPUSchedInfo() { return gpuSchedInfo; }
    CompScheduler* getCompSched() { return compSched; }
    void setGPUpdBatchEnabled(bool gpupdBatchEnabled_) { gpupdBatchEnabled = gpupdBatchEnabled_; }
    void setGPUpdBatchThreshold(u32bit gpupdBatchThreshold_) { gpupdBatchThreshold = gpupdBatchThreshold_; }
    bool isGPUpdBatchEnabled() const { return gpupdBatchEnabled; }
    u32bit getGPUpdBatchThreshold() const { return gpupdBatchThreshold; }
    void setNumTrisPerBatch(u32bit frameId, u32bit batchId, u32bit numTris);
    u32bit getNumTrisPerBatch(u32bit frameId, u32bit batchId);
