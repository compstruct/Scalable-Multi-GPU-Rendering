...
void D3D9VolumeLockUpdater::update(size_t buffer_size, void * buffer) {
    ...
    {
        char buff[256];
        sprintf(buff, "D3D9VolumeLockUpdater::update => Buffer size is %ld.  Required size is %d.  Format is %08x.  Ignoring update.\n",
...
void D3D9SurfaceLockUpdater::update(size_t buffer_size, void * buffer)
{
    ...
    {
        char buff[256];
        sprintf(buff, "D3D9SurfaceLockUpdater::update => Buffer size is %ld.  Required size is %d.  Format is %08x.  Ignoring update.\n",
            buffer_size, requiredSize, description.Format);
        includelog::logfile().write(includelog::Warning, buff);
        return;
    }
...
void D3D9IndexBufferLockUpdater::update(size_t buffer_size, void * buffer)
{
    ...
    {
        char buff[256];
        sprintf(buff, "D3D9IndexBufferLockUpdater::update => Buffer size is %ld.  Required size is %ld.  Ignoring update.\n",
...
void D3D9VertexBufferLockUpdater::update(size_t buffer_size, void * buffer)
{
    ...
    {
        char buff[256];
        sprintf(buff, "D3D9VertexBufferLockUpdater::update => Buffer size is %ld.  Required size is %ld.  Ignoring update.\n",
            buffer_size, dataSize);
        includelog::logfile().write(includelog::Warning, buff);
        return;
    }
