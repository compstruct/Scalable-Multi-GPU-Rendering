...
VectorShaderFetch::VectorShaderFetch(u32bit gpuId, u32bit unitID_, ShaderEmulator &shEmu, u32bit nThreads, u32bit resources,
    ...
    //  Pre-initializations.
    MultiClockBox(gpuId, name, parent), unitID(unitID_), shEmul(shEmu), numThreads(nThreads), numResources(resources),
...
//printf("ShFetch %s -> %p \n", fullName, this);
    ...
    vertexProcessCycles = &getSM(gpuId).getNumericStatistic("VertexProcessCycles", u32bit(0), fullName, postfix);
    triangleProcessCycles = &getSM(gpuId).getNumericStatistic("TriangleProcessCycles", u32bit(0), fullName, postfix);
    fragmentProcessCycles = &getSM(gpuId).getNumericStatistic("FragmentProcessCycles", u32bit(0), fullName, postfix);
    ...
    globalVertexProcessCycles = &getSM().getNumericStatistic("VertexProcessCycles", u32bit(0), fullName, postfix);
    globalTriangleProcessCycles = &getSM().getNumericStatistic("TriangleProcessCycles", u32bit(0), fullName, postfix);
    globalFragmentProcessCycles = &getSM().getNumericStatistic("FragmentProcessCycles", u32bit(0), fullName, postfix);
...
void VectorShaderFetch::updateFetchStatistics()
{
    ...
    globalNThReadyAvg->incavg(readyThreads);
    globalNThReadyMax->max(readyThreads);
    globalNThReadyMin->min(readyThreads);
    globalNThReady->inc(readyThreads);
    globalNThBlocked->inc(blockedThreads);
    globalNThFinished->inc(endThreadFIFO.items());
    globalNThFree->inc(freeVectorThreads);
...
void VectorShaderFetch::fetchStage(u64bit cycle)
{
    ...
    if (freeThreadFIFO.items() == numThreads) {
        emptyCycles->inc();
        globalEmptyCycles->inc();
    } else if (cyclesToNextFetch > 0) {
        fetchCycles->inc();
        globalFetchCycles->inc();
    } else if (readyThreads == 0) {
        noReadyCycles->inc();
        globalNoReadyCycles->inc();
    }
...
void VectorShaderFetch::freeVectorThread(u64bit cycle, u32bit threadID, u32bit elementResources)
{
    ...
    switch(partition)
    {
        case VERTEX_PARTITION:

            vertexProcessCycles->inc(cycle - threadArray[threadID].startCycle);            
            globalVertexProcessCycles->inc(cycle - threadArray[threadID].startCycle);            
            break;

        case TRIANGLE_PARTITION:

            triangleProcessCycles->inc(cycle - threadArray[threadID].startCycle);            
            globalTriangleProcessCycles->inc(cycle - threadArray[threadID].startCycle);            
            break;

        case FRAGMENT_PARTITION:

            fragmentProcessCycles->inc(cycle - threadArray[threadID].startCycle);            
            globalFragmentProcessCycles->inc(cycle - threadArray[threadID].startCycle);            
            break;

        default:
            panic("VectorShaderFetch", "freeVectorThread", "Invalid partition.");
            break;
    }
    ...
    if (debugThreads.count(threadID)) {
        debugThreads.erase(threadID);
        char filename[32];
        sprintf(filename, "debugVSFInput%d.txt", unitID);
        FILE *debug = fopen(filename, "a+");
        fprintf(debug, "%s => Freeing resources. ThreadID = %d | Partition = %s | ElemResources = %d | FreeResources = %d | NumResources = %d\n",
                getName(), threadID, (partition == VERTEX_PARTITION) ? "VERTEX" : "FRAGMENT", 
                (partition != VERTEX_PARTITION) ? GPU_MAX(activeInputs[partition], resourceAlloc[partition]) : GPU_MAX(activeInputs[partition],
        	GPU_MAX(resourceAlloc[partition], activeOutputs[partition])), freeResources, numResources);
        fflush(debug);
        fclose(debug);
    }
...
void VectorShaderFetch::processShaderCommand(ShaderCommand *command, u32bit partition)
{
    ...
    switch(command->getCommandType())
    {
        case LOAD_PROGRAM:
            ...
            if (debugThreads.size() > 0 && partition == FRAGMENT_PARTITION) {
	        char filename[32];
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s >> LOAD_PROGRAM Size %d PC %06x\n",
                        getName(), command->getProgramCodeSize(), command->getLoadPC());
	        fflush(debug);
	        fclose(debug);
	    }
        ...
        case PARAM_WRITE:
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                for (u32bit i = firstAddr; i < lastAddr; i ++)
                    fprintf(debug, "%s >> PARAM_WRITE %d = {%f, %f, %f, %f}\n",
                            getName(), i,
	        	    values[i - firstAddr][0], values[i - firstAddr][1],
                            values[i - firstAddr][2],values[i - firstAddr][3]);
	        fflush(debug);
	        fclose(debug);
	    }
        ...
        case SET_INIT_PC:
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => SET_INIT_PC[", getName());
                switch(command->getShaderTarget())
                {
                    case VERTEX_TARGET:
                        fprintf(debug, "VERTEX_TARGET");
                        break;
                    case FRAGMENT_TARGET:
                        fprintf(debug, "FRAGMENT_TARGET");
                        break;
                    case TRIANGLE_TARGET:
                        fprintf(debug, "TRIANGLE_TARGET");
                        break;
                    case MICROTRIANGLE_TARGET:
                        fprintf(debug, "MICROTRIANGLE_TARGET");
                        break;
                    default:
                        panic("VectorShaderFetch", "processShaderCommand", "Undefined shader target.");
                        break;
        ...
        case SET_THREAD_RES:
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                ...
                fprintf(debug, "%s => resourceAlloc[%s] = %d, maxResources =  %d\n",
                        getName(), buffer, resourceAlloc[command->getShaderTarget()], maxThreadResources);
	        fflush(debug);
	        fclose(debug);
	    }
        ...
        case SET_IN_ATTR:
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => Setting input attribute %d as %s for partition %s\n", getName(), command->getAttribute(),
                        command->isAttributeActive()?"ACTIVE":"INACTIVE",
                        (partition == VERTEX_PARTITION)?"VERTEX":((partition == FRAGMENT_PARTITION)?"FRAGMENT":"TRIANGLE"));
	        fflush(debug);
	        fclose(debug);
        ...
        case SET_OUT_ATTR:
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => Setting output attribute %d as %s for partition %s\n", getName(), command->getAttribute(),
                        command->isAttributeActive()?"ACTIVE":"INACTIVE",
                        (partition == VERTEX_PARTITION)?"VERTEX":((partition == FRAGMENT_PARTITION)?"FRAGMENT":"TRIANGLE"));
	        fflush(debug);
	        fclose(debug);
        ...
        case SET_MULTISAMPLING:
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => Setting multisampling register: %s\n", getName(), command->multisamplingEnabled()?"ENABLED":"DISABLED");
	        fflush(debug);
	        fclose(debug);
	    }
        ...
        case SET_MSAA_SAMPLES:
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => Setting MSAA samples register: %d\n", getName(), command->samplesMSAA());
	        fflush(debug);
	        fclose(debug);
	    }
...
void VectorShaderFetch::processShaderInput(u64bit cycle, ShaderInput *input)
{
    ...
    if (traceVertex)
    {
        ...
        {
            printf("%s => cycle %lld Enabling trace for vertex %d instance %d at vector %d element %d\n", getName(), cycle,
    ...
    if (traceFragment)
    {
            ...
            (input->getShaderInputID().id.fragmentID.y == watchFragmentY))
        {
            ...
            printf("%s => cycle %lld Enabling trace for pixel (%d, %d) triangle ID %d at vector %d element %d\n", getName(), cycle,
    ...
    if (input->getDebug())
    {
        debugThreads.insert(inputThread);

        char filename[32];
        sprintf(filename, "debugVSFInput%d.txt", unitID);
        FILE *debug = fopen(filename, "a+");
        fprintf(debug, "%s => cycle %lld Enabling trace for pixel (%d, %d) triangle ID %d at vector %d element %d unit %d entry %d\n",
        	getName(), cycle,
                input->getShaderInputID().id.fragmentID.x, input->getShaderInputID().id.fragmentID.y,
                input->getShaderInputID().id.fragmentID.triangle, inputThread, nextInputElement,
        	input->getUnit(), input->getEntry());
        QuadFloat* attributes = input->getAttributes();
	...
	threadArray[inputThread].startCycle = cycle;
    ...
    globalInputRegisters->inc(resourceAlloc[partition]);
...
void VectorShaderFetch::processDecodeCommand(ShaderDecodeCommand *decodeCommand)
{
    ...
    switch(decodeCommand->getCommandType())
    {
	    ...
	    if (debugThreads.count(vectorThreadID)) {
	        char filename[32];
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => UNBLOCK_THREAD VectorThreadID = %d PC = %x\n", getName(), vectorThreadID, pc);
	        fflush(debug);
	        fclose(debug);
	    }
	    ...
	    if (debugThreads.count(vectorThreadID)) {
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => BLOCK_THREAD VectorThreadID = %d PC = %x\n", getName(), vectorThreadID, pc);
	        fflush(debug);
	        fclose(debug);
	    }
	    ...
	    if (debugThreads.count(vectorThreadID)) {
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => END_THREAD VectorThreadID = %d PC = %x\n", getName(), vectorThreadID, pc);
	        fflush(debug);
	        fclose(debug);
	    }
	    ...
	    if (debugThreads.count(vectorThreadID)) {
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => REPEAT_LAST VectorThreadID = %d PC = %x\n", getName(), vectorThreadID, pc);
	        fflush(debug);
	        fclose(debug);
	    }
	    ...
	    if (debugThreads.count(vectorThreadID)) {
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => NEW_PC VectorThreadID = %d PC = %x\n", getName(), vectorThreadID, pc);
	        fflush(debug);
	        fclose(debug);
	    }
	    ...
	    if (debugThreads.count(vectorThreadID)) {
	        ...
	        sprintf(filename, "debugVSFInput%d.txt", unitID);
                FILE *debug = fopen(filename, "a+");
                fprintf(debug, "%s => ZEXPORT_THREAD VectorThreadID = %d PC = %x\n", getName(), vectorThreadID, pc);
	        fflush(debug);
	        fclose(debug);
	    }
    ...
    ShaderExecInstruction *&shExecInstr)
{
    ...
    if (scalarALU)
    {
        ...
        if (fetchedSIMD && (!shInstr->isScalar()))
        {
            ...
            shExecInstr->getShaderInstruction().setBatchCounter(threadArray[threadID].shInput[element]->getBatchCounter());
            shExecInstr->getShaderInstruction().setFrameCounter(threadArray[threadID].shInput[element]->getFrameCounter());

	    if (debugThreads.count(threadID)) {
	        shExecInstr->getShaderInstruction().setDebug(true);
	    }
...
void VectorShaderFetch::sendOutput(u64bit cycle, u32bit threadID, u32bit elementID)
{
    ...
    // Keep old 3D attributes
    shOutput->keep3DAttributes();
    ...
    if (debugThreads.count(threadID) && shOutput->getDebug())
    {
        char filename[32];
        sprintf(filename, "debugVSFOutput%d.txt", unitID);
        FILE *debug = fopen(filename, "a+");
        fprintf(debug, "%s => cycle %lld Sending output for pixel (%d, %d) triangle ID %d at vector %d element %d unit %d entry %d\n",
		getName(), cycle,
        	...
        	shOutput->getUnit(), shOutput->getEntry());
     
        QuadFloat *attributes = shOutput->getAttributes();
        for(u32bit o = 0; o < MAX_FRAGMENT_ATTRIBUTES; o++)
            fprintf(debug, "attrib[%d] = { %f, %f, %f, %f } active %s\n", o,
                    attributes[o][0], attributes[o][1], attributes[o][2], attributes[o][3],
		    activeOutAttr[FRAGMENT_PARTITION][o] ? "T" : "F");
        fflush(debug);
        fclose(debug);
...
void VectorShaderFetch::processOutputs(u64bit cycle)
{
    ...
    else
    {
        ...
        if ((nextOutputElement < vectorLength) || !endThreadFIFO.empty())
        {
		    ...
		    if (debugThreads.count(outputThread)) {
	                char filename[32];
	                sprintf(filename, "debugVSFInput%d.txt", unitID);
                        FILE *debug = fopen(filename, "a+");
                        fprintf(debug, "%s => Initiating transference of outputs for vectorThread = %d\n", getName(), outputThread);
		        fflush(debug);
		        fclose(debug);
		    }
