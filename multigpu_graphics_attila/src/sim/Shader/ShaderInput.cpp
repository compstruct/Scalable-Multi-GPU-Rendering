...
QuadFloat *ShaderInput::get3DAttributes()
{
    return attributes3D;
}

/*  Keep the shader input attributes in 3D space.  */
void ShaderInput::keep3DAttributes()
{
    GPU_ASSERT(
	if (attributes3D != NULL)
	    panic("ShaderInput", "keep3DAttributes", "attributes3D should have not been allocated.");
    )
    attributes3D = new QuadFloat[MAX_VERTEX_ATTRIBUTES];
    for (u32bit r = 0; r < MAX_VERTEX_ATTRIBUTES; r++)
        attributes3D[r] = attributes[r];
}
