...
TextureUnit::TextureUnit(u32bit gpuId, u32bit shaderID_, u32bit unitID_, u32bit cacheID, u32bit numTextUnits,
    TextureEmulator &textEmu, u32bit stFrags, u32bit reqCycle, u32bit reqQueueSz,
    ...
    /*  Pre-initializations.  */
    Box(gpuId, name, parent), shaderID(shaderID_), unitID(unitID_),
    txEmul(textEmu), stampFragments(stFrags), requestsCycle(reqCycle), requestQueueSize(reqQueueSz),
...

{
    ...
    /*  Create statistics.  */
    txRequests = &getSM(gpuId).getNumericStatistic("TextureRequests", u32bit(0), fullName, postfix);
    results = &getSM(gpuId).getNumericStatistic("TextureResults", u32bit(0), fullName, postfix);
    fetchOK = &getSM(gpuId).getNumericStatistic("FetchesOK", u32bit(0), fullName, postfix);
    fetchFail = &getSM(gpuId).getNumericStatistic("FetchesFailed", u32bit(0), fullName, postfix);
    fetchSkip = &getSM(gpuId).getNumericStatistic("FetchesSkiped", u32bit(0), fullName, postfix);
    fetchStallFetch = &getSM(gpuId).getNumericStatistic("FetchStallFetch", u32bit(0), fullName, postfix);
    fetchStallAddress = &getSM(gpuId).getNumericStatistic("FetchStallAddress", u32bit(0), fullName, postfix);
    fetchStallWaitRead = &getSM(gpuId).getNumericStatistic("FetchStallWaitRead", u32bit(0), fullName, postfix);
    fetchStallReadyRead = &getSM(gpuId).getNumericStatistic("FetchStallReadyRead", u32bit(0), fullName, postfix);
    readOK = &getSM(gpuId).getNumericStatistic("ReadsOK", u32bit(0), fullName, postfix);
    readFail = &getSM(gpuId).getNumericStatistic("ReadsFailed", u32bit(0), fullName, postfix);
    readBytes = &getSM(gpuId).getNumericStatistic("ReadBytesMemory", u32bit(0), fullName, postfix);
    readTrans = &getSM(gpuId).getNumericStatistic("ReadTransactions", u32bit(0), fullName, postfix);
    readyReadLevel = &getSM(gpuId).getNumericStatistic("ReadyReadQueueOccupation", u32bit(0), fullName, postfix);
    waitReadLevel = &getSM(gpuId).getNumericStatistic("WaitReadWindowOccupation", u32bit(0), fullName, postfix);
    resultLevel = &getSM(gpuId).getNumericStatistic("ResultQueueOccupation", u32bit(0), fullName, postfix);
    accessLevel = &getSM(gpuId).getNumericStatistic("AccessQueueOccupation", u32bit(0), fullName, postfix);
    requestLevel = &getSM(gpuId).getNumericStatistic("RequestQueueOccupation", u32bit(0), fullName, postfix);
    addrALUBusy = &getSM(gpuId).getNumericStatistic("AddressALUBusyCycles", u32bit(0), fullName, postfix);
    filterALUBusy = &getSM(gpuId).getNumericStatistic("FilterALUBusyCycles", u32bit(0), fullName, postfix);
    textResLat = &getSM(gpuId).getNumericStatistic("TextureResultLatency", u32bit(0), fullName, postfix);
    bilinearSamples = &getSM(gpuId).getNumericStatistic("BilinearSamples", u32bit(0), fullName, postfix);
    anisoRatio = &getSM(gpuId).getNumericStatistic("AnisotropyRatio", u32bit(0), fullName, postfix);
    addrCalcFinished  = &getSM(gpuId).getNumericStatistic("AddressCalculationFinished", u32bit(0), fullName, postfix);

    globalTxRequests = &getSM().getNumericStatistic("TextureRequests", u32bit(0), fullName, postfix);
    globalResults = &getSM().getNumericStatistic("TextureResults", u32bit(0), fullName, postfix);
    globalFetchOK = &getSM().getNumericStatistic("FetchesOK", u32bit(0), fullName, postfix);
    globalFetchFail = &getSM().getNumericStatistic("FetchesFailed", u32bit(0), fullName, postfix);
    globalFetchSkip = &getSM().getNumericStatistic("FetchesSkiped", u32bit(0), fullName, postfix);
    globalFetchStallFetch = &getSM().getNumericStatistic("FetchStallFetch", u32bit(0), fullName, postfix);
    globalFetchStallAddress = &getSM().getNumericStatistic("FetchStallAddress", u32bit(0), fullName, postfix);
    globalFetchStallWaitRead = &getSM().getNumericStatistic("FetchStallWaitRead", u32bit(0), fullName, postfix);
    globalFetchStallReadyRead = &getSM().getNumericStatistic("FetchStallReadyRead", u32bit(0), fullName, postfix);
    globalReadOK = &getSM().getNumericStatistic("ReadsOK", u32bit(0), fullName, postfix);
    globalReadFail = &getSM().getNumericStatistic("ReadsFailed", u32bit(0), fullName, postfix);
    globalReadBytes = &getSM().getNumericStatistic("ReadBytesMemory", u32bit(0), fullName, postfix);
    globalReadTrans = &getSM().getNumericStatistic("ReadTransactions", u32bit(0), fullName, postfix);
    globalReadyReadLevel = &getSM().getNumericStatistic("ReadyReadQueueOccupation", u32bit(0), fullName, postfix);
    globalWaitReadLevel = &getSM().getNumericStatistic("WaitReadWindowOccupation", u32bit(0), fullName, postfix);
    globalResultLevel = &getSM().getNumericStatistic("ResultQueueOccupation", u32bit(0), fullName, postfix);
    globalAccessLevel = &getSM().getNumericStatistic("AccessQueueOccupation", u32bit(0), fullName, postfix);
    globalRequestLevel = &getSM().getNumericStatistic("RequestQueueOccupation", u32bit(0), fullName, postfix);
    globalAddrALUBusy = &getSM().getNumericStatistic("AddressALUBusyCycles", u32bit(0), fullName, postfix);
    globalFilterALUBusy = &getSM().getNumericStatistic("FilterALUBusyCycles", u32bit(0), fullName, postfix);
    globalTextResLat = &getSM().getNumericStatistic("TextureResultLatency", u32bit(0), fullName, postfix);
    globalBilinearSamples = &getSM().getNumericStatistic("BilinearSamples", u32bit(0), fullName, postfix);
    globalAnisoRatio = &getSM().getNumericStatistic("AnisotropyRatio", u32bit(0), fullName, postfix);
    globalAddrCalcFinished  = &getSM().getNumericStatistic("AddressCalculationFinished", u32bit(0), fullName, postfix);
    ...
    for(u32bit aratio = 0; aratio <= MAX_ANISOTROPY; aratio++)
    {
        ...
        anisoRatioHisto[aratio] = &getSM(gpuId).getNumericStatistic(statsName, u32bit(0), fullName, postfix);
        globalAnisoRatioHisto[aratio] = &getSM().getNumericStatistic(statsName, u32bit(0), fullName, postfix);
    }
    ...
    for(u32bit bl = 1; bl <= (MAX_ANISOTROPY * 2); bl++)
    {
        ...
        bilinearsHisto[bl - 1] = &getSM(gpuId).getNumericStatistic(statsName, u32bit(0), fullName, postfix);
        globalBilinearsHisto[bl - 1] = &getSM().getNumericStatistic(statsName, u32bit(0), fullName, postfix);
    }

    magnifiedPixels = &getSM(gpuId).getNumericStatistic("MagnifiedPixels", u32bit(0), fullName, postfix);

    mipsSampled[0] = &getSM(gpuId).getNumericStatistic("MipsSampled1", u32bit(0), fullName, postfix);
    mipsSampled[1] = &getSM(gpuId).getNumericStatistic("MipsSampled2", u32bit(0), fullName, postfix);

    pointSampled = &getSM(gpuId).getNumericStatistic("PointSampled", u32bit(0), fullName, postfix);
    
    globalMagnifiedPixels = &getSM().getNumericStatistic("MagnifiedPixels", u32bit(0), fullName, postfix);

    globalMipsSampled[0] = &getSM().getNumericStatistic("MipsSampled1", u32bit(0), fullName, postfix);
    globalMipsSampled[1] = &getSM().getNumericStatistic("MipsSampled2", u32bit(0), fullName, postfix);

    globalPointSampled = &getSM().getNumericStatistic("PointSampled", u32bit(0), fullName, postfix);
...
//this, cycle, textureTickets, requests, requestQueueSize);
    ...
    globalReadyReadLevel->inc(numReadyReads);
    globalWaitReadLevel->inc(numWaitReads);
    globalResultLevel->inc(numResults);
    globalAccessLevel->inc(numCalcTexAcc + numFetchTexAcc);
    globalRequestLevel->inc(requests);
}
...
void TextureUnit::calculateAddress(u64bit cycle)
{
    ...
    if ((numCalcTexAcc > 0) && (addressALUCycles == 0))
    {
	    ...
	    globalAddrCalcFinished->inc();
            globalBilinearSamples->inc(filterLoops * texAccessQ[addressAccess]->anisoSamples);
            globalAnisoRatio->inc(texAccessQ[addressAccess]->anisoSamples);
            
	    if (maxAnisotropy[texAccessQ[addressAccess]->textUnit] == 1) {
                anisoRatioHisto[0]->inc();
                globalAnisoRatioHisto[0]->inc();
	    } else {
                anisoRatioHisto[texAccessQ[addressAccess]->anisoSamples]->inc();
                globalAnisoRatioHisto[texAccessQ[addressAccess]->anisoSamples]->inc();
            }

            bilinearsHisto[(filterLoops * texAccessQ[addressAccess]->anisoSamples) - 1]->inc();
            globalBilinearsHisto[(filterLoops * texAccessQ[addressAccess]->anisoSamples) - 1]->inc();
            
	    magnifiedPixels->inc(texAccessQ[addressAccess]->magnified);
	    globalMagnifiedPixels->inc(texAccessQ[addressAccess]->magnified);
            
	    for(u32bit f = 0; f < STAMP_FRAGMENTS; f++)
            {
                if (texAccessQ[addressAccess]->filter[f] == GPU_NEAREST) {
                    pointSampled->inc();
                    globalPointSampled->inc();
		}
            }
            if (textureMode[texAccessQ[addressAccess]->textUnit] != GPU_TEXTURE3D) {
                mipsSampled[filterLoops - 1]->inc();
                globalMipsSampled[filterLoops - 1]->inc();
	    }
        }
    }
...
void TextureUnit::fetch(u64bit cycle)
{
        ...
        (numFreeReadyReads > 0) && (numFreeWaitReads > 0))
    {
        ...
        for(frag = 0; frag < stampFragments; frag++)
        {
			...
			texAccessQ[fetchAccess]->debug[frag],
			texAccessQ[fetchAccess]->threadID[frag]
                        ))
                    {
                        GPU_TEX_TRACE(
                ...
                if (texAccessQ[readAccess]->trilinear[nextTrilinear]->fetched[frag][start + i])
                {
                    ...
                    if (!texAccessQ[readAccess]->trilinear[nextTrilinear]->read[frag][start + i])
                    {
                        ...
                        if (textCache->read(
                            ...
                            data, texAccessQ[readAccess]->debug[frag],
			    texAccessQ[readAccess]->threadID[frag]
                            ))
                        {
...
void TextureUnit::sendTextureSample(u64bit cycle)
{
    ...
    if (numResults > 0)
    {
        ...
        globalResults->inc();
        globalTextResLat->inc(cycle - textResultQ[nextResult]->getStartCycle());
...
void TextureUnit::processRegisterWrite(u64bit cycle, GPURegister reg, u32bit subReg, GPURegData data)
{
    ...
    switch(reg)
    {
	    ...
	    //debugfile = fopen("debugTexAddr.txt", "a+");
            //fprintf(debugfile, "Debug: %s => cycle %lld Writing register GPU_TEXTURE_ADDRESS[%d][%d][%d] = %x\n",
            //        getName(), cycle, textUnit, mipmap, cubemap, textureAddress[textUnit][mipmap][cubemap]);
	    //fflush(debugfile);
	    //fclose(debugfile);
        ...
        case GPU_TEXTURE_BORDER_COLOR:
            ...
            GPU_DEBUG_BOX(
                printf("%s => Writing register GPU_TEXTURE_BORDER_COLOR[%d] = {%lf, %lf, %lf, %lf}\n",
        ...
        case GPU_TEXTURE_NON_NORMALIZED:
            ...
            GPU_DEBUG_BOX(
                printf("%s => Writing register GPU_TEXTURE_NON_NORMALIZED[%d] = %s", getName(), subReg, data.booleanVal ? "T" : "F");
            )
