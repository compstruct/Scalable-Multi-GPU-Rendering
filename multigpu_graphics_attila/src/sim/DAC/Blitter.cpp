...
ColorBufferBlock::ColorBufferBlock()
    :   gpuId(0), address(0), block(0), state(ROPBlockState::UNCOMPRESSED), 
...
Blitter::Blitter(u32bit gpuId, u32bit numGPUs_, u32bit multiGPUExecMode_, BlitterTransactionInterface& transactionInterface,
                 u32bit stampH, u32bit stampW, u32bit genH, u32bit genW, u32bit scanH, u32bit scanW, u32bit overH, u32bit overW,
		 u32bit displayW, u32bit displayH, 
    ...
    gpuId(gpuId), numGPUs(numGPUs_), multiGPUExecMode(multiGPUExecMode_),
    readBlocks(transactionInterface.getSM(gpuId).getNumericStatistic("ReadBlocks", u32bit(0), "Blitter", "Blitter")),
    writtenBlocks(transactionInterface.getSM(gpuId).getNumericStatistic("WrittenBlocks", u32bit(0), "Blitter", "Blitter")),
    
    requestPendingQueueFull(transactionInterface.getSM(gpuId).getNumericStatistic("RequestPendingQueueFull", u32bit(0), "Blitter", "Blitter")),
    receiveDataPendingWakeUpQueueFull(transactionInterface.getSM(gpuId).getNumericStatistic("ReceiveDataPendingWakeUpQueueFull", u32bit(0), "Blitter", "Blitter")),
    decompressionPendingQueueFull(transactionInterface.getSM(gpuId).getNumericStatistic("DecompressionPendingQueueFull", u32bit(0), "Blitter", "Blitter")),
    swizzlingPendingQueueFull(transactionInterface.getSM(gpuId).getNumericStatistic("SwizzlingPendingQueueFull", u32bit(0), "Blitter", "Blitter")),
    writePendingQueueFull(transactionInterface.getSM(gpuId).getNumericStatistic("WritePendingQueueFull", u32bit(0), "Blitter", "Blitter")),
    
    clearColorBlocks(transactionInterface.getSM(gpuId).getNumericStatistic("ColorStateBlocksClear", u32bit(0), "Blitter", "Blitter")),
    uncompressedBlocks(transactionInterface.getSM(gpuId).getNumericStatistic("ColorStateBlocksUncompressed", u32bit(0), "Blitter", "Blitter")),
    compressedBestBlocks(transactionInterface.getSM(gpuId).getNumericStatistic("ColorStateBlocksCompressedBest", u32bit(0), "Blitter", "Blitter")),
    compressedNormalBlocks(transactionInterface.getSM(gpuId).getNumericStatistic("ColorStateBlocksCompressedNormal", u32bit(0), "Blitter", "Blitter")),
    compressedWorstBlocks(transactionInterface.getSM(gpuId).getNumericStatistic("ColorStateBlocksCompressedWorst", u32bit(0), "Blitter", "Blitter")),

    globalReadBlocks(transactionInterface.getSM().getNumericStatistic("ReadBlocks", u32bit(0), "Blitter", "Blitter")),
    globalWrittenBlocks(transactionInterface.getSM().getNumericStatistic("WrittenBlocks", u32bit(0), "Blitter", "Blitter")),
    
    globalRequestPendingQueueFull(transactionInterface.getSM().getNumericStatistic("RequestPendingQueueFull", u32bit(0), "Blitter", "Blitter")),
    globalReceiveDataPendingWakeUpQueueFull(transactionInterface.getSM().getNumericStatistic("ReceiveDataPendingWakeUpQueueFull", u32bit(0), "Blitter", "Blitter")),
    globalDecompressionPendingQueueFull(transactionInterface.getSM().getNumericStatistic("DecompressionPendingQueueFull", u32bit(0), "Blitter", "Blitter")),
    globalSwizzlingPendingQueueFull(transactionInterface.getSM().getNumericStatistic("SwizzlingPendingQueueFull", u32bit(0), "Blitter", "Blitter")),
    globalWritePendingQueueFull(transactionInterface.getSM().getNumericStatistic("WritePendingQueueFull", u32bit(0), "Blitter", "Blitter")),
    
    globalClearColorBlocks(transactionInterface.getSM().getNumericStatistic("ColorStateBlocksClear", u32bit(0), "Blitter", "Blitter")),
    globalUncompressedBlocks(transactionInterface.getSM().getNumericStatistic("ColorStateBlocksUncompressed", u32bit(0), "Blitter", "Blitter")),
    globalCompressedBestBlocks(transactionInterface.getSM().getNumericStatistic("ColorStateBlocksCompressedBest", u32bit(0), "Blitter", "Blitter")),
    globalCompressedNormalBlocks(transactionInterface.getSM().getNumericStatistic("ColorStateBlocksCompressedNormal", u32bit(0), "Blitter", "Blitter")),
    globalCompressedWorstBlocks(transactionInterface.getSM().getNumericStatistic("ColorStateBlocksCompressedWorst", u32bit(0), "Blitter", "Blitter")),
    ...
    swizzlingLatency(maxColorBufferBlocksPerTextureBlock)
{
    ...
    pixelMapper.setupDisplay(hRes, vRes, stampW, stampH, genW, genH, scanW, scanH, overW, overH, displayW, displayH, 1, 4);
...
void Blitter::reset()
{
    ...
    pixelMapper.setupDisplay(hRes, vRes, stampW, stampH, genW, genH, scanW, scanH, overW, overH, displayW, displayH, samples, bytesPixel);
    colorBufferSize = pixelMapper.computeFrameBufferSize();                      
...
void Blitter::clock(u64bit cycle)
{
    ...
    if (!decompressionPendingQueue.empty())
    {
        ...
        if (decompressionCycles == 0 && !swizzlingPendingQueue.full())
        {
            ...
            globalSwizzlingPendingQueueFull++;
	}
    ...
    if (!receiveDataPendingWakeUpQueue.empty())
    {
        ...
        if (textureBlock && !decompressionPendingQueue.full())
        {
            ...
            globalDecompressionPendingQueueFull++;
	}
    }
    ...
    if (!requestPendingQueue.empty() && !memWriteRequestThisCycle)
    {
        ...
        { 
             ...
             if (receiveDataPendingWakeUpQueue.full()) { /*  Update statistic.  */
                 ...
                 globalReceiveDataPendingWakeUpQueueFull++;
             }
        }
    ...
    if (colorBufferStateReady && !requestPendingQueue.full())
    {
        if (currentYBlock <= lastYBlock && currentXBlock <= lastXBlock)
        {
             ...
             {   
                  ...
                  */
                 if (processNewTextureBlock(cycle, currentXBlock, currentYBlock, textureBlock)) {
...
void Blitter::decompressBlock(u8bit* outBuffer, u8bit* inBuffer, u32bit colorBufferBlockSize, ROPBlockState state, QuadFloat clearColor)
{
    ...
    switch (state.state)
    {
        ...
        case ROPBlockState::COMPRESSED:
            ...
            case 0: compressedBestBlocks++;   globalCompressedBestBlocks++;   break;
            case 1: compressedNormalBlocks++; globalCompressedNormalBlocks++; break;
            case 2: compressedWorstBlocks++;  globalCompressedWorstBlocks++;  break;
            default:
                GPU_DEBUG(
    ...
    else
    {
        ...
        if (textureBlock->written == textureBlockSize)
        {
            ...
            GPU_DEBUG(
		       ...
		       transactionInterface.getBoxName().c_str(), textureBlock->xBlock, textureBlock->yBlock,
		       blocksWritten, totalBlocksToWrite);
            )
...
bool Blitter::processNewTextureBlock(u64bit cycle, u32bit xBlock, u32bit yBlock, TextureBlock* textureBlock)
{
...
//printf("%s::Blitter => Block texels: (%d,%d)->(%d,%d). Start address: %d\n", transactionInterface.getBoxName().c_str(), startBlockTexelX, startBlockTexelY, endBlockTexelX, endBlockTexelY, startBlockAddress);
    ...
    map<u32bit, u32bit> colorBufferBlocksGPUId;
    ...
    {        
		...
		//        transactionInterface.getBoxName().c_str(), i, j, texelBlockOffset);
                ...
                u32bit pixelGPU = GPU_MOD(pixelMapper.mapToDisplayTile(pixelX, pixelY, GPUPD, numGPUs), numGPUs);
		...
		//if (colorBufferAddress >= 0x70b40 && colorBufferAddress <= 0x70b50) {
	        //    FILE* debugfile = fopen("debugBlit.txt", "a+");
                //    fprintf(debugfile, "Blitter[%d] i %d j %d blitXOffset %d blitYOffset %d blitIniX %d blitIniY %d pixelX %d pixelY %d colorBufferAddr %x colorBufferBlock %d offset %d\n",
	        //    	gpuId, i, j, blitXOffset, blitYOffset, blitIniX, blitIniY,
		//    	pixelX, pixelY, colorBufferAddress, colorBufferBlock, colorBufferBlockOffset);
	        //    fflush(debugfile);
	        //    fclose(debugfile);
		//}
		...
		colorBufferBlocksGPUId[colorBufferBlock] = pixelGPU;
	...
	cbBlock->gpuId = colorBufferBlocksGPUId[cbBlock->block];
       
	//FILE* debugfile = fopen("debugBlit.txt", "a+");
        //fprintf(debugfile, "Blitter[%d] destAddr %x source addr %x bytesTexel %d texelsPerBlock %d bytesPerBlock %d gpuId %d\n",
	//	gpuId, textureBlock->address, cbBlock->address + backBufferAddress, bytesTexel, texelsPerBlock, bytesPerBlock, cbBlock->gpuId);
	//fflush(debugfile);
	//fclose(debugfile);
        ...
        switch(cbBlock->state.state)
        {
            ...
            case ROPBlockState::COMPRESSED: 
                blockSize = ColorCompressorEmulator::getInstance().getLevelBlockSize(cbBlock->state.comprLevel);
                break;
            default:
    ...
    while ( iter != textureBlock->colorBufferBlocks.end() && blockFinished )
    {
        ...
        switch(block.state)
        {
            ...
            case ROPBlockState::UNCOMPRESSED: 
                ...
                blockFinished = requestColorBlock(cycle,(*iter), colorBufferBlockSize, generatedTickets, textureBlock->address);
            ...
            case ROPBlockState::COMPRESSED: 
                ...
                blockFinished = requestColorBlock(cycle, (*iter), 
                                                  ...
                                                  generatedTickets, textureBlock->address);
...
bool Blitter::requestColorBlock(u64bit cycle, ColorBufferBlock *bInfo, u32bit blockRequestSize, set<u32bit>& generatedTickets, u32bit textureBlockAddr)
{
    ...
    if (transactionInterface.sendReadTransaction(cycle, size, requestAddress,
			                         bInfo->compressedContents + blockOffset,
						 nextTicket, bInfo->gpuId))
    {
        GPU_DEBUG(
              printf("GPU[%d] %s::Blitter %lld => Requesting %d bytes at %x for color buffer block %d (ticket %d).\n",
                     gpuId, transactionInterface.getBoxName().c_str(), cycle, size, requestAddress,
                     bInfo->block, nextTicket);
        )

	//FILE *debugfile = fopen("debugBlit.txt", "a+");
        //fprintf(debugfile, "%s::Blitter %lld => Requesting %d bytes at %x for color buffer block %d (ticket %d) blockAddr %x offset %d backBufferAddr %x textureBlockAddr %x.\n",
        //        transactionInterface.getBoxName().c_str(), cycle, size, requestAddress,
        //        bInfo->block, nextTicket, bInfo->address, blockOffset, backBufferAddress, textureBlockAddr);
	//fflush(debugfile);
	//fclose(debugfile);
...
bool Blitter::requestUncompressedBlock(u64bit cycle, ColorBufferBlock *bInfo, set<u32bit>& generatedTickets)
{
    ...
    if (transactionInterface.sendReadTransaction(cycle, size, bInfo->address + blockOffset,
			                         bInfo->compressedContents + blockOffset,
						 nextTicket, bInfo->gpuId))
    {
        GPU_DEBUG(
...
void Blitter::decompressColorBufferBlocks(TextureBlock* textureBlock)
{
	...
	//if ((*iter)->address + backBufferAddress == 0x7f00) {
	//    FILE* debugfile = fopen("debugBlit.txt", "a+");
        //    fprintf(debugfile, "Blitter[%d]::decompressColorBufferBlocks => textBlockAddr %x block %d address %x size %d | state = %d | level = %d\n",
        //        gpuId, textureBlock->address, (*iter)->block , (*iter)->address, (*iter)->requested,
	//	(*iter)->state.getState(), (*iter)->state.getComprLevel());
        //    for(u32bit b = 0; b < (*iter)->requested; b++)
        //        fprintf(debugfile, "data[%d] = %02x ", b, (*iter)->compressedContents[b]);
        //    fprintf(debugfile, "\n");
	//    fflush(debugfile);
	//    fclose(debugfile);
	//}
...
void Blitter::swizzleTextureBlock(TextureBlock* textureBlock)
{
    ...
    //FILE* debugfile = NULL;
    //if (textureBlock->address == 0x7652fc0) {
    ...
    {
	//if (debugfile) {
        //    fprintf(debugfile, "Blitter[%d] blockAddr %x block %d\n", gpuId, (*iter)->address, (*iter)->block);
	//    u32bit numWords = colorBufferBlockSize / 4;
	//    for (u32bit word = 0; word < numWords; word++) {
	//        fprintf(debugfile, "content[%d] = %08x ", word, *((u32bit*)((*iter)->contents) + word));
        //    }
	//    fprintf(debugfile, "\n");
	//    fflush(debugfile);
	//    fclose(debugfile);
...
void Blitter::dumpBlitSource(u32bit frameCounter, u32bit blitCounter)
{
    ...
    sprintf(filename, "gpu%d-blitop-f%04d-%04d.ppm", gpuId, frameCounter, blitCounter);
