...
/*  Clipper constructor.  */
Clipper::Clipper(u32bit gpuId, char* gpuPrefix, u32bit numGPUs_, u32bit multiGPUExecMode_, u32bit execRound_,
    u32bit trCycle, u32bit clipUnits, u32bit startLat, u32bit execLat, u32bit bufferSize,
    u32bit rastLat, u32bit outputLat, u32bit peLat, char *name, Box* parent) :
    ...
    rasterizerStartLat(rastLat), rasterizerOutputLat(outputLat), peStartLat(peLat),
    Box(gpuId, name, parent)
{
    ...
    clipperCommand = newInputSignal("ClipperCommand", 1, 1, gpuPrefix);

    /*  Create state signal to the CommandProcessor.  */
    clipperCommState = newOutputSignal("ClipperCommandState", 1, 1, gpuPrefix);
    ...
    clipperInput = newInputSignal("ClipperInput", trianglesCycle, 1, gpuPrefix);

    /*  Create request signal to Primitive Assembly.  */
    clipperRequest = newOutputSignal("ClipperRequest", 1, 1, gpuPrefix);
    ...
    if (numGPUs > 1 && multiGPUExecMode == GPUPD && execRound == 2) {
        ...
        rasterizerNewTriangle = newOutputSignal("RasterizerNewTriangle", trianglesCycle, rasterizerOutputLat, gpuPrefix);

        /*  Create request signal from the Rasterizer unit.  */
        rasterizerRequest = newInputSignal("ClipperRasterizerRequest", 1, 1, gpuPrefix);
    }

    /*  Create clipping start signal to the Clipper.  */
    clipStart = newOutputSignal("ClipperExecution", clipperUnits, execLatency, gpuPrefix);

    /*  Create clipping end signal from the Clipper.  */
    clipEnd = newInputSignal("ClipperExecution", clipperUnits, execLatency, gpuPrefix);

    if (numGPUs > 1 && multiGPUExecMode == GPUPD && execRound == 1) {
        primitiveExchangerInput = newOutputSignal("PrimitiveExchangerInput", trianglesCycle, 1, gpuPrefix);
        primitiveExchangerRequest = newInputSignal("PrimitiveExchangerRequest", 1, 1, gpuPrefix);
    } else {
   ...
   /*  Check buffer allocation.  */
    ...
    /*  Create statistics.  */
    inputs = &getSM(gpuId).getNumericStatistic("InputTriangles", u32bit(0), "Clipper", "CLP");
    outputs = &getSM(gpuId).getNumericStatistic("OutputTriangles", u32bit(0), "Clipper", "CLP");
    clipped = &getSM(gpuId).getNumericStatistic("ClippedTriangles", u32bit(0), "Clipper", "CLP");

    globalInputs = &getSM().getNumericStatistic("InputTriangles", u32bit(0), "Clipper", "CLP");
    globalOutputs = &getSM().getNumericStatistic("OutputTriangles", u32bit(0), "Clipper", "CLP");
    globalClipped = &getSM().getNumericStatistic("ClippedTriangles", u32bit(0), "Clipper", "CLP");
...
void Clipper::clock(u64bit cycle)
{
    ...
    if (numGPUs > 1 && multiGPUExecMode == GPUPD) {
	if (execRound == 1) {
            if (primitiveExchangerRequest->read(cycle, (DynamicObject *&) paRequest)) {
	        requestedTriangles += paRequest->getRequest();
		delete paRequest;
	    }    	
	}
    
    } else if (rasterizerRequest->read(cycle, (DynamicObject *&) paRequest)) {
    ...
    switch(state)
    {
        ...
        case CLP_DRAW:
		    ...
		    //    GPUDriver::getGPUDriver()->getGPUSchedInfo()->clearNumProcessedTris(gpuId);
		    //}
                }
                else
                {
		...
		peCycles--;
            ...
            if ((requestedTriangles > 0) &&
		((rasterizerNewTriangle && (rasterizerCycles == 0)) || (primitiveExchangerInput && (peCycles == 0))) &&
                ...
                clipBuffer[GPU_PMOD(nextClipTriangle + clippedTriangles - 1, clipBufferSize)]->isLast())))
            {
		        ...
		        GPU_ASSERT(
			    if (execRound != 1)
	                        panic("Clipper", "clock", "Only execution round 1 of GPUPD could reach here.");
			)
			primitiveExchangerInput->write(cycle, clipBuffer[nextClipTriangle]);
		    } else {
...
void Clipper::processCommand(ClipperCommand *clipComm)
{
    ...
    switch(clipComm->getCommand())
    {
	    ...
	    batchCounter = clipComm->getBatchCounter();
	    frameCounter = clipComm->getFrameCounter();
