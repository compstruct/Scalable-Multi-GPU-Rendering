...
StreamerFetch::StreamerFetch(u32bit gpuId, char* gpuPrefix, u32bit numGPUs_, u32bit multiGPUExecMode_, u32bit execRound_,
	u32bit idxCycle, u32bit idxBufferSize, u32bit oFIFOSize,
    ...
    streamerLoaderUnits(streamLoadUnits), slIndicesCycle(slIdxCycle), Box(gpuId, name, parent)

{
    ...
    streamerFetchMemReq = newOutputSignal("StreamerFetchMemoryRequest", 1, 1, gpuPrefix);

    /*  Data signal from the Memory Controller.  */
    streamerFetchMemData = newInputSignal("StreamerFetchMemoryData", 2, 1, gpuPrefix);

    /*  Signal from the Streamer main box.  */
    streamerFetchCommand = newInputSignal("StreamerFetchCommand", 1, 1, gpuPrefix);

    /*  State signal to the Streamer main box.  */
    streamerFetchState = newOutputSignal("StreamerFetchState", 1, 1, gpuPrefix);
    ...
    streamerOutputCache = newOutputSignal("StreamerOutputCache", indicesCycle, 1, gpuPrefix);
    ...
    streamerCommitDeAlloc = newInputSignal("StreamerCommitDeAlloc", 4 * verticesCycle * streamerLoaderUnits, 1, gpuPrefix);
    ...
    /*  Create statistics.  */
    bytesRead = &getSM(gpuId).getNumericStatistic("ReadBytes", u32bit(0), "StreamerFetch", "StF");
    transactions = &getSM(gpuId).getNumericStatistic("MemoryTransactions", u32bit(0), "StreamerFetch", "StF");
    sentIdx = &getSM(gpuId).getNumericStatistic("IndexesSent", u32bit(0), "StreamerFetch", "StF");
    
    globalBytesRead = &getSM().getNumericStatistic("ReadBytes", u32bit(0), "StreamerFetch", "StF");
    globalTransactions = &getSM().getNumericStatistic("MemoryTransactions", u32bit(0), "StreamerFetch", "StF");
    globalSentIdx = &getSM().getNumericStatistic("IndexesSent", u32bit(0), "StreamerFetch", "StF");
}
    ...
    */
    ...
    switch(state)
    {
		...
		//if (numGPUs > 1 && multiGPUExecMode == GPUPD && execRound == 2) {
                //    indexAddress = indexStreamAddress +
		//	           ((streamStart + streamCount * readInstance) * streamDataSize[indexStreamData]) +
		//	           requestedIndexData;
		//} else {
		...
		//}
			...
			if (fetchedIndexes == 0 && gpuId > 0 && multiGPUExecMode == GPUPD && execRound == 1 && primitiveMode == TRIANGLE_FAN) {
			    nextIndex = streamStartOfFan;
			} else {
                            nextIndex++;
			}
                    }
     ...
     GPURegData gpuData)
{
    ...
    switch(gpuReg)
    {
        ...
        case GPU_PRIMITIVE:
            /*  Streaming primitive mode.  */

            primitiveMode = gpuData.uintVal;

            GPU_DEBUG_BOX(
                printf("StreamerFetch => GPU_PRIMITIVE = %d.\n", gpuData.uintVal);
            )
        ...
        case GPU_STREAM_START_OF_FAN:
            /*  Streaming start position of the edge of fan primitive.  */

            streamStartOfFan = gpuData.uintVal;

            GPU_DEBUG_BOX(
                printf("StreamerFetch => GPU_STREAM_START_OF_FAN = %d.\n", gpuData.uintVal);
            )
	    ...
	    GPU_ASSERT(
	        if (numGPUs > 1 && multiGPUExecMode == GPUPD && execRound == 2 && indexedMode == false)
		    panic("StremerFetch", "processGPURegisterWrite", "indexed mode has to be true in the second round of GPUpd.");
	    )
