...
Streamer::Streamer(u32bit gpuId, char* gpuPrefix, u32bit numGPUs_, u32bit multiGPUExecMode_, u32bit execRound_,
    u32bit idxCycle, u32bit idxBufferSz, u32bit outFIFOSz, u32bit outMemSz,
    ...
    Box(gpuId, name, parent)
{
    ...
    streamCtrlSignal = newInputSignal("StreamerControl", 1, 1, gpuPrefix);

    /*  Create state signal to the Command Processor.  */
    streamStateSignal = newOutputSignal("StreamerState", 1, 1, gpuPrefix);
    ...
    streamerFetchCom = newOutputSignal("StreamerFetchCommand", 1, 1, gpuPrefix);

    /*  Create state signal from the Streamer Fetch.  */
    streamerFetchState = newInputSignal("StreamerFetchState", 1, 1, gpuPrefix);

    /*  Create command signal to the Streamer Output Cache.  */
    streamerOCCom = newOutputSignal("StreamerOutputCacheCommand", 1, 1, gpuPrefix);

    /*  Create command signal to the Streamer Commit.  */
    streamerCommitCom = newOutputSignal("StreamerCommitCommand", 1, 1, gpuPrefix);

    /*  Create state signal from the Streamer Commit.  */
    streamerCommitState = newInputSignal("StreamerCommitState", 1, 1, gpuPrefix);

    /*  Create streamer sub boxes.  */

    /*  Create Streamer Fetch box.  */
    sprintf(boxName, "GPU%dStreamerFetch", gpuId);
    ...
    sprintf(boxName, "GPU%dStreamerOutputCache", gpuId);
    ...
    {
        sLoaderName[i] = new char[24];

        sprintf(sLoaderName[i], "GPU%dStreamerLoader%d", gpuId, i);

        printf("Creating StreamerLoader Unit %d in GPU[%d]\n", i, gpuId); 
    ...
    sprintf(boxName, "GPU%dStreamerCommit", gpuId);
    ...
    switch(state)
    {
        ...
        case ST_STREAMING:
            ...
            /*  Check if the Streamer Fetch and the Streamer Commit have
                finished.  */
	    allMemoryTransDone = true;
            for (u32bit i = 0; i < streamerLoaderUnits; i++) {
	        allMemoryTransDone = allMemoryTransDone && streamerLoader[i]->allMemoryTransDone();
	    }

            if ((streamFetch == ST_FINISHED) && (streamCommit == ST_FINISHED) && allMemoryTransDone)
            {
        ...
        case STCOM_START:
            {
                ...
                //    if (count == 0) {
		//        char msg[128];
		//	sprintf(msg, "GPU%d is trying to draw an empty vertex batch.", gpuId);
                //        panic("Streamer", "processStreamerCommand", msg);
		//    }
                //)
		    ...
		    auxStCom->setBatchCounter(streamCom->getBatchCounter());
		    auxStCom->setFrameCounter(streamCom->getFrameCounter());
     ...
     GPURegData gpuData, u64bit cycle)
{
	...
	case GPU_PRIMITIVE:
	    ...
	    GPU_DEBUG(
                printf("Streamer => GPU_PRIMITIVE = %d.\n", gpuData.uintVal);
            )
        ...
        case GPU_STREAM_START_OF_FAN:
            ...
            startOfFan = gpuData.uintVal;

            GPU_DEBUG(
                printf("Streamer => GPU_STREAM_START_OF_FAN = %d.\n", gpuData.uintVal);
            )
