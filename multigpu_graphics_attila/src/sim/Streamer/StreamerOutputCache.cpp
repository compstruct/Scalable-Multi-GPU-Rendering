...
StreamerOutputCache::StreamerOutputCache(u32bit gpuId, char* gpuPrefix,
    u32bit idxCycle, u32bit omSize, u32bit vertCycle, u32bit sLoaderUnits, u32bit slIdxCycle, char *name, Box *parent) :

    indicesCycle(idxCycle), outputMemorySize(omSize), verticesCycle(vertCycle), streamerLoaderUnits(sLoaderUnits), 
    slIndicesCycle(slIdxCycle), Box(gpuId, name, parent)

{
    ...
    streamerCommand = newInputSignal("StreamerOutputCacheCommand", 1, 1, gpuPrefix);

    /*  Create new index signal from the Streamer Fetch box.  */
    streamerFetchNewIndex = newInputSignal("StreamerOutputCache", indicesCycle, 1, gpuPrefix);

    /*  Create new index signal to the Streamer Commit box.  */
    streamerCommitNewIndex = newOutputSignal("StreamerCommitNewIndex", indicesCycle, 1, gpuPrefix);
    ...
    {
        sLoaderPrefix[i] = new char[12];
        //sprintf(sLoaderPrefix[i], "SL%d", gpuId * sLoaderUnits + i);
        sprintf(sLoaderPrefix[i], "GPU%dSL%d", gpuId, i);
    ...
    streamerCommitDeAlloc = newInputSignal("StreamerCommitDeAllocOC", 2 * verticesCycle * streamerLoaderUnits, 1, gpuPrefix);

    /*  Create update signal to the Streamer Output Cache.  */
    streamerOCUpdateWrite = newOutputSignal("StreamerOCUpdate", indicesCycle, 1, gpuPrefix);

    /*  Create update signal from the Streamer Output Cache.  */
    streamerOCUpdateRead = newInputSignal("StreamerOCUpdate", indicesCycle, 1, gpuPrefix);
    ...
    /*  Create statistics.  */
    hits = &getSM(gpuId).getNumericStatistic("Hits", u32bit(0), "StreamerOutputCache", "StOC");
    misses = &getSM(gpuId).getNumericStatistic("Misses", u32bit(0), "StreamerOutputCache", "StOC");
    indices = &getSM(gpuId).getNumericStatistic("Indices", u32bit(0), "StreamerOutputCache", "StOC");

    globalHits = &getSM().getNumericStatistic("Hits", u32bit(0), "StreamerOutputCache", "StOC");
    globalMisses = &getSM().getNumericStatistic("Misses", u32bit(0), "StreamerOutputCache", "StOC");
    globalIndices = &getSM().getNumericStatistic("Indices", u32bit(0), "StreamerOutputCache", "StOC");
