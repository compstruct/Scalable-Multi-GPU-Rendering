...
StreamerCommit::StreamerCommit(u32bit gpuId, char *gpuPrefix, u32bit streamLoadUnits,
    u32bit idxCycle, u32bit oFIFOSize, u32bit omSize, u32bit nShaders,
...

{
   ...
   /*  Check shaders and shader signals prefixes.  */
    ...
    streamerCommitCommand = newInputSignal("StreamerCommitCommand", 1, 1, gpuPrefix);

    /*  Create state signal to the Streamer main box.  */
    streamerCommitState = newOutputSignal("StreamerCommitState", 1, 1, gpuPrefix);
    ...
    assemblyOutputSignal = newOutputSignal("PrimitiveAssemblyInput", verticesCycle,
        outputBusLat + MAX_VERTEX_ATTRIBUTES / attributesCycle , gpuPrefix);

    /*  Create new index signal from the Streamer Output Cache.  */
    streamerCommitNewIndex = newInputSignal("StreamerCommitNewIndex", indicesCycle, 1, gpuPrefix);

    /*  Create deallocation signal to the Streamer Fetch.  */
    streamerCommitDeAlloc = newOutputSignal("StreamerCommitDeAlloc", 4 * verticesCycle * streamLoadUnits, 1, gpuPrefix);

    /*  Create deallocation signal to the Streamer Output Cache.  */
    streamerCommitDeAllocOC = newOutputSignal("StreamerCommitDeAllocOC", 2 * verticesCycle * streamLoadUnits, 1, gpuPrefix);

    /*  Create the request signal from the Primitive Assembly unit.  */
    assemblyRequestSignal = newInputSignal("PrimitiveAssemblyRequest", 1, 1, gpuPrefix);
    ...
    {
        outputMemory[i] = new QuadFloat[MAX_VERTEX_ATTRIBUTES*2];
    ...
    /*  Create statistics.  */
    indices = &getSM(gpuId).getNumericStatistic("Indices", u32bit(0), "StreamerCommit", "StC");
    shOutputs = &getSM(gpuId).getNumericStatistic("ShaderOutputs", u32bit(0), "StreamerCommit", "StC");
    paOutputs = &getSM(gpuId).getNumericStatistic("Outputs", u32bit(0), "StreamerCommit", "StC");
    paRequests = &getSM(gpuId).getNumericStatistic("Requests", u32bit(0), "StreamerCommit", "StC");
    outAttributes = &getSM(gpuId).getNumericStatistic("OutputAttributes", u32bit(0), "StreamerCommit", "StC");
    avgVtxShadeLat = &getSM(gpuId).getNumericStatistic("AvgShadeLatency", u32bit(0), "StreamerCommit", "StC");

    globalIndices = &getSM().getNumericStatistic("Indices", u32bit(0), "StreamerCommit", "StC");
    globalShOutputs = &getSM().getNumericStatistic("ShaderOutputs", u32bit(0), "StreamerCommit", "StC");
    globalPaOutputs = &getSM().getNumericStatistic("Outputs", u32bit(0), "StreamerCommit", "StC");
    globalPaRequests = &getSM().getNumericStatistic("Requests", u32bit(0), "StreamerCommit", "StC");
    globalOutAttributes = &getSM().getNumericStatistic("OutputAttributes", u32bit(0), "StreamerCommit", "StC");
    globalAvgVtxShadeLat = &getSM().getNumericStatistic("AvgShadeLatency", u32bit(0), "StreamerCommit", "StC");
...
void StreamerCommit::clock(u64bit cycle)
{
    ...
    while (assemblyRequestSignal->read(cycle, (DynamicObject *&) assemblyRequest))
    {
        ...
        globalPaRequests->inc(assemblyRequest->getRequest());
        ...
        case ST_STREAMING:
            ...
            for(i = 0; (i < verticesCycle) && (nextVertexBus < verticesCycle); i++)
            {
                ...
                if (outputFIFO[nextOutput].isShaded && (transCycles[nextVertexBus] == 0))
                {
                    ...
                    if (requestVertex > 0)
                    {
                        ...
                        oaux3D = new QuadFloat[MAX_VERTEX_ATTRIBUTES];

                        /*  Check memory allocation.  */
                        GPU_ASSERT(
                            if (oaux == NULL || oaux3D == NULL)
                        ...
                        for(j = 0; j < MAX_VERTEX_ATTRIBUTES; j++)
                        {
                            ...
                            oaux3D[j] = outputMemory[oMemLine][MAX_VERTEX_ATTRIBUTES + j];
                        }
                        ...
                        assemblyInput = new PrimitiveAssemblyInput(outputFIFO[nextOutput].index, oaux, oaux3D);
                        ...
                        GPU_DEBUG_BOX(
                            ...
                            printf("v[2] = {%f, %f, %f, %f}\n",
                                oaux[2][0], oaux[2][1], oaux[2][2], oaux[2][3]);
                            printf("v3D[0] = {%f, %f, %f, %f}\n",
                                oaux3D[0][0], oaux3D[0][1], oaux3D[0][2], oaux3D[0][3]);
                            printf("v3D[1] = {%f, %f, %f, %f}\n",
                                oaux3D[1][0], oaux3D[1][1], oaux3D[1][2], oaux3D[1][3]);
                            printf("v3D[2] = {%f, %f, %f, %f}\n",
                                oaux3D[2][0], oaux3D[2][1], oaux3D[2][2], oaux3D[2][3]);
                        )
            ...
            for(i = 0; i < numShaders; i++)
            {
                ...
                if (shOutputSignal[i]->read(cycle, (DynamicObject *&) output))
                {
                    ...
                    globalAvgVtxShadeLat->incavg(outputFIFO[output->getEntry()].shadingLatency);
                    ...
                    if (!output->getKill())
                    {
                        ...
                        for(j = 0; j < MAX_VERTEX_ATTRIBUTES; j++)
                        {
                            ...
                            outputMemory[oMemLine][MAX_VERTEX_ATTRIBUTES + j] = output->get3DAttributes()[j];
                        }
...
void StreamerCommit::processStreamerCommand(StreamerCommand *streamCom)
{
    ...
    switch(streamCom->getCommand())
    {
        ...
        case STCOM_START:
            ...
            globalOutAttributes->inc(activeOutputs);
