...
address(addr), size(dataSize), data(dataBuffer), md(_md), locked(isLocked), numRefs(0)
...
/*  Set object attributes.  */
address(addr), size(dataSize), data(dataBuffer), md(_md), locked(true), numRefs(0)
...
numPackets(1), locked(false), numRefs(0)
{
    /*  Set object color for tracing.  */
...
/*  Set object attributes.  */
agpTrans(AGP_REG_READ),gpuReg(gpuR), subReg(subR), md(0), numPackets(1), locked(false), numRefs(0)
{
    /*  Set object color for tracing.  */
...
/*  Set object attributes.  */
agpTrans(AGP_COMMAND), gpuCommand(gpuComm), md(0), numPackets(1), locked(false), numRefs(0)
{
    /*  Set object color for tracing.  */
...
AGPTransaction::AGPTransaction() :
    agpTrans(AGP_INIT_END), md(0), numPackets(1), locked(false), numRefs(0)
{
    setTag("AGPTr");
}
...
//  Set object attributes.
agpTrans(AGP_EVENT), gpuEvent(gpuEvent), eventMsg(msg), numRefs(0)
...
AGPTransaction::AGPTransaction(AGPTransaction *sourceAGPTrans)
{
    ...
    numRefs = sourceAGPTrans->getRefs();
...
void AGPTransaction::dump(std::ostream& os) const
{
    ...
    if ( agpTrans == AGP_READ || agpTrans == AGP_WRITE || agpTrans == AGP_PRELOAD )
    {
        ...
        switch ( gpuReg ) {
            ...
            CASE_PRINT( GPU_STREAM_START_OF_FAN )
