...
MemoryController::MemoryController(
    ...
    Box* parent )  // parent box
        :
    MultiClockBox(gpuId, name, parent),
    ...
    _lastCycle(0), prevMemPreloads(0), prevMemReads(0), prevMemWrites(0)
{
    ...
    MemoryTransaction::setBusWidth(PRIMITIVEMAPPER,params.peMapperBus);
    MemoryTransaction::setBusWidth(PRIMITIVESYNCER,params.peSyncerBus);
    ...
    GPU_ASSERT(
        ...
        if (params.requestQueueSize < ((numTexUnits + numStampUnits + numStrLoaderUnits + 3 + 2)*2))
            panic("MemoryController", "ctor", "Minimum number of request queue entries is: "
               "(numTextUnits + numStampsUnits + numStrLoaderUnits + 3 + 2) * 2");
        ...
        if (freeWriteBuffers <= (2 + 4*numStampUnits + 4))
            panic("MemoryController", "ctor",
                  "Minimum number of write buffer lines required is larger than config");
        if ( numStampUnits == 0 )
    ...
    for ( u32bit i = 0; i < systemMemoryBuses; i++ )
    {
        ...
        systemMemoryRequestSignal.push_back(
            newOutputSignal(signalName.c_str(), 1, systemMemoryReadLatency, gpuPrefix));
        systemMemoryDataSignal.push_back(
            newInputSignal(signalName.c_str(), 1, systemMemoryWriteLatency, gpuPrefix));
    }
    ...
    mcCommSignal = newInputSignal("MemoryControllerCommand", 1, 1, gpuPrefix);
	...
	ss << "GPU" << gpuId;
        ...
        channelScheds[i] = createChannelScheduler(gpuId, i, ss.str().c_str(),
                                                   prefix.c_str(),
                                                  params,
                                                  this);
        ss.str("");
        ss << "GPU" << gpuId << "DDRModule" << i;

        ddrModules[i] = new DDRModule(gpuId, i, ss.str().c_str(), prefix.c_str(),
                                      params.burstLength,
    ...
    for ( u32bit i = 0; i < LASTGPUBUS; i++ )
    {
        ...
        switch ( i )
        {
            ...
            case STREAMERFETCH:
            case DACB:
	    case PRIMITIVEMAPPER:
	    case PRIMITIVESYNCER:
                count = 1;
                break;
            case STREAMERLOADER:
...
void MemoryController::createStatistics(u32bit gpuId)
{
    totalTransStat = &getSM(gpuId).getNumericStatistic("TotalTransactions", u32bit(0),
                                                    "MemoryController", "MC");
    totalReadTransStat = &getSM(gpuId).getNumericStatistic("TotalReadTransactions", u32bit(0),
                                                    "MemoryController", "MC");
    totalWriteTransStat = &getSM(gpuId).getNumericStatistic("TotalWriteTransactions", u32bit(0),
                                                    "MemoryController", "MC");
    totalReadBytesStat = &getSM(gpuId).getNumericStatistic("TotalReadBytes", u32bit(0),
                                                    "MemoryController", "MC");
    totalWriteBytesStat = &getSM(gpuId).getNumericStatistic("TotalWriteBytes", u32bit(0),
                                                    "MemoryController", "MC");

    globalTotalTransStat = &getSM().getNumericStatistic("TotalTransactions", u32bit(0),
                                                    "MemoryController", "MC");
    globalTotalReadTransStat = &getSM().getNumericStatistic("TotalReadTransactions", u32bit(0),
                                                    "MemoryController", "MC");
    globalTotalWriteTransStat = &getSM().getNumericStatistic("TotalWriteTransactions", u32bit(0),
                                                    "MemoryController", "MC");
    globalTotalReadBytesStat = &getSM().getNumericStatistic("TotalReadBytes", u32bit(0),
                                                    "MemoryController", "MC");
    globalTotalWriteBytesStat = &getSM().getNumericStatistic("TotalWriteBytes", u32bit(0),
                                                    "MemoryController", "MC");

    channelReq_ReadStallCyclesStat = &getSM(gpuId).getNumericStatistic("ChannelReq_ReadStallCycles", u32bit(0),
                                                    "MemoryController", "MC");

    channelReq_WriteStallCyclesStat = &getSM(gpuId).getNumericStatistic("ChannelReq_WriteStallCycles", u32bit(0),
                                                    "MemoryController", "MC");

    globalChannelReq_ReadStallCyclesStat = &getSM().getNumericStatistic("ChannelReq_ReadStallCycles", u32bit(0),
                                                    "MemoryController", "MC");

    globalChannelReq_WriteStallCyclesStat = &getSM().getNumericStatistic("ChannelReq_WriteStallCycles", u32bit(0),
                                                    "MemoryController", "MC");
    ...
    for ( u32bit i = 0; i < LASTGPUBUS; i++ )
    {
        ...
        for ( u32bit j = 0; j < count; j++ )
        {
            ...
            usg.initUnitStatGroup(gpuId, busName2);
            ...
            for ( u32bit k = 0; k < gpuMemoryChannels; ++k ) {
                ...
                stringstream auxStr;
                auxStr << k;
                usg.initUnitChannelStatsGroup(gpuId, busName2, "_Sched" + auxStr.str());
        ...
        if ( count > 1 )
        {
            ...
            UnitStatsGroup usg;
            usg.initUnitStatGroup(gpuId, busName + "_");
            unitStats[i].push_back(usg);
        }
    }

    // create preload transaction counter
    preloadStat = &getSM(gpuId).getNumericStatistic("PreloadTransactions", u32bit(0), "MemoryController", "MC");

    globalPreloadStat = &getSM().getNumericStatistic("PreloadTransactions", u32bit(0), "MemoryController", "MC");

    // Create system memory statistics
    sysBusReadBytesStat = &getSM(gpuId).getNumericStatistic("ReadBytesSystemBus", u32bit(0),
                                                       "MemoryController", "MC");
    sysBusWriteBytesStat = &getSM(gpuId).getNumericStatistic("WriteBytesSystemBus", u32bit(0),
                                                        "MemoryController", "MC");

    globalSysBusReadBytesStat = &getSM().getNumericStatistic("ReadBytesSystemBus", u32bit(0),
                                                       "MemoryController", "MC");
    globalSysBusWriteBytesStat = &getSM().getNumericStatistic("WriteBytesSystemBus", u32bit(0),
                                                        "MemoryController", "MC");


    stat_requestBufferAccumSizeStat = &getSM(gpuId).getNumericStatistic("Request_Buffer_ACCUM_Allocated_Entries", u32bit(0),
                                                                   "MemoryController", "MC");

    stat_avgRequestBufferAllocated = &getSM(gpuId).getNumericStatistic("Request_Buffer_AVG_Allocated_Entries", u32bit(0),
                                                                   "MemoryController", "MC");

    stat_serviceQueueAccumSizeStat = &getSM(gpuId).getNumericStatistic("Service_Queue_ACCUM_Items", u32bit(0),
                                                                   "MemoryController", "MC");

    stat_avgServiceQueueItems = &getSM(gpuId).getNumericStatistic("Service_Queue_AVG_Items", u32bit(0),
                                                                   "MemoryController", "MC");

    globalStat_requestBufferAccumSizeStat = &getSM().getNumericStatistic("Request_Buffer_ACCUM_Allocated_Entries", u32bit(0),
                                                                   "MemoryController", "MC");

    globalStat_avgRequestBufferAllocated = &getSM().getNumericStatistic("Request_Buffer_AVG_Allocated_Entries", u32bit(0),
                                                                   "MemoryController", "MC");

    globalStat_serviceQueueAccumSizeStat = &getSM().getNumericStatistic("Service_Queue_ACCUM_Items", u32bit(0),
                                                                   "MemoryController", "MC");

    globalStat_avgServiceQueueItems = &getSM().getNumericStatistic("Service_Queue_AVG_Items", u32bit(0),
                                                                   "MemoryController", "MC");

}


void MemoryController::UnitStatsGroup::initUnitStatGroup( u32bit gpuId, const std::string& prefix )
{
    // prefix           STAT      postfix
    // ZStencilTest0_Transactions_Sched0
    trans = &getSM(gpuId).getNumericStatistic((prefix + "Transactions").c_str(), u32bit(0), "MemoryController", "MC");
    readTrans = &getSM(gpuId).getNumericStatistic((prefix + "ReadTransactions").c_str(), u32bit(0), "MemoryController", "MC");
    writeTrans = &getSM(gpuId).getNumericStatistic((prefix + "WriteTransactions").c_str(), u32bit(0), "MemoryController", "MC");
    readBytes = &getSM(gpuId).getNumericStatistic((prefix + "ReadBytes").c_str(), u32bit(0), "MemoryController", "MC");
    writeBytes = &getSM(gpuId).getNumericStatistic((prefix + "WriteBytes").c_str(), u32bit(0), "MemoryController", "MC");
    acceptReadCycles = &getSM(gpuId).getNumericStatistic((prefix + "AcceptReadCycles").c_str(), u32bit(0), "MemoryController", "MC");
    acceptWriteCycles = &getSM(gpuId).getNumericStatistic((prefix + "AcceptWriteCycles").c_str(), u32bit(0), "MemoryController", "MC");
    readServiceAccumTime = &getSM(gpuId).getNumericStatistic((prefix + "serviceAccumTime").c_str(), u32bit(0), "MemoryController", "MC");
    completedReadTrans = &getSM(gpuId).getNumericStatistic((prefix + "completedReadTrans").c_str(), u32bit(0), "MemoryController", "MC");
    readServiceTimeAvg = &getSM(gpuId).getNumericStatistic((prefix + "serviceTimeAvg").c_str(), u32bit(0), "MemoryController", "MC");
    
    globalTrans = &getSM().getNumericStatistic((prefix + "Transactions").c_str(), u32bit(0), "MemoryController", "MC");
    globalReadTrans = &getSM().getNumericStatistic((prefix + "ReadTransactions").c_str(), u32bit(0), "MemoryController", "MC");
    globalWriteTrans = &getSM().getNumericStatistic((prefix + "WriteTransactions").c_str(), u32bit(0), "MemoryController", "MC");
    globalReadBytes = &getSM().getNumericStatistic((prefix + "ReadBytes").c_str(), u32bit(0), "MemoryController", "MC");
    globalWriteBytes = &getSM().getNumericStatistic((prefix + "WriteBytes").c_str(), u32bit(0), "MemoryController", "MC");
    globalAcceptReadCycles = &getSM().getNumericStatistic((prefix + "AcceptReadCycles").c_str(), u32bit(0), "MemoryController", "MC");
    globalAcceptWriteCycles = &getSM().getNumericStatistic((prefix + "AcceptWriteCycles").c_str(), u32bit(0), "MemoryController", "MC");
    globalReadServiceAccumTime = &getSM().getNumericStatistic((prefix + "serviceAccumTime").c_str(), u32bit(0), "MemoryController", "MC");
    globalCompletedReadTrans = &getSM().getNumericStatistic((prefix + "completedReadTrans").c_str(), u32bit(0), "MemoryController", "MC");
    globalReadServiceTimeAvg = &getSM().getNumericStatistic((prefix + "serviceTimeAvg").c_str(), u32bit(0), "MemoryController", "MC");
}

void MemoryController::UnitChannelStatsGroup::initUnitChannelStatsGroup( u32bit gpuId, const string& prefix, const string& postfix )
{
    readBytes  = &getSM(gpuId).getNumericStatistic((prefix + "ReadBytes" + postfix).c_str(), u32bit(0), "MemoryController", "MC");
    writeBytes = &getSM(gpuId).getNumericStatistic((prefix + "WriteBytes" + postfix).c_str(), u32bit(0), "MemoryController", "MC");
    readChannelTransAccumTime = &getSM(gpuId).getNumericStatistic((prefix + "ReadChannelTransAccumTime" + postfix).c_str(), u32bit(0), "MemoryController", "MC");
    completedReadChannelTrans = &getSM(gpuId).getNumericStatistic((prefix + "CompletedReadChannelTrans" + postfix).c_str(), u32bit(0), "MemoryController", "MC");
    
    globalReadBytes  = &getSM().getNumericStatistic((prefix + "ReadBytes" + postfix).c_str(), u32bit(0), "MemoryController", "MC");
    globalWriteBytes = &getSM().getNumericStatistic((prefix + "WriteBytes" + postfix).c_str(), u32bit(0), "MemoryController", "MC");
    globalReadChannelTransAccumTime = &getSM().getNumericStatistic((prefix + "ReadChannelTransAccumTime" + postfix).c_str(), u32bit(0), "MemoryController", "MC");
    globalCompletedReadChannelTrans = &getSM().getNumericStatistic((prefix + "CompletedReadChannelTrans" + postfix).c_str(), u32bit(0), "MemoryController", "MC");
}

void MemoryController::createUsageSignals(char *gpuPrefix, const MemoryControllerParameters& params)
{
    ...
    for ( u32bit i = 0; i < LASTGPUBUS; i++ )
    {
	    ...
	    case PRIMITIVESYNCER:
                ...
                busName = ss.str();
                memBusIn[i].push_back(newInputSignal(busName.c_str(), 1, 1, gpuPrefix));
                memBusOut[i].push_back(newOutputSignal(busName.c_str(), 1, 1, gpuPrefix));
...
void MemoryController::createIOSignals(char *gpuPrefix)
{
    ...
    dataSignals.back().push_back(newOutputSignal("CommProcMemoryRead", 2, 1, gpuPrefix));
    requestSignals.push_back(newInputSignal("CommProcMemoryWrite", 1, 1, gpuPrefix));
    ...
    dataSignals.back().push_back(newOutputSignal("StreamerFetchMemoryData", 2, 1, gpuPrefix));
    requestSignals.push_back(newInputSignal("StreamerFetchMemoryRequest", 1, 1, gpuPrefix));
    ...
    dataSignals.back().push_back(newOutputSignal("DACMemoryData", 2, 1, gpuPrefix));
    requestSignals.push_back(newInputSignal("DACMemoryRequest", 1, 1, gpuPrefix));
    ...
    dataSignals.push_back(vector<Signal*>()); // Primitive mapper
    dataSignals.back().push_back(newOutputSignal("PEMapperMemoryData", 2, 1, gpuPrefix));
    requestSignals.push_back(newInputSignal("PEMapperMemoryRequest", 1, 1, gpuPrefix));

    dataSignals.push_back(vector<Signal*>()); // Primitive syncer
    dataSignals.back().push_back(newOutputSignal("PESyncerMemoryData", 2, 1, gpuPrefix));
    requestSignals.push_back(newInputSignal("PESyncerMemoryRequest", 1, 1, gpuPrefix));
...
void MemoryController::updateCompletedReadStats(MemoryRequest& mr, u64bit cycle)
{
    ...
    usg.globalReadServiceAccumTime->inc(cycle - mr.getArrivalTime());
    usg.globalCompletedReadTrans->inc();
    usg.globalReadServiceTimeAvg->incavg(cycle - mr.getArrivalTime());
	...
	usgGlobal.globalReadServiceAccumTime->inc(cycle - mr.getArrivalTime());
        usgGlobal.globalCompletedReadTrans->inc();
        usgGlobal.globalReadServiceTimeAvg->incavg(cycle - mr.getArrivalTime());
    }
...
void MemoryController::processMemoryTransaction(u64bit cycle, MemoryTransaction* memTrans)
{
    ...
    switch ( command )
    {
	    ...
	    prevMemReads++;
            ...
            globalTotalReadTransStat->inc();
            unitStats[unit][unitID].readTrans->inc();
            unitStats[unit][unitID].globalReadTrans->inc();
            if ( unitStats[unit].size() > 1 ) {
                unitStats[unit].back().readTrans->inc();
                unitStats[unit].back().globalReadTrans->inc();
	    }
	    ...
	    prevMemWrites++;
            ...
            globalTotalWriteTransStat->inc();
            unitStats[unit][unitID].writeTrans->inc();
            unitStats[unit][unitID].globalWriteTrans->inc();
            if ( unitStats[unit].size() > 1 ) {
                unitStats[unit].back().writeTrans->inc();
                unitStats[unit].back().globalWriteTrans->inc();
	    }
...
u32bit MemoryController::addRequest(MemoryTransaction* memTrans, u64bit cycle)
{
        ...
        MCSplitter::CTVCIt it = ctv.begin(); it != ctv.end(); it++ )
    {
        ...
        ctrans->addCookie();
        // set STV info
        sprintf((char*)ctrans->getInfo(), "%s", ctrans->toString().c_str());
                    ...
                    case MT_WRITE_DATA:
                        {
                            ...
                            unitStats[currentUnit][unitID].globalWriteBytes->inc(tsize);
                            if ( unitStats[currentUnit].size() > 1 ) {
                                unitStats[currentUnit].back().writeBytes->inc(tsize);
                                unitStats[currentUnit].back().globalWriteBytes->inc(tsize);
			    }

                            // update written bytes from system bus statistic
                            sysBusWriteBytesStat->inc(tsize);
                            globalSysBusWriteBytesStat->inc(tsize);
            ...
            if ( !channelQueue[i][0].empty() && channelQueue[i][0].head().first->ready() )
            {
                   ...
                   ( state == SchedulerState::AcceptWrite && !ct->isRead() ) )
                {
                    ...
                    if ( ct->isRead() ) {
                        ...
                        globalChannelReq_ReadStallCyclesStat->inc();
		    } else {
                        channelReq_WriteStallCyclesStat->inc();
                        globalChannelReq_WriteStallCyclesStat->inc();
		    }
                }
            }
        }
    ...
    for ( u32bit i = 0; i < gpuMemoryChannels; i++ )
    {
        ...
        if ( channelReply[i]->read(cycle, (DynamicObject*&)ct) )
        {
            ...
            if ( ct->isRead() )
            {
                ...
                globalTotalReadBytesStat->inc();
                unitStats[unit][unitID].readBytes->inc(ct->bytes());
                unitStats[unit][unitID].globalReadBytes->inc(ct->bytes());
                if (unitStats[unit].size() > 1 ) {
                    unitStats[unit].back().readBytes->inc(ct->bytes());
                    unitStats[unit].back().globalReadBytes->inc(ct->bytes());
		}
                ...
                ucsg.globalReadBytes->inc(ct->bytes());
                ucsg.globalCompletedReadChannelTrans->inc();
                ...
                ucsg.globalReadChannelTransAccumTime->inc(cycle - req->getArrivalTime());
            }
            else // it is a write
            {
                totalWriteBytesStat->inc();
                globalTotalWriteBytesStat->inc();

                unitStats[unit][unitID].writeBytes->inc(ct->bytes());
                unitStats[unit][unitID].globalWriteBytes->inc(ct->bytes());

                if ( unitStats[unit].size() > 1 ) {
                    unitStats[unit].back().writeBytes->inc(ct->bytes());
                    unitStats[unit].back().globalWriteBytes->inc(ct->bytes());
		}

                // Update per unit/channel stats
                unitChannelStats[unit][unitID][i].writeBytes->inc(ct->bytes());
                unitChannelStats[unit][unitID][i].globalWriteBytes->inc(ct->bytes());
            }
...
void MemoryController::sendBusStateToClients(u64bit cycle)
{
    ...
    sendBusState(cycle, PRIMITIVEMAPPER, state);
    sendBusState(cycle, PRIMITIVESYNCER, state);
}
...
void MemoryController::sendBusState(u64bit cycle, GPUUnit unit, MemState state)
{
    ...
    for ( u32bit i = 0; i < signals.size(); i++ )
    {
        ...
        if ((unitState & MS_READ_ACCEPT) != 0) {
            ...
            unitStats[unit][i].globalAcceptReadCycles->inc();
        }

        if ((unitState & MS_WRITE_ACCEPT) != 0) {
            unitStats[unit][i].acceptWriteCycles->inc();
            unitStats[unit][i].globalAcceptWriteCycles->inc();
	}
    }
}
...
void MemoryController::clock(u64bit cycle)
{
    ...
    globalStat_avgRequestBufferAllocated->incavg(requestQueueSize - freeRequestQueue.items());
    globalStat_requestBufferAccumSizeStat->inc(requestQueueSize - freeRequestQueue.items());
    ...
    globalStat_avgServiceQueueItems->incavg(serviceQueue.items());
    globalStat_serviceQueueAccumSizeStat->inc(serviceQueue.items());
