...
ChannelScheduler* gpu3d::memorycontroller::createChannelScheduler(u32bit gpuId, u32bit schedulerId, const char* name, 
                                         const char* prefix, 
                                         ...
                                         Box* parent)
{
    ...
    if ( params.schedulerType == MCParams::Fifo )
    {
        ...
        sched = new FifoScheduler( gpuId, schedulerId, name, prefix, parent, commonConfig ); //, params.fifo_maxChannelTransactions);
    }
    else if ( params.schedulerType == MCParams::RWFifo )
    {
        cout << "Creating RWFifoScheduler: " << prefix << "::" << name << "\n";
        sched = new RWFifoScheduler( gpuId, schedulerId, name, prefix, parent, commonConfig );
    ...
    else if ( params.schedulerType == MCParams::BankQueueFifo )
    {
        ...
        sched = new BankQueueScheduler( gpuId, schedulerId, name, prefix, parent, commonConfig );
