...
DDRModule::DDRModule(u32bit gpuId, u32bit moduleId_, const char* name, const char* prefix, u32bit burstLength, 
...
Box(gpuId, name, parentBox), moduleId(moduleId_), nBanks(nBanks), bankRows(bankRows), bankCols(bankCols),
...
bypassConstraint(0),
// Statistics creation
transmissionCyclesStat(getSM(gpuId).getNumericStatistic("DataCycles", u32bit(0), "DDRModule", prefix)),
transmissionBytesStat(getSM(gpuId).getNumericStatistic("DataBytes", u32bit(0), "DDRModule", prefix)),
idleCyclesStat(getSM(gpuId).getNumericStatistic("IdleCycles", u32bit(0), "DDRModule", prefix)),
readTransmissionCyclesStat(getSM(gpuId).getNumericStatistic("ReadDataCycles", u32bit(0), "DDRModule", prefix)),
readTransmissionBytesStat(getSM(gpuId).getNumericStatistic("ReadDataBytes", u32bit(0), "DDRModule",  prefix)),
writeTransmissionCyclesStat(getSM(gpuId).getNumericStatistic("WriteDataCycles", u32bit(0), "DDRModule", prefix)),
writeTransmissionBytesStat(getSM(gpuId).getNumericStatistic("WriteDataBytes", u32bit(0), "DDRModule", prefix)),
casCyclesStat(getSM(gpuId).getNumericStatistic("WastedCycles_CASLatency", u32bit(0), "DDRModule", prefix)),
wlCyclesStat(getSM(gpuId).getNumericStatistic("WastedCycles_WriteLatency", u32bit(0), "DDRModule", prefix)),
allBanksPrechargedCyclesStat(getSM(gpuId).getNumericStatistic("AllBanksPrechargedCycles", u32bit(0), "DDRModule", prefix)),
actCommandsReceived(getSM(gpuId).getNumericStatistic("ACTCommandsReceived", u32bit(0), "DDRModule", prefix)),

globalTransmissionCyclesStat(getSM().getNumericStatistic("DataCycles", u32bit(0), "DDRModule", prefix)),
globalTransmissionBytesStat(getSM().getNumericStatistic("DataBytes", u32bit(0), "DDRModule", prefix)),
globalIdleCyclesStat(getSM().getNumericStatistic("IdleCycles", u32bit(0), "DDRModule", prefix)),
globalReadTransmissionCyclesStat(getSM().getNumericStatistic("ReadDataCycles", u32bit(0), "DDRModule", prefix)),
globalReadTransmissionBytesStat(getSM().getNumericStatistic("ReadDataBytes", u32bit(0), "DDRModule",  prefix)),
globalWriteTransmissionCyclesStat(getSM().getNumericStatistic("WriteDataCycles", u32bit(0), "DDRModule", prefix)),
globalWriteTransmissionBytesStat(getSM().getNumericStatistic("WriteDataBytes", u32bit(0), "DDRModule", prefix)),
globalCasCyclesStat(getSM().getNumericStatistic("WastedCycles_CASLatency", u32bit(0), "DDRModule", prefix)),
globalWlCyclesStat(getSM().getNumericStatistic("WastedCycles_WriteLatency", u32bit(0), "DDRModule", prefix)),
globalAllBanksPrechargedCyclesStat(getSM().getNumericStatistic("AllBanksPrechargedCycles", u32bit(0), "DDRModule", prefix)),
globalActCommandsReceived(getSM().getNumericStatistic("ACTCommandsReceived", u32bit(0), "DDRModule", prefix))
{
    ...
    for ( u32bit i = DDRCommand::PC_none + 1; i < DDRCommand::PC_count; ++i ) {
        ...
        constraintCyclesStats[i] = &getSM(gpuId).getNumericStatistic(constraintStatName.c_str(), u32bit(0), "DDRModule", prefix);
        globalConstraintCyclesStats[i] = &getSM().getNumericStatistic(constraintStatName.c_str(), u32bit(0), "DDRModule", prefix);
    }
	...
	banks[i].setGpuID(gpuId);
	banks[i].setModuleID(moduleId);
	banks[i].setBankID(i);
    }
...
void DDRModule::clock(u64bit cycle)
{
    ...
    if ( allBanksPrecharged ) {
        ...
        globalAllBanksPrechargedCyclesStat.inc();
    }
    ...
    if ( dataPinsSignal->read(cycle, (DynamicObject*&)dynObj) )
    {
        if ( dynObj->whattype() == DataPinItem::IS_READ )
        {
            ...
            globalTransmissionCyclesStat++; // inc data pins transmission stats
            readTransmissionCyclesStat++;
            globalReadTransmissionCyclesStat++;
            //readTransmissionBytesStat.inc(4*burstElementsPerCycle);
            readTransmissionBytesStat.inc(burstBytesPerCycle);
            globalReadTransmissionBytesStat.inc(burstBytesPerCycle);
        }
        else if ( dynObj->whattype() == DataPinItem::IS_WRITE )
        {
            ...
            if ( dynObj->whattype() == DataPinItem::IS_CAS_LATENCY ) {
                ...
                globalConstraintCyclesStats[dynObj->_whatis - DataPinItem::STV_COLOR_PROTOCOL_CONSTRAINT_BASE]->inc();
            }
            else {
                dump();
...
void DDRModule::processCommand(u64bit cycle, DDRCommand* cmd)
{
    ...
    if (cmd->getDebug()) {
        ...
        fprintf(debugfile, "GPU[%d]DDRModule[%d]: thread %d command %d\n",
        	gpuId, moduleId, cmd->getDebugThread(), cmd->which());
        fprintf(debugfile, "%s", cmd->toString().c_str());
        fprintf(debugfile, "\n");
        fflush(debugfile);
        fclose(debugfile);
    }
    ...
    switch ( cmd->which() )
    {
        case DDRCommand::Active:
            ...
            globalActCommandsReceived.inc(); // Keep a counter of ACT commands (to compute our power model)
...
void DDRModule::preprocessCommand(u64bit cycle, const DDRCommand* ddrCommand)
{
    ...
    if ( ddrCommand->getProtocolConstraint() != DDRCommand::PC_none ) {
        ...
        sprintf((char*)bypassConstraint->getInfo(), "%s", DDRCommand::protocolConstraintToString(pc).c_str());
    }
}
...
void DDRModule::processReadCommand(u64bit cycle, DDRCommand* readCommand)
{
    ...
    DDRBurst* burst = banks[bankId].read(readCommand->getColumn(), burstLength,
		                         readCommand->getDebug(), readCommand->getDebugThread());
