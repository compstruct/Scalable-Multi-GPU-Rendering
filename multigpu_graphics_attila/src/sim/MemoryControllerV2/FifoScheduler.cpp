...
FifoScheduler::FifoScheduler(u32bit gpuId, u32bit schedulerId, const char* name, const char* prefix,
	Box* parent, const CommonConfig& config) : // , u32bit maxTransactions) :
FifoSchedulerBase(gpuId, schedulerId, name, prefix, parent, config),
maxSizeQ(config.maxChannelTransactions),
useInnerSignals(config.createInnerSignals),
pendingBankAccesses(config.moduleBanks, 0),
closePageActivationsCount(getSM(gpuId).getNumericStatistic("ClosePageActivationCount", u32bit(0), "FifoSchedulerBase", prefix)),
globalClosePageActivationsCount(getSM().getNumericStatistic("ClosePageActivationCount", u32bit(0), "FifoSchedulerBase", prefix))
{
        ...
        for ( u32bit i = 0; i < Banks; ++i ) {
            ...
            if ( pendingBankAccesses[i] == 0 && moduleState().getActiveRow(i) != DDRModuleState::NoActiveRow ) 
            {
                ...
                if ( !sendDDRCommand(cycle, preCommand, 0) ) 
                    ...
                    globalClosePageActivationsCount.inc();
                    break;
                }
            }
        }
