...
DDRBurst* DDRBank::read(u32bit column, u32bit burstSize, bool debug, u32bit debugThread) const
{
    /*
    ...
    */
    ...
    if (debug) {
        ...
        fprintf(debugfile, "GPU[%d]Module[%d]DDRBank[%d]: thread %d read row %d column %d size %d\n",
        	gpuID, moduleID, bankID, debugThread, activeRow, column, burstSize*4);
        for (u32bit i = 0; i < burstSize; i++)
             fprintf(debugfile, "data[%d] = %08x ", i, data[activeRow][i+column]);
        fprintf(debugfile, "\n");
        fflush(debugfile);
        fclose(debugfile);
    }
...
void DDRBank::write(u32bit column, const DDRBurst* burst)
{
    ...
    //        sprintf(filename, "debugMem.txt");
    //        FILE *debugfile = fopen(filename, "a+");
    //        fprintf(debugfile, "GPU[%d]Module[%d]DDRBank[%d]: write row %d column %d size %d\n",
    //        	    gpuID, moduleID, bankID, activeRow, column, burstSize*4);
    //        for (u32bit i = 0; i < burstSize; i++)
    //             fprintf(debugfile, "data[%d] = %08x ", i, data[activeRow][i+column]);
    //        fprintf(debugfile, "\n");
    //        fflush(debugfile);
...
void DDRBank::writeData(u32bit row, u32bit startCol, u32bit bytes, std::istream& inStream)
{
    ...
    //if (row == 1843) {
    //    if (startCol >= 0 && startCol <= 100) {
    //        char filename[32];
    //        sprintf(filename, "debugMem.txt");
    //        FILE *debugfile = fopen(filename, "a+");
    //        fprintf(debugfile, "GPU[%d]Module[%d]DDRBank[%d]: writeData row %d column %d size %d\n",
    //        	    gpuID, moduleID, bankID, row, startCol, bytes);
    //        for (u32bit i = 0; i < bytes; i++)
    //             fprintf(debugfile, "data[%d] = %08x ", i, *((u8bit*)&data[row][startCol] + i));
    //        fprintf(debugfile, "\n");
    //        fflush(debugfile);
    //        fclose(debugfile);
    //    }
