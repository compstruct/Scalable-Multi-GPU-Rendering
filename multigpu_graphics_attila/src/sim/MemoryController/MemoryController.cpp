...
MemoryController::MemoryController(u32bit gpuId, char *gpuPrefix,
    u32bit memSize, u32bit clockMult, u32bit memFreq, u32bit memBusWidth, u32bit numBuses,
...

{
    ...
    commProcReadSignal = newOutputSignal("CommProcMemoryRead", 2, 1, gpuPrefix);
    ...
    commProcWriteSignal = newInputSignal("CommProcMemoryWrite", 1, 1, gpuPrefix);
    ...
    streamFetchDataSignal = newOutputSignal("StreamerFetchMemoryData", 2, 1, gpuPrefix);
    ...
    streamFetchReqSignal = newInputSignal("StreamerFetchMemoryRequest", 1, 1, gpuPrefix);
    ...
    dacDataSignal = newOutputSignal("DACMemoryData", 2, 1, gpuPrefix);
    ...
    dacReqSignal = newInputSignal("DACMemoryRequest", 1, 1, gpuPrefix);
    ...
    for(i = 0; i < gpuMemoryBuses; i++)
    {
        ...
        memoryModuleRequestSignal[i] = newOutputSignal(signalName, 1, gpuMemReadLatency, gpuPrefix);

        /*  Memory module data signal.  */
        memoryModuleDataSignal[i] = newInputSignal(signalName, 1, gpuMemReadLatency, gpuPrefix);
    }
    ...
    for(i = 0; i < MAPPED_MEMORY_BUSES; i++)
    {
        ...
        mappedMemoryRequestSignal[i] = newOutputSignal(signalName, 1, MAPPED_MEMORY_LATENCY, gpuPrefix);

        /*  System memory data signal.  */
        mappedMemoryDataSignal[i] = newInputSignal(signalName, 1, MAPPED_MEMORY_LATENCY, gpuPrefix);
    }
    ...
    for(i = 0; i < LASTGPUBUS; i++)
    {
        switch(i)
        {
            ...
            case DACB:
                ...
                memBusIn[i][0] = newInputSignal(busName, 1, 1, gpuPrefix);
                memBusOut[i][0] = newOutputSignal(busName, 1, 1, gpuPrefix);
            ...
            case TEXTUREUNIT:
                ...
                for(j = 0; j < numTextUnits; j++)
                {
                        ...
                        memBusIn[i][j] = newInputSignal(busName, 1, 1, gpuPrefix);
                        memBusOut[i][j] = newOutputSignal(busName, 1, 1, gpuPrefix);
                }
    ...
    */
    ...
    for(i = 0; i < LASTGPUBUS; i++)
    {
        ...
        transactions[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalTransactions[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");

        sprintf(busName,"%sReadTransactions", busNames[i]);
        readTrans[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalReadTrans[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");

        sprintf(busName,"%sWriteTransactions", busNames[i]);
        writeTrans[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalWriteTrans[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");

        sprintf(busName,"%sReadBytes", busNames[i]);
        readBytes[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalReadBytes[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");

        sprintf(busName,"%sWriteBytes", busNames[i]);
        writeBytes[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalWriteBytes[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
    }

    preloadTrans = &getSM(gpuId).getNumericStatistic("PreloadTransactions", u32bit(0), "MemoryController", "MC");
    unusedCycles = &getSM(gpuId).getNumericStatistic("UnusedCycles", u32bit(0), "MemoryController", "MC");
    globalPreloadTrans = &getSM().getNumericStatistic("PreloadTransactions", u32bit(0), "MemoryController", "MC");
    globalUnusedCycles = &getSM().getNumericStatistic("UnusedCycles", u32bit(0), "MemoryController", "MC");
    ...
    for(i = 0; i < gpuMemoryBuses; i++)
    {
        ...
        dataCycles[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalDataCycles[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");

        sprintf(busName,"ReadToWritePenalty%02d", i);
        rtowCycles[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalRtowCycles[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");

        sprintf(busName,"WriteToReadPenalty%02d", i);
        wtorCycles[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalWtorCycles[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");

        sprintf(busName,"OpenPagePenalty%02d", i);
        openPageCycles[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalOpenPageCycles[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");

        sprintf(busName,"OpenPages%02d", i);
        openPages[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalOpenPages[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");

        sprintf(busName,"ReadBytesMemoryBus%02d", i);
        gpuBusReadBytes[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalGpuBusReadBytes[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");

        sprintf(busName,"WriteBytesMemoryBus%02d", i);
        gpuBusWriteBytes[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalGpuBusWriteBytes[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
    }

    sysBusReadBytes = &getSM(gpuId).getNumericStatistic("ReadBytesSystemBus", u32bit(0), "MemoryController", "MC");;
    sysBusWriteBytes = &getSM(gpuId).getNumericStatistic("WriteBytesSystemBus", u32bit(0), "MemoryController", "MC");;
    globalSysBusReadBytes = &getSM().getNumericStatistic("ReadBytesSystemBus", u32bit(0), "MemoryController", "MC");;
    globalSysBusWriteBytes = &getSM().getNumericStatistic("WriteBytesSystemBus", u32bit(0), "MemoryController", "MC");;

    for(i = 0; i < MAPPED_MEMORY_BUSES; i++)
    {
        sprintf(busName,"SystemDataCycles%02d", i);
        sysDataCycles[i] = &getSM(gpuId).getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
        globalSysDataCycles[i] = &getSM().getNumericStatistic(busName, u32bit(0), "MemoryController", "MC");
    }
...
void MemoryController::receiveTransaction(MemoryTransaction *memTrans)
{
    ...
    switch(memTrans->getCommand())
    {
        case MT_READ_REQ:
            ...
            globalReadTrans[memTrans->getRequestSource()]->inc();
        ...
        case MT_WRITE_DATA:
            ...
            globalWriteTrans[memTrans->getRequestSource()]->inc();
    ...
    globalTransactions[memTrans->getRequestSource()]->inc();
}
        ...
        else
        {
            ...
            if ((cycle - lastCycle[bus]) >= TRANSACTION_CYCLES)
            {
                    ...
                    ((cycle - lastCycle[bus]) < (TRANSACTION_CYCLES + gpuMemWriteLatency + gpuMemWriteToReadLatency)))
                {
                    ...
                    globalWtorCycles[address2Bus(memTrans->getAddress())]->inc();
                }
                else
                {
                    ...
                    globalOpenPageCycles[address2Bus(memTrans->getAddress())]->inc();
                }
            }
        }
            ...
            else
            {
                ...
                if ((cycle - lastCycle[bus]) >= TRANSACTION_CYCLES)
                {
                    ...
                    if ((lastCommand[bus] == MT_READ_REQ) && ((cycle - lastCycle[bus]) < (TRANSACTION_CYCLES + gpuMemReadLatency)))
                    {
                        ...
                        globalRtowCycles[address2Bus(memTrans->getAddress())]->inc();
                    }
                    else
                    {
    ...
    if ((pendingServices > 0) /*&& (serviceQueue[nextService].state == MRS_WAITING)*/)
    {
        ...
        if (busCycles[unit][unitID] == 0)
        {
            ...
            if (reserveBus[unit][unitID])
            {
                ...
                GPU_DEBUG_BOX(
                    printf("MemoryController %lld => Response %d to the GPU Unit %s[%d].  Bus cycles %d.\n",
...
void MemoryController::updateMemoryBus(u64bit cycle, u32bit bus)
{
    ...
    if (busCycles[MEMORYMODULE][bus] > 0)
    {
        ...
        globalDataCycles[address2Bus(moduleTrans[bus]->getAddress())]->inc();
...
//printf("MC "U64FMT" => remove MT_READ_DATA request %p bus %d\n", cycle, moduleTrans[bus], currentBusRequest[bus]);
                    ...
                    globalReadBytes[currentUnit]->inc(moduleTrans[bus]->getSize());
                    ...
                    globalGpuBusReadBytes[address2Bus(moduleTrans[bus]->getAddress())]->inc(moduleTrans[bus]->getSize());
...
//printf("MC "U64FMT" => remove MT_WRITE_DATA request %p bus %d\n", cycle, moduleTrans[bus], currentBusRequest[bus]);
                    ...
                    globalWriteBytes[currentUnit]->inc(moduleTrans[bus]->getSize());
                    ...
                    globalGpuBusWriteBytes[address2Bus(moduleTrans[bus]->getAddress())]->inc(moduleTrans[bus]->getSize());
        ...
        if (busCycles[SYSTEM][bus] == 0)
        {
            ...
            switch(systemTrans[bus]->getCommand())
            {
                case MT_READ_DATA:
                    ...
                    globalReadBytes[currentUnit]->inc(systemTrans[bus]->getSize());
                    sysBusReadBytes->inc(systemTrans[bus]->getSize());
                    globalSysBusReadBytes->inc(systemTrans[bus]->getSize());
                ...
                case MT_WRITE_DATA:
                    ...
                    globalWriteBytes[currentUnit]->inc(systemTrans[bus]->getSize());
                    sysBusWriteBytes->inc(systemTrans[bus]->getSize());
                    globalSysBusWriteBytes->inc(systemTrans[bus]->getSize());
