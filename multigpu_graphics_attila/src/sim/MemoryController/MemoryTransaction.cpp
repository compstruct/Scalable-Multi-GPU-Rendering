...
u64bit MemoryTransaction::globalMemReqID = 0;
...
u32bit MemoryTransaction::busWidth[LASTGPUBUS] =
{
    ...
    32, // Data bus with the Primitive mapper
    32, // Data bus with the Primitive syncer
#ifndef USE_MEMORY_CONTROLLER_V2
   16,  // Data bus with the Memory modules
#endif
    8  // Data bus with system memory
    //32, // Data bus with the Z Stencil Test unit sync (pixels depth/stencil)
    //32  // Data bus with the Color Write unit sync (pixels color)
};
...
MemoryTransaction::MemoryTransaction(MemTransCom com, u32bit addr,
    ...
    state(MS_BOTH), preloadData(NULL), debug(false), debugThread(0), sync(false), blit(false)
{
    ++instances;
    globalRequestID = globalMemReqID++;
    ...
    switch(com)
    {
        ...
        case MT_PRELOAD_DATA:
            //  Copy data from the input buffer to the write data buffer
	    preloadData = new u8bit[size];
            memcpy(preloadData, dataBuffer, size);
            //  Calculate transaction bus cycles
...
MemoryTransaction::MemoryTransaction(u32bit addr, u32bit sz, u8bit *dataBuffer,
    ...
    preloadData(NULL), debug(false), debugThread(0), sync(false), blit(false)
{
    ++instances;
    globalRequestID = globalMemReqID++;
...
MemoryTransaction::MemoryTransaction(MemoryTransaction *request)
{
    ...
    debug = request->debug;
    debugThread = request->debugThread;
    sync = request->sync;
    blit = request->blit;
...
MemoryTransaction::MemoryTransaction(MemState memState) : state(memState), preloadData(NULL),
	debug(false), debugThread(0), sync(false), blit(false)
{
    ++instances;
    globalRequestID = globalMemReqID++;
...
std::string MemoryTransaction::getRequestSourceStr(bool compactName) const
{
	...
	case PRIMITIVEMAPPER:
	   if ( compactName )
	       return string("PEMAPPER");
	   else
	       return string("PRIMITIVEMAPPER") ;
	case PRIMITIVESYNCER:
	   if ( compactName )
	       return string("PESYNCER");
	   else
	       return string("PRIMITIVESYNCER") ;
        default:
...
{
    --instances;
    if (command == MT_PRELOAD_DATA) delete[] preloadData;
}
