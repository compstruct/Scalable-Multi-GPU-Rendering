...
RecursiveDescent::RecursiveDescent(u32bit gpuId, char* gpuPrefix, RasterizerEmulator &emu, u32bit trFIFOSize,
...

{
    ...
    rastCommand = newInputSignal("RecursiveDescentCommand", 1, 1, gpuPrefix);

    /*  Create state signal to the rasterizer main box.  */
    rastState = newOutputSignal("RecursiveDescentState", 1, 1, gpuPrefix);
    ...
    newTriangle = newInputSignal("TriangleSetupOutput", 1, 2, gpuPrefix);

    /*  Create request signal to Triangle Setup.  */
    triangleRequest = newOutputSignal("TriangleSetupRequest", 1, 1, gpuPrefix);
    ...
    interpolator = newOutputSignal("InterpolatorNewFragment", stampFragments * stampsCycle, 1, gpuPredix);

    /*  Create state signal from the Interpolator.  */
    interpolatorState = newInputSignal("InterpolatorState", 1, 1, gpuPrefix);
