...
enum RastComm
{
    ...
    RSCOM_SYNC_COLOR,               /**<  Flush caches to memory (color).  */
    RSCOM_SYNC_STENCIL,             /**<  Flush caches to memory (z and stencil).  */
    RSCOM_COMP,                     /**<  Inter-GPU composition.  */
    RSCOM_CLEARROPBUFFER_FOR_COMP
};
...
public:
    ...
    bool getBlit() { return blit; }
    void setBlit(bool blit_) { blit = blit_; }
    bool getSyncForBlit() { return syncForBlit; }
    void setSyncForBlit(bool syncForBlit_) { syncForBlit = syncForBlit_; }

    void setBatchCounter(u32bit batchCounter_) { batchCounter = batchCounter_; }
    u32bit getBatchCounter() { return batchCounter; }
    void setFrameCounter(u32bit frameCounter_) { frameCounter = frameCounter_; }
    u32bit getFrameCounter() { return frameCounter; }
    
};

class RasterizerSyncCommand : public DynamicObject
{
public:
    u32bit block;
    u32bit address;
    ...
    u8bit* data;
    u32bit written;
    ROPBlockState *blockState;
    bool last;
    bool blit;

    RasterizerSyncCommand(u32bit block, u32bit addr, u32bit sz, u8bit* syncData, ROPBlockState *state, bool last);
    RasterizerSyncCommand(bool last);
    void setBlit(bool blit_) { blit = blit_; }
    bool getBlit() { return blit; }
    ~RasterizerSyncCommand();
};

class ROPBlockStateSyncCommand : public DynamicObject
{
public:
    u32bit block;
    ROPBlockState *blockState;
    ...
    ROPBlockStateSyncCommand(u32bit block, ROPBlockState *state, bool last);
    ~ROPBlockStateSyncCommand();
};
