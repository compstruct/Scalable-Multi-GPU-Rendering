...
Rasterizer::Rasterizer(u32bit gpuId, u32bit numGPUs, char* gpuPrefix, u32bit multiGPUExecMode_, u32bit execRound_,
    bool compSchedEnabled, bool idealComp, u32bit interGPUBusWidth, u32bit interGPUBusLat,
    RasterizerEmulator &rsEmu, u32bit trCycle, u32bit tsFIFOSize,
    u32bit trSetupUnits, u32bit trSetupLat, u32bit trSetupStartLat, u32bit trInputLat, u32bit trOutputLat,
    bool shSetup, u32bit trShQSz, u32bit stampsPerCycle, u32bit depthSamplesCycle,
    ...
    u32bit trBatchSz, u32bit trBatchQSz,
    ...
    u32bit syncQSz, u32bit interGPUSyncQSz, u32bit nStampUnits, f32bit dummyFragsGenPossibility,
    char **suPrefixes, char *name, Box *parent):

    /*  Initializations.  */
    multiGPUExecMode(multiGPUExecMode_), execRound(execRound_),
    ...
    genH(genHeight), genW(genWidth), displayW(displayWidth), displayH(displayHeight),
    trBatchSize(trBatchSz), trBatchQueueSize(trBatchQSz), recursiveMode(recursive),
    ...
    rastQueueSz(rastQSz), testQueueSz(testQSz), intQueueSz(intQSz), shQueueSz(shQSz), syncQueueSz(syncQSz),
    interGPUSyncQueueSz(interGPUSyncQSz), numStampUnits(nStampUnits), Box(gpuId, name, parent)

{
    ...
    stampsCycle = stampsPerCycle * numStampUnits;
    ...
    rastCommSignal = newInputSignal("RasterizerCommand", 1, 1, gpuPrefix);

    /*  Create state signal to the Command Processor.  */
    rastStateSignal = newOutputSignal("RasterizerState", 1, 1, gpuPrefix);
    ...
    triangleSetupComm = newOutputSignal("TriangleSetupCommand", 1, 1, gpuPrefix);

    /*  Create state signal from Triangle Setup.  */
    triangleSetupState = newInputSignal("TriangleSetupRasterizerState", 1, 1, gpuPrefix);

    /*  Create command signal to Triangle Traversal.  */
    triangleTraversalComm = newOutputSignal("TriangleTraversalCommand", 1, 1, gpuPrefix);

    /*  Create state signal from Triangle Traversal.  */
    triangleTraversalState = newInputSignal("TriangleTraversalState", 1, 1, gpuPrefix);

    /*  Create command signal to Hierarchical Z.  */
    hzCommand = newOutputSignal("HZCommand", 1, 1, gpuPrefix);

    /*  Create state sginal from Hierarchical Z.  */
    hzState = newInputSignal("HZState", 1, 1, gpuPrefix);

    /*  Create command signal to Interpolator box.  */
    interpolatorComm = newOutputSignal("InterpolatorCommand", 1, 1, gpuPrefix);

    /*  Create state signal from Interpolator box.  */
    interpolatorState = newInputSignal("InterpolatorRasterizerState", 1, 1, gpuPrefix);

    /*  Create command signal to the Fragment FIFO.  */
    fFIFOCommand = newOutputSignal("FFIFOCommand", 1, 1, gpuPrefix);

    /*  Create state signal from the Fragment FIFO.  */
    fFIFOState = newInputSignal("FFIFOState", 1, 1, gpuPrefix);


    /*  Create rasterizer boxes.  */

    /*  Create Triangle Setup box.  */
    sprintf(boxName, "GPU%dTriangleSetup", gpuId);
    ...
    sprintf(boxName, "GPU%dTriangleTraversal", gpuId);
	...
	gpuPrefix,
        ...
        displayW,               /*  Display tile width in scan tiles.  */
        displayH,               /*  Display tile height in scan tiles.  */
    ...
    sprintf(boxName, "GPU%dHierarchicalZ", gpuId);
	...
	gpuPrefix,
        ...
        hzQueueSize * stampsCycle,  /*  Size of the Hierarchical Z test queue.  */
    ...
    sprintf(boxName, "GPU%dInterpolator", gpuId);
    interpolator = new Interpolator(
	gpuId,
	gpuPrefix,
        rastEmu,                /*  Reference to the rasterizer emulator to use for interpolation.  */
        interpolators,          /*  Number of attribute interpolator units.  */
        stampsCycle * threadGroup / STAMP_FRAGMENTS,            /*  Stamps received per cycle.  */
        numStampUnits,          /*  Number of stamp units in the GPU.  */
        boxName,
	this);

    /*  Create Fragment FIFO box.  */
    sprintf(boxName, "GPU%dFragmentFIFO", gpuId);
	...
	gpuPrefix,
        ...
        rastQueueSz * stampsCycle,   /*  Generated stamp queue size (per stamp unit).  */
        testQueueSz * stampsPerCycle,   /*  Early Z tested stamp queue size.  */
        intQueueSz * threadGroup / STAMP_FRAGMENTS,             /*  Interpolated stamp queue size.  */
        shQueueSz,              /*  Shaded stamp queue size (per stamp unit).  */
	syncQueueSz * stampsPerCycle,
	interGPUSyncQueueSz,
	dummyFragsGenPossibility,
...
void Rasterizer::clock(u64bit cycle)
{
	...
	case RAST_COMP:
	    ...
	    //if (ffState == RAST_COMP_END && hierZState == RAST_CLEAR_END)
	    if (ffState == RAST_COMP_END)
	        state = RAST_COMP_END;
	...
	default:
            panic("Rasterizer", "clock", "Invalid state for Fragment FIFO.");
    ...
    u64bit cycle)
{
    ...
    switch(rastComm->getCommand())
    {
	    ...
	    rastAuxComm->setBatchCounter(rastComm->getBatchCounter());
	    rastAuxComm->setFrameCounter(rastComm->getFrameCounter());
	...
	case RSCOM_COMP:
            ...
            //rastAuxComm = new RasterizerCommand(RSCOM_CLEARROPBUFFER_FOR_COMP);
            ...
            /*  Send COMP command to Fragment FIFO.  */
            rastAuxComm = new RasterizerCommand(RSCOM_COMP);
    ...
    GPURegData data, u64bit cycle)
{
    ...
    switch(reg)
    {
        ...
        case GPU_COMP_Z_BUFFER_CLEAR:
            ...
            GPU_DEBUG_BOX(
                printf("Rasterizer => Write GPU_COMP_Z_BUFFER_CLEAR = %x.\n", clearDepthForComp);
            )
	...
	case GPU_DEPTH_MASK:
	    ...
	    depthMask = data.booleanVal;
            ...
            GPU_DEBUG_BOX(
                printf("Rasterizer => GPU_DEPTH_MASK = %s.\n", data.booleanVal ? "TRUE":"FALSE");
            )
        ...
        case GPU_COLOR_BLEND:
            ...
            GPU_DEBUG_BOX(
                printf("Rasterizer => GPU_COLOR_BLEND[%d] = %s.\n", subreg, data.booleanVal?"TRUE":"FALSE");
            )
...
void Rasterizer::setPtrOfKDTreeNodeExtentsInSortLast(kdTreeNodeExtentsInSortLast_t *ptr) {
    hierarchicalZ->setPtrOfKDTreeNodeExtentsInSortLast(ptr);
}
