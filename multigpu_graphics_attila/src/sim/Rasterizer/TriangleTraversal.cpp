...
TriangleTraversal::TriangleTraversal(u32bit gpuId, char* gpuPrefix, RasterizerEmulator &rasEmu, u32bit cycleTriangles, u32bit setupLat,
    ...
    u32bit displayWidth, u32bit displayHeight, u32bit overWidth, u32bit overHeight, u32bit scanWidth, u32bit scanHeight,
...

{
    ...
    /*  Create statistics.  */
    inputs = &getSM(gpuId).getNumericStatistic("InputTriangles", u32bit(0), "TriangleTraversal", "TT");
    requests = &getSM(gpuId).getNumericStatistic("RequestedTriangles", u32bit(0), "TriangleTraversal", "TT");
    outputs = &getSM(gpuId).getNumericStatistic("GeneratedFragments", u32bit(0), "TriangleTraversal", "TT");
    utilizationCycles = &getSM(gpuId).getNumericStatistic("UtilizationCycles", u32bit(0), "TriangleTraversal", "TT");
    stamps0frag = &getSM(gpuId).getNumericStatistic("Stamps0Fragment", u32bit(0), "TriangleTraversal", "TT");
    stamps1frag = &getSM(gpuId).getNumericStatistic("Stamps1Fragment", u32bit(0), "TriangleTraversal", "TT");
    stamps2frag = &getSM(gpuId).getNumericStatistic("Stamps2Fragment", u32bit(0), "TriangleTraversal", "TT");
    stamps3frag = &getSM(gpuId).getNumericStatistic("Stamps3Fragment", u32bit(0), "TriangleTraversal", "TT");
    stamps4frag = &getSM(gpuId).getNumericStatistic("Stamps4Fragment", u32bit(0), "TriangleTraversal", "TT");
    numRastStamps = &getSM(gpuId).getNumericStatistic("NumberOfSentRastStamps", u32bit(0), "TriangleTraversal", "TT");
    triangleBatchSize = &getSM(gpuId).getNumericStatistic("TriangleBatchSize", u32bit(0), "TriangleTraversal", "TT");

    globalInputs = &getSM().getNumericStatistic("InputTriangles", u32bit(0), "TriangleTraversal", "TT");
    globalRequests = &getSM().getNumericStatistic("RequestedTriangles", u32bit(0), "TriangleTraversal", "TT");
    globalOutputs = &getSM().getNumericStatistic("GeneratedFragments", u32bit(0), "TriangleTraversal", "TT");
    globalUtilizationCycles = &getSM().getNumericStatistic("UtilizationCycles", u32bit(0), "TriangleTraversal", "TT");
    globalStamps0frag = &getSM().getNumericStatistic("Stamps0Fragment", u32bit(0), "TriangleTraversal", "TT");
    globalStamps1frag = &getSM().getNumericStatistic("Stamps1Fragment", u32bit(0), "TriangleTraversal", "TT");
    globalStamps2frag = &getSM().getNumericStatistic("Stamps2Fragment", u32bit(0), "TriangleTraversal", "TT");
    globalStamps3frag = &getSM().getNumericStatistic("Stamps3Fragment", u32bit(0), "TriangleTraversal", "TT");
    globalStamps4frag = &getSM().getNumericStatistic("Stamps4Fragment", u32bit(0), "TriangleTraversal", "TT");
    globalNumRastStamps = &getSM().getNumericStatistic("NumberOfSentRastStamps", u32bit(0), "TriangleTraversal", "TT");
    globalTriangleBatchSize = &getSM().getNumericStatistic("TriangleBatchSize", u32bit(0), "TriangleTraversal", "TT");
    ...
    traversalCommand = newInputSignal("TriangleTraversalCommand", 1, 1, gpuPrefix);

    /*  Create state signal to the main rasterizer box.  */
    traversalState = newOutputSignal("TriangleTraversalState", 1, 1, gpuPrefix);
    ...
    setupTriangle = newInputSignal("TriangleSetupOutput", trianglesCycle, setupLatency, gpuPrefix);

    /*  Create request signal to Triangle Setup.  */
    setupRequest = newOutputSignal("TriangleSetupRequest", 1, 1, gpuPrefix);

    /*  Create new fragment signal to the Hierarchical Z early test.  */
    newFragment = newOutputSignal("HZInputFragment", stampsCycle * STAMP_FRAGMENTS, 1, gpuPrefix);

    /*  Create state signal from the Hierarchical Z early test.  */
    hzState = newInputSignal("HZTestState", 1, 1, gpuPrefix);
...
storedTriangles, requestedTriangles, lastFragment?"T":"F");
    ...
    switch(state)
    {
        ...
        case RAST_DRAWING:
		    ...
		    numStamps = 0;
                    ...
                    for(i = 0; (i < stampsCycle) && (!lastFragment); i++)
                    {
                        ...
                        if (stamp != NULL)
                        {
                            ...
                            switch(count)
                            {
                                    case 0: stamps0frag->inc(); globalStamps0frag->inc(); break;
                                    case 1: stamps1frag->inc(); globalStamps1frag->inc(); break;
                                    case 2: stamps2frag->inc(); globalStamps2frag->inc(); break;
                                    case 3: stamps3frag->inc(); globalStamps3frag->inc(); break;
                                    case 4: stamps4frag->inc(); globalStamps4frag->inc(); break;
                            }
                        }
			    ...
			    frInput->setBatchCounter(batchCounter);
			    frInput->setFrameCounter(frameCounter);

			    //if (stamp[j] &&
			    //    ((stamp[j]->getX() == 172 || stamp[j]->getX() == 173) && (stamp[j]->getY() == 146 || stamp[j]->getY() == 147))
			    //    //&& ((stampTileID.getX() == 0 && stampTileID.getY() == 1) ||
			    //    //    (stampTileID.getX() == 0 && stampTileID.getY() == 0))
			    //   )
			    //   frInput->setDebug(true);
            ...
            {
                printf("B_%d_%d", gpuId, batchCounter);
		fflush(stdout);
                /*  Set current triangle as finished.  */
                traversalFinished = TRUE;
		...
		triangleBatchSize->incavg(batchTriangles);
		globalTriangleBatchSize->incavg(batchTriangles);
