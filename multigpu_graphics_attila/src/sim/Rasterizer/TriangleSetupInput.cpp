...
TriangleSetupInput::TriangleSetupInput(u32bit ID,
    QuadFloat *attrib1, QuadFloat *attrib2, QuadFloat *attrib3,
    u32bit index1, u32bit index2, u32bit index3, bool lastTriangle)
{
...
void TriangleSetupInput::getIndexes(u32bit &index1, u32bit &index2, u32bit &index3) const
{
    index1 = indexes[0];
    index2 = indexes[1];
    index3 = indexes[2];
...
void TriangleSetupInput::setMappedGPUIds(u32bit gpuId)
{
    GPU_ASSERT(
	if (gpuId > mappedGPUIds.size())
            panic("TriangleSetupInput", "setMappedGPUIds", "Exceed the supported maximum number of GPUs.");	
    )

    mappedGPUIds.set(gpuId);
}

u32bit TriangleSetupInput::getMappedGPUIds() const
{
    return (u32bit)(mappedGPUIds.to_ulong());
}
