...
Interpolator::Interpolator(u32bit gpuId, char* gpuPrefix, RasterizerEmulator &rastEmu, u32bit interp,
...

{
    ...
    interpolatorCommand = newInputSignal("InterpolatorCommand", 1, 1, gpuPrefix);

    /*  Create state signal to the main rasterizer box.  */
    interpolatorRastState = newOutputSignal("InterpolatorRasterizerState", 1, 1, gpuPrefix);
    ...
    newFragment = newInputSignal("InterpolatorInput", stampsCycle * STAMP_FRAGMENTS, 1, gpuPrefix);

    /*  Create interpolation start signal.  */
    interpolationStart = newOutputSignal("Interpolation",
        stampsCycle * STAMP_FRAGMENTS, MAX_INTERPOLATION_LATENCY, gpuPrefix);
    ...
    interpolatorOutput = newOutputSignal("InterpolatorOutput", stampsCycle * STAMP_FRAGMENTS, 1, gpuPrefix);
...
void Interpolator::clock(u64bit cycle)
{
    ...
    switch(state)
    {
        ...
        case RAST_DRAWING:
		    ...
		    if (frInput->getDebug()) {
		        FILE *debug = fopen("debugIntOp.txt", "a+");
		        fprintf(debug, "GPU[%d] Interpolator => cycle %lld batch %d stamp at (tri = %d, tile = (%d, %d), x = %d, y = %d z = %d).\n",
                                gpuId, cycle, batchCounter, frInput->getTriangleID(), frInput->getTileID().getX(), frInput->getTileID().getY(),
		        	frInput->getFragment()->getX(), frInput->getFragment()->getY(), frInput->getFragment()->getZ());
		        s32bit v1, v2, v3;
		        frInput->getFragment()->getTriangle()->getIndexes(v1, v2, v3);
		        fprintf(debug, "indexes {%d, %d, %d}, coords {%f, %f, %f, %f}\n",
		        	v1, v2, v3,
		        	frInput->getFragment()->getCoordinates()[0],
		        	frInput->getFragment()->getCoordinates()[1],
		        	frInput->getFragment()->getCoordinates()[2],
		        	frInput->getFragment()->getCoordinates()[3]);
                        for(i = 0; i < MAX_FRAGMENT_ATTRIBUTES; i++) {
                            fprintf(debug, "attr %d fragmentAttributes %s interpolation %s { %f, %f, %f, %f }\n",
		                    i, fragmentAttributes[i] ? "T" : "F", interpolation[i] ? "T" : "F",
		                    attributes[i][0], attributes[i][1],
                                    attributes[i][2], attributes[i][3]);
		            if (fragmentAttributes[i] && interpolation[i]) {
		                fprintf(debug, "triAttr[0] = { %f, %f, %f, %f }\n",
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(0, i)))[0],
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(0, i)))[1],
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(0, i)))[2],
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(0, i)))[3]);
		                fprintf(debug, "triAttr[1] = { %f, %f, %f, %f }\n",
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(1, i)))[0],
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(1, i)))[1],
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(1, i)))[2],
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(1, i)))[3]);
		                fprintf(debug, "triAttr[2] = { %f, %f, %f, %f }\n",
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(2, i)))[0],
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(2, i)))[1],
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(2, i)))[2],
		        		(*(frInput->getFragment()->getTriangle()->getAttribute(2, i)))[3]);
		            }
		        }
		        fflush(debug);
		        fclose(debug);
