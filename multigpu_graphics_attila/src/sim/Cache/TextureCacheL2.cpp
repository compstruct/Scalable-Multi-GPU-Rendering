...
TextureCacheL2::TextureCacheL2(u32bit gpuId, u32bit shaderID_, u32bit unitID_, u32bit cacheID_,
	u32bit numTextUnits, u32bit waysL0, u32bit linesL0, u32bit bytesLineL0,
    ...
    gpuId(gpuId), shaderID(shaderID_), unitID(unitID_),
...

{
    ...
    //cacheID = cacheCounter - gpuId * numTextUnits;
    cacheID = cacheID_;
    ...
    cacheL0 = new FetchCache64(gpuId, shaderID, unitID, waysL0, linesL0, lineSizeL0, reqQSize, postfix);

    /*  Create the L1 fetch cache object.  */
    cacheL1 = new FetchCache(gpuId, waysL1, linesL1, lineSizeL1, reqQSize, postfix);
    cacheL1->setShaderID(shaderID);
    cacheL1->setUnitID(unitID);

    /*  Create statistics.  */
    fetchBankConflicts = &GPUStatistics::StatisticsManager::instance(gpuId).getNumericStatistic("FetchBankConflicts", u32bit(0), "TextureCache", postfix);
    readBankConflicts = &GPUStatistics::StatisticsManager::instance(gpuId).getNumericStatistic("ReadBankConflicts", u32bit(0), "TextureCache", postfix);
    memRequests = &GPUStatistics::StatisticsManager::instance(gpuId).getNumericStatistic("MemoryRequests", u32bit(0), "TextureCache", postfix);
    memReqLatency = &GPUStatistics::StatisticsManager::instance(gpuId).getNumericStatistic("MemoryRequestLatency", u32bit(0), "TextureCache", postfix);
    pendingRequests = &GPUStatistics::StatisticsManager::instance(gpuId).getNumericStatistic("PendingMemoryRequests", u32bit(0), "TextureCache", postfix);
    pendingRequestsAvg = &GPUStatistics::StatisticsManager::instance(gpuId).getNumericStatistic("PendingMemoryRequestsAvg", u32bit(0), "TextureCache", postfix);

    globalFetchBankConflicts = &GPUStatistics::StatisticsManager::instance().getNumericStatistic("FetchBankConflicts", u32bit(0), "TextureCache", postfix);
    globalReadBankConflicts = &GPUStatistics::StatisticsManager::instance().getNumericStatistic("ReadBankConflicts", u32bit(0), "TextureCache", postfix);
    globalMemRequests = &GPUStatistics::StatisticsManager::instance().getNumericStatistic("MemoryRequests", u32bit(0), "TextureCache", postfix);
    globalMemReqLatency = &GPUStatistics::StatisticsManager::instance().getNumericStatistic("MemoryRequestLatency", u32bit(0), "TextureCache", postfix);
    globalPendingRequests = &GPUStatistics::StatisticsManager::instance().getNumericStatistic("PendingMemoryRequests", u32bit(0), "TextureCache", postfix);
    globalPendingRequestsAvg = &GPUStatistics::StatisticsManager::instance().getNumericStatistic("PendingMemoryRequestsAvg", u32bit(0), "TextureCache", postfix);
		           ...
		           bool debug, u32bit debugThread)
{
    ...
    fetchResult = cacheL0->fetch(address, way, line, isMiss, ready, source, debug, debugThread);
...
/*  Reads vertex data.  */
bool TextureCacheL2::read(u64bit address, u32bit way, u32bit line, u32bit size, u8bit *data, bool debug, u32bit debugThread)
{
    u32bit port;
    u32bit i;
    u64bit bank;
    ...
    if (i == dataBankAccess[bank])
    {
        ...
        else
        {
            ...
            if (cacheL0->read(address, way, line, size, data, debug, debugThread))
            {
    ...
    else
    {
        ...
        return cacheL0->read(address, way, line, size, data, debug, debugThread);
    }
}
...
void TextureCacheL2::clock(u64bit cycle)
{
    ...
    if ((cacheRequestL0 != NULL) && (freeReadsL0 > 0))
    {
        ...
        if ((cacheRequestL0->fill) && (freeReadsL0 > 0))
        {
            ...
            readQueueL0[nextFreeReadL0].debug = cacheRequestL0->debug;
            readQueueL0[nextFreeReadL0].debugThread = cacheRequestL0->debugThread;
    ...
    if ((cacheRequestL1 != NULL) && (freeReadsL1 > 0))
    {
        ...
        {
            GPU_DEBUG(
                printf("TextureCacheL2-%02d => At cycle %lld.  Adding read (fill) L1 cache request with ID %d to input request entry %d for line (%d, %d) address %x.\n",
            ...
            readQueueL1[nextFreeReadL1].debug = cacheRequestL1->debug;
            readQueueL1[nextFreeReadL1].debugThread = cacheRequestL1->debugThread;
    ...
    if (memoryCycles > 0)
    {
        ...
        if (memoryCycles == 0)
        {
            ...
            globalMemRequests->inc();
            globalMemReqLatency->inc(cycle - memRStartCycle[readTicket]);
        }
    }
...
//}
        ...
        if (cacheL1->fetch(readQueueL0[nextFetchInputL0].memAddress,
            ...
            readQueueL0[nextFetchInputL0].request->source, 1,
	    readQueueL0[nextFetchInputL0].debug, readQueueL0[nextFetchInputL0].debugThread))
        {
            GPU_DEBUG(
    ...
    if (waitReadInputsL0 > 0)
    {
	    ...
	    readQueueL0[nextReadInputL0].debug, readQueueL0[nextReadInputL0].debugThread))
        {
            GPU_DEBUG(
    ...
    if ((readInputsL0 > 0) && (uncompressCycles == 0))
    {
        ...
        switch(readQueueL0[nextReadL0].address & TEXTURE_ADDRESS_SPACE_MASK)
        {
            case UNCOMPRESSED_TEXTURE_SPACE:
                ...
                //printf("\n");
                if (readQueueL0[nextReadL0].debug) {
	            char filename[32];
	            sprintf(filename, "debugVSFInput%d.txt", shaderID);
	            FILE* debugfile = fopen(filename, "a+");
		    fprintf(debugfile, "TU:%d:%d:%d thread %d uncompress address %llx size %d \n",
			    gpuId, shaderID, unitID,
			    readQueueL0[nextReadL0].debugThread,
			    readQueueL0[nextReadL0].address,
			    readQueueL0[nextReadL0].size);
		    for (u32bit index = 0; index < lineSizeL0; index++)
			fprintf(debugfile, "data[%d] = %d ", index, inputBufferL0[nextReadL0][index]);
		    fprintf(debugfile, "\n");
	            fflush(debugfile);
	            fclose(debugfile);	    
		}
            ...
            case COMPRESSED_TEXTURE_SPACE_DXT1_RGB:
                ...
                TextureEmulator::decompressDXT1RGB(comprBuffer, inputBufferL0[nextReadL0], lineSizeL0 >> DXT1_SPACE_SHIFT,
				                   readQueueL0[nextReadL0].debug, readQueueL0[nextReadL0].debugThread, shaderID);
            ...
            case COMPRESSED_TEXTURE_SPACE_DXT3_RGBA:
                ...
                /*  Uncompress the block.  */
                TextureEmulator::decompressDXT3RGBA(comprBuffer, inputBufferL0[nextReadL0], lineSizeL0 >> DXT3_DXT5_SPACE_SHIFT,
				                    readQueueL0[nextReadL0].debug, readQueueL0[nextReadL0].debugThread, shaderID);
            ...
            case COMPRESSED_TEXTURE_SPACE_DXT5_RGBA:
                ...
                /*  Uncompress the block.  */
                TextureEmulator::decompressDXT5RGBA(comprBuffer, inputBufferL0[nextReadL0], lineSizeL0 >> DXT3_DXT5_SPACE_SHIFT,
				                    readQueueL0[nextReadL0].debug, readQueueL0[nextReadL0].debugThread, shaderID);
            ...
            case COMPRESSED_TEXTURE_SPACE_LATC1:
                ...
                //  Uncompress the block.
                TextureEmulator::decompressLATC1(comprBuffer, inputBufferL0[nextReadL0], lineSizeL0 >> LATC1_LATC2_SPACE_SHIFT,
				                 readQueueL0[nextReadL0].debug, readQueueL0[nextReadL0].debugThread, shaderID);
            ...
            case COMPRESSED_TEXTURE_SPACE_LATC1_SIGNED:
                ...
                //  Uncompress the block.
                TextureEmulator::decompressLATC1Signed(comprBuffer, inputBufferL0[nextReadL0], lineSizeL0 >> LATC1_LATC2_SPACE_SHIFT,
				                       readQueueL0[nextReadL0].debug, readQueueL0[nextReadL0].debugThread, shaderID);
            ...
            case COMPRESSED_TEXTURE_SPACE_LATC2:
                ...
                //  Uncompress the block.
                TextureEmulator::decompressLATC2(comprBuffer, inputBufferL0[nextReadL0], lineSizeL0 >> LATC1_LATC2_SPACE_SHIFT,
				                 readQueueL0[nextReadL0].debug, readQueueL0[nextReadL0].debugThread, shaderID);
            ...
            case COMPRESSED_TEXTURE_SPACE_LATC2_SIGNED:
                ...
                //  Uncompress the block.
                TextureEmulator::decompressLATC2Signed(comprBuffer, inputBufferL0[nextReadL0], lineSizeL0 >> LATC1_LATC2_SPACE_SHIFT,
				                       readQueueL0[nextReadL0].debug, readQueueL0[nextReadL0].debugThread, shaderID);
    ...
    globalPendingRequests->inc(MAX_MEMORY_TICKETS - freeTickets);
    globalPendingRequestsAvg->incavg(MAX_MEMORY_TICKETS - freeTickets);
}
...
MemoryTransaction *TextureCacheL2::requestBlock(u32bit readReq)
{
	...
	memTrans->setDebug(readQueueL1[readReq].debug);
	memTrans->setDebugThread(readQueueL1[readReq].debugThread);
...
void TextureCacheL2::stallReport(u64bit cycle, string &stallReport)
{
    ...
    char fullName[128];
    sprintf(fullName, "GPU%dTextureCacheL2-%02d", gpuId, cacheID);
