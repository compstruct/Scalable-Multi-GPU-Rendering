...
ColorCache::ColorCache(u32bit gpuId, u32bit ways, u32bit lines, u32bit stampsLine,
    ...
    gpuId(gpuId), lineStamps(stampsLine), stampBytes(bytesStamp), readPorts(readP), writePorts(writeP), portWidth(pWidth),
...

{
    ...
    cache = new FetchCache(gpuId, ways, lines, lineStamps * bytesStamp, reqQSize, postfix);
