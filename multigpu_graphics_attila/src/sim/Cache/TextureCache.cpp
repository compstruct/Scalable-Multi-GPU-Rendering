...
TextureCache::TextureCache(u32bit gpuId, u32bit shaderID_, u32bit unitID_, u32bit cacheID_,
	u32bit numTextUnits, u32bit ways, u32bit lines, u32bit bytesLine,
    ...
    gpuId(gpuId), shaderID(shaderID_), unitID(unitID_), portWidth(pWidth), inputRequests(inReqs), lineSize(bytesLine), banks(numBanks),
...

{
    ...
    cache = new FetchCache64(gpuId, shaderID, unitID, ways, lines, lineSize, reqQSize, postfix);
		         ...
		         bool debug, u32bit debugThread)
{
    ...
    fetchResult = cache->fetch(address, way, line, isMiss, ready, source, debug, debugThread);
...
/*  Reads vertex data.  */
bool TextureCache::read(u64bit address, u32bit way, u32bit line, u32bit size, u8bit *data, bool debug, u32bit debugThread)
{
    u32bit port;
    u32bit i;
    u64bit bank;
    ...
    if (i == dataBankAccess[bank])
    {
        ...
        else
        {
            ...
            if (cache->read(address, way, line, size, data, debug, debugThread))
            {
    ...
    else
    {
        ...
        return cache->read(address, way, line, size, data, debug, debugThread);
    }
}
...
void TextureCache::stallReport(u64bit cycle, string &stallReport)
{
    ...
    char fullName[128];
    sprintf(fullName, "GPU%dTextureCacheL1-%02d", gpuId, cacheID);
