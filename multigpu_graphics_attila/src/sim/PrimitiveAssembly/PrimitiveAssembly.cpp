...
PrimitiveAssembly::PrimitiveAssembly(u32bit gpuId, char* gpuPrefix, u32bit numGPUs_, u32bit multiGPUExecMode_, u32bit execRound_,
    u32bit vertCycle, u32bit trCycle, u32bit attrCycle, u32bit vertBusLat,
    u32bit paQueueSz, u32bit clipStartLat, u32bit rastStartLat, u32bit rastOutputLat, bool profileForSortLast_,
    char *name, Box *parent) :

    numGPUs(numGPUs_), multiGPUExecMode(multiGPUExecMode_), execRound(execRound_),
    verticesCycle(vertCycle), trianglesCycle(trCycle), attributesCycle(attrCycle), vertexBusLat(vertBusLat),
    paQueueSize(paQueueSz), clipperStartLat(clipStartLat),
    rasterizerStartLat(rastStartLat), rasterizerOutputLat(rastOutputLat),
    profileForSortLast(profileForSortLast_), Box(gpuId, name, parent)
{
    ...
    assemblyCommand = newInputSignal("PrimitiveAssemblyCommand", 1, 1, gpuPrefix);

    /*  Create state signal to the Command Processor.  */
    commandProcessorState = newOutputSignal("PrimitiveAssemblyCommandState", 1, 1, gpuPrefix);
    ...
    streamerNewOutput = newInputSignal("PrimitiveAssemblyInput", verticesCycle,
        vertexBusLat + MAX_VERTEX_ATTRIBUTES / attributesCycle, gpuPrefix);

    /*  Create request signal to the Streamer Commit.  */
    assemblyRequest = newOutputSignal("PrimitiveAssemblyRequest", 1, 1, gpuPrefix);

    /*  Create triangle output signal to the Clipper.  */
    clipperInput = newOutputSignal("ClipperInput", trianglesCycle, 1, gpuPrefix);

    /*  Create request signal from the Clipper.  */
    clipperRequest = newInputSignal("ClipperRequest", 1, 1, gpuPrefix);

    if (numGPUs > 1 && multiGPUExecMode == GPUPD && execRound == 2) {
        rasterizerNewTriangle = newOutputSignal("RasterizerNewTriangleFromPAS", trianglesCycle, rasterizerOutputLat, gpuPrefix);
        rasterizerRequest = newInputSignal("PASRasterizerRequest", 1, 1, gpuPrefix);
    } else {
        rasterizerNewTriangle = NULL;
	rasterizerRequest = NULL;
    }
    ...
    for(i = 0; i < paQueueSize; i++)
    {
        ...
        assemblyQueue[i].attributes3D = NULL;
    }

    /*  Create statistics.  */
    vertices = &getSM(gpuId).getNumericStatistic("Vertices", u32bit(0), "PrimitiveAssembly", "PA");
    requests = &getSM(gpuId).getNumericStatistic("Requests", u32bit(0), "PrimitiveAssembly", "PA");
    triangles = &getSM(gpuId).getNumericStatistic("Triangles", u32bit(0), "PrimitiveAssembly", "PA");
    degenerated = &getSM(gpuId).getNumericStatistic("Degenerated", u32bit(0), "PrimitiveAssembly", "PA");

    globalVertices = &getSM().getNumericStatistic("Vertices", u32bit(0), "PrimitiveAssembly", "PA");
    globalRequests = &getSM().getNumericStatistic("Requests", u32bit(0), "PrimitiveAssembly", "PA");
    globalTriangles = &getSM().getNumericStatistic("Triangles", u32bit(0), "PrimitiveAssembly", "PA");
    globalDegenerated = &getSM().getNumericStatistic("Degenerated", u32bit(0), "PrimitiveAssembly", "PA");
    ...
    inputVertexFile = NULL;
    GPU_ASSERT(
        if (profileForSortLast && numGPUs > 1)
            panic("PrimitiveAssembly", "PrimitiveAssembly", "Only profile input vertexes in single-GPU system.");
    )
    vertexProgramPC = 0;
    vertexProgramPCChanged = false;
    fragmentProgramPC = 0;
    fragmentProgramPCChanged = false;
    ...
    switch(state)
    {
        case PAST_RESET:
            ...
            for(i = 0; i < paQueueSize; i++)
            {
                ...
                if (assemblyQueue[i].attributes3D != NULL)
                {
                    delete[] assemblyQueue[i].attributes3D;
                    assemblyQueue[i].attributes3D = NULL;
                }
            }
	    ...
	    if (numGPUs >1 && multiGPUExecMode == GPUPD && execRound == 2)
	    {
	        if ((requestedTriangles > 0) && (rasterizerCycles == 0) && lastTriangle)
		{
                    /*  Send the last triangle signal down the pipeline.  */
                    setupInput = new TriangleSetupInput(triangleCount, NULL, NULL, NULL, 0, 0, 0, TRUE);
                    ...
                    rasterizerNewTriangle->write(cycle, setupInput);
	    ...
	    if (numGPUs > 1 && multiGPUExecMode == GPUPD && execRound == 2) {
	        if (rasterizerCycles == 0)
		{
                    for(i = 0; (i < trianglesCycle) && (requestedTriangles > 0); i++)
                    {
		...
		if (clipCycles == 0)
            ...
            while (streamerNewOutput->read(cycle, (DynamicObject *&) paInput))
            {
                ...
                delete[] assemblyQueue[nextFreeEntry].attributes3D;
                ...
                assemblyQueue[nextFreeEntry].attributes3D = paInput->get3DAttributes();
		...
		GPU_ASSERT(
		    if (numGPUs > 1 && multiGPUExecMode == GPUPD && execRound == 2 && primitiveMode != TRIANGLE)
		        panic("PrimitiveAssembly", "clock", "Round 2 of GPUpd only support TRIANGLE.");
		)
...
{
    if (state == PAST_DRAWING && !(profileForSortLast && command->getCommand() == PACOM_REG_WRITE && command->getRegister() == GPU_FRAGMENT_PROGRAM_PC))
	panic("PrimitiveAssembly", "processCommand", "Should not receive this command in state PAST_DRAWING.");
    ...
    switch(command->getCommand())
    {
	    ...
	    if (profileForSortLast) {
		inputVertexFile = fopen("inputVertexes.obj", "a+");
		if (vertexProgramPCChanged)
		    fprintf(inputVertexFile, "vertexProgramPCChanged\n");
		vertexProgramPCChanged = false;
		if (fragmentProgramPCChanged)
		    fprintf(inputVertexFile, "fragmentProgramPCChanged\n");
		fragmentProgramPCChanged = false;
		fprintf(inputVertexFile, "d %d %d\n", frameCounter, batchCounter);
	    }
	    ...
	    if (profileForSortLast) {
	        fflush(inputVertexFile);
		fclose(inputVertexFile);
	    }
        ...
        case PACOM_REG_WRITE:
            ...
            GPU_ASSERT(
                if (state != PAST_READY && !(profileForSortLast && command->getRegister() == GPU_FRAGMENT_PROGRAM_PC))
                    panic("PrimitiveAssembly", "processCommand", "Command not supported in current state.");
            )
    ...
    GPURegData data)
{
	...
	case GPU_VERTEX_PROGRAM_PC:
	    vertexProgramPCChanged = (data.uintVal != vertexProgramPC);
	    vertexProgramPC = data.uintVal;

	    break;

	case GPU_FRAGMENT_PROGRAM_PC:
	    fragmentProgramPCChanged = (data.uintVal != fragmentProgramPC);
	    fragmentProgramPC = data.uintVal;
...
void PrimitiveAssembly::assembleTriangle(u64bit cycle)
{
    ...
    QuadFloat *vertexAttr2D1, *vertexAttr2D2, *vertexAttr2D3;
    QuadFloat *vertexAttr3D1, *vertexAttr3D2, *vertexAttr3D3;
    u32bit vertexIndex1, vertexIndex2, vertexIndex3;
        ...
        ((storedVertex >= 4) && ((primitiveMode == QUAD) || (primitiveMode == QUAD_STRIP)))))
    {
	    ...
	    if ((numGPUs == 1 && multiGPUExecMode == FRAME_DUP) || (numGPUs > 1 && multiGPUExecMode == SORT_LAST &&
	        ...
	        GPUDriver::getGPUDriver()->getGPUSchedInfo()->incNumProcessedVertexes(frameCounter, batchCounter, gpuId);
	    }
        }
        else
        {
	    ...
	    vertexIndex1 = assemblyQueue[vertex1].index;
	    vertexIndex2 = assemblyQueue[vertex2].index;
	    vertexIndex3 = assemblyQueue[vertex3].index;

            /*  Create triangle setup input.  */
            setupInput = new TriangleSetupInput(triangleCount, vertexAttr1, vertexAttr2, vertexAttr3,
		vertexIndex1, vertexIndex2, vertexIndex3, FALSE);
	    ...
	    if (profileForSortLast) {
                /*  Allocate the triangle vertex attributes.  */
                vertexAttr2D1 = new QuadFloat[MAX_VERTEX_ATTRIBUTES];
                vertexAttr2D2 = new QuadFloat[MAX_VERTEX_ATTRIBUTES];
                vertexAttr2D3 = new QuadFloat[MAX_VERTEX_ATTRIBUTES];

                /*  Check memory allocation.  */
                GPU_ASSERT(
                    if ((vertexAttr2D1 == NULL) || (vertexAttr2D2 == NULL) || (vertexAttr2D3 == NULL))
                        panic("PrimitiveAssembly", "clock", "Error allocating triangle vertex 2D attributes.");
                )

                /*  Copy the triangle vertex attributes.  */
                memcpy(vertexAttr2D1, assemblyQueue[vertex1].attributes, sizeof(QuadFloat) * MAX_VERTEX_ATTRIBUTES);
                memcpy(vertexAttr2D2, assemblyQueue[vertex2].attributes, sizeof(QuadFloat) * MAX_VERTEX_ATTRIBUTES);
                memcpy(vertexAttr2D3, assemblyQueue[vertex3].attributes, sizeof(QuadFloat) * MAX_VERTEX_ATTRIBUTES);
                
		vertexAttr3D1 = assemblyQueue[vertex1].attributes3D;
                vertexAttr3D2 = assemblyQueue[vertex2].attributes3D;
                vertexAttr3D3 = assemblyQueue[vertex3].attributes3D;

		bool clipped = ClipperEmulator::trivialReject(vertexAttr2D1[0], vertexAttr2D2[0], vertexAttr2D3[0], d3d9DepthRange);

		if (d3d9PixelCoordinates) {
	  	    vertexAttr2D1[POSITION_ATTRIBUTE][1] = -vertexAttr2D1[POSITION_ATTRIBUTE][1];
	  	    vertexAttr2D2[POSITION_ATTRIBUTE][1] = -vertexAttr2D2[POSITION_ATTRIBUTE][1];
	  	    vertexAttr2D3[POSITION_ATTRIBUTE][1] = -vertexAttr2D3[POSITION_ATTRIBUTE][1];
		}
		
		for (u32bit i = 0; i < 3; i++) {
		    vertexAttr2D1[POSITION_ATTRIBUTE][i] = vertexAttr2D1[POSITION_ATTRIBUTE][i]/vertexAttr2D1[POSITION_ATTRIBUTE][3];
		    vertexAttr2D2[POSITION_ATTRIBUTE][i] = vertexAttr2D2[POSITION_ATTRIBUTE][i]/vertexAttr2D2[POSITION_ATTRIBUTE][3];
		    vertexAttr2D3[POSITION_ATTRIBUTE][i] = vertexAttr2D3[POSITION_ATTRIBUTE][i]/vertexAttr2D3[POSITION_ATTRIBUTE][3];
		}

		fprintf(inputVertexFile, "f %d %d %d %s %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f %f\n",
			vertexIndex1, vertexIndex2, vertexIndex3, (clipped ? "T" : "F"),
			vertexAttr3D1[POSITION_ATTRIBUTE][0], vertexAttr3D1[POSITION_ATTRIBUTE][1], vertexAttr3D1[POSITION_ATTRIBUTE][2],
			vertexAttr3D2[POSITION_ATTRIBUTE][0], vertexAttr3D2[POSITION_ATTRIBUTE][1], vertexAttr3D2[POSITION_ATTRIBUTE][2],
			vertexAttr3D3[POSITION_ATTRIBUTE][0], vertexAttr3D3[POSITION_ATTRIBUTE][1], vertexAttr3D3[POSITION_ATTRIBUTE][2],
			vertexAttr2D1[POSITION_ATTRIBUTE][0], vertexAttr2D1[POSITION_ATTRIBUTE][1], vertexAttr2D1[POSITION_ATTRIBUTE][2],
			vertexAttr2D2[POSITION_ATTRIBUTE][0], vertexAttr2D2[POSITION_ATTRIBUTE][1], vertexAttr2D2[POSITION_ATTRIBUTE][2],
			vertexAttr2D3[POSITION_ATTRIBUTE][0], vertexAttr2D3[POSITION_ATTRIBUTE][1], vertexAttr2D3[POSITION_ATTRIBUTE][2]);

		delete [] vertexAttr2D1;
		delete [] vertexAttr2D2;
		delete [] vertexAttr2D3;
	    }
        }
    }
}
