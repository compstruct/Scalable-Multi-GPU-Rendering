...
//  Constructor.
GPUEmulator::GPUEmulator(u32bit gpuId, SimParameters simP, TraceDriverInterface *trDriver) :

    gpuId(gpuId), simP(simP), trDriver(trDriver), abortEmulation(false), traceEnd(false),
...
printf("GPUEmulator => Creating rasterizer emulator.\n");
	...
	simP.numGPUs,
        ...
        simP.ras.displayScanWidth,      /*  Display tile width in multi-GPU architecture.  */
        simP.ras.displayScanHeight,     /*  Display tile height in multi-GPU architecture.  */
...
printf("GPUEmulator => Creating shader emulator.\n");
	...
	0,
        (char*)(std::string("ShaderEmu").c_str()),      //  Shader name.
...
GPUEmulator::CompressedTextureCache::CompressedTextureCache(GPUEmulator &emu, u32bit blocks, u32bit blockSize, u64bit blockMask, u32bit ratioShift,
                                                            void (*decompFunc) (u8bit *, u8bit *, u32bit, bool, u32bit, u32bit)) :
    emu(emu)
{
...
void GPUEmulator::CompressedTextureCache::readData(u64bit address, u8bit *data, u32bit size)
{
    ...
    else
    {
        ...
        decompressionFunction(&memory[comprAddress], &decompressedData[cachedBlocks * decompressedBlockSize],
            decompressedBlockSize >> compressionRatioShift, false, 0, 0);
...
void GPUEmulator::emulateCommandProcessor(AGPTransaction *currentTransaction)
{
    ...
    switch(currentTransaction->getAGPCommand())
    {
	    ...
	    currentTransaction->decRefs();
	    if (currentTransaction->getRefs() == 0) {
...
void GPUEmulator::processRegisterWrite(GPURegister gpuReg, u32bit gpuSubReg, GPURegData gpuData)
{
    ...
    switch (gpuReg)
    {
        ...
        case GPU_BLIT_DST_TX_BLOCK:
            ...
            GPU_DEBUG(
                printf("Write GPU_BLIT_DST_TX_BLOCK = ");
        ...
        case GPU_TEXTURE_NON_NORMALIZED:
            ...
            GPU_DEBUG(
                printf("Write GPU_TEXTURE_NON_NORMALIZED[%d] = %s", gpuSubReg, gpuData.booleanVal ? "T" : "F");
            )
        ...
        case GPU_TEXTURE_MIN_LOD:
            ...
            GPU_DEBUG(
                printf("Write GPU_TEXTURE_MIN_LOD[%d] = %u.\n", gpuSubReg, gpuData.uintVal);
            )
        ...
        case GPU_TEXTURE_MAX_LOD:
            ...
            GPU_DEBUG(
                printf("Write GPU_TEXTURE_MAX_LOD[%d] = %u.\n", gpuSubReg, gpuData.uintVal);
            )
...
void GPUEmulator::clearColorBuffer()
{
    ...
    if (!skipBatchMode)
    {
        ...
        pixelMapper[0].setupDisplay(state.displayResX, state.displayResY, STAMP_WIDTH, STAMP_WIDTH,
                                 ...
                                 simP.ras.displayScanWidth, simP.ras.displayScanHeight,
                                 samples, bytesPixel);
...
bool GPUEmulator::emulationLoopEnd()
{

    if (!traceEnd && (frameCounter < (simP.startFrame + simP.simFrames)) && !abortEmulation) {
	return false;
    } else {
        if (abortEmulation)
...
{
    char emuName[100];
    sprintf(emuName, "Emu[%d]", gpuId);
    GLOBALPROFILER_ENTERREGION(emuName, "driver", "")
    
    //  Get next transaction from the driver
    AGPTransaction *currentTransaction;
    currentTransaction = trDriver->nextAGPTransaction(gpuId);
    ...
    {
        traceEnd = true;
        printf("End of trace im emulator[%d].\n", gpuId);
    }
}
