...
u64bit GPUSimulator::globalGpuCycle = 0;
u64bit GPUSimulator::globalShaderCycle = 0;
u64bit GPUSimulator::globalMemoryCycle = 0;

struct GPUSync* GPUSimulator::gpuSync = NULL;

//  Constructor.
GPUSimulator::GPUSimulator(u32bit gpuId, SimParameters simP, TraceDriverInterface *trDriver, bool unified,
                           bool d3d9Trace, bool oglTrace, bool agpTrace, u32bit execRound_) :

    gpuId(gpuId), simP(simP), trDriver(trDriver), unifiedShader(unified),
    d3d9Trace(d3d9Trace), oglTrace(oglTrace), agpTrace(agpTrace), execRound(execRound_),
    sigBinder(SignalBinder::getBinder()), simulationStarted(false),
    endSimLoop(false), endMultiClockSimLoop(false), dotCount(0),
    cycle(0), gpuCycle(0), shaderCycle(0), memoryCycle(0)

{
    ...
    cyclesCounter = &GPUStatistics::StatisticsManager::instance(gpuId).getNumericStatistic("CycleCounter", u32bit(0), "GPU", 0);
    ...
    for(i = 0; i < simP.str.streamerLoaderUnits; i++)
    {
        ...
        //  Write prefix.
        //sprintf(slPrefixes[i], "SL%d", gpuId * simP.str.streamerLoaderUnits + i);
        sprintf(slPrefixes[i], "GPU%dSL%d", gpuId, i);
   }
    ...
    for(i = 0; i < numVShaders; i++)
    {
        ...
        //  Write prefix.
        //sprintf(vshPrefix[i], "VS%d", gpuId * numVShaders + i);
        sprintf(vshPrefix[i], "GPU%dVS%d", gpuId, i);
    }
    ...
    for(i = 0; i < simP.gpu.numFShaders; i++)
    {
        ...
        //  Write prefix.
        //sprintf(fshPrefix[i], "FS%d", gpuId * simP.gpu.numFShaders + i);
        sprintf(fshPrefix[i], "GPU%dFS%d", gpuId, i);
    }
    ...
    for(i = 0; i < simP.gpu.numFShaders; i++)
    {
        ...
        {
            //  Allocate prefix.
            tuPrefixes[i * simP.fsh.textureUnits + j] = new char[18];
            ...
            //  Write prefix.
            //sprintf(tuPrefixes[i * simP.fsh.textureUnits + j], "FS%dTU%d", gpuId * simP.gpu.numFShaders + i, j);
            sprintf(tuPrefixes[i * simP.fsh.textureUnits + j], "GPU%dFS%dTU%d", gpuId, i, j);
        }
    }
    ...
    for(i = 0; i < simP.gpu.numStampUnits; i++)
    {
        ...
        //  Write prefix.
        //sprintf(suPrefix[i], "SU%d", gpuId * simP.gpu.numStampUnits + i);
        sprintf(suPrefix[i], "GPU%dSU%d", gpuId, i);
    }
    ...
    sprintf(boxName, "GPU%dCommandProc", gpuId);
    commProc = new CommandProcessor(
        gpuId,                                          //  GPU Id in multi-GPU architecture
	gpuPrefix,
	simP.numGPUs,
	...
	simP.pe.gpupdBatchEnabled,
	simP.pe.gpupdBatchThreshold,
	simP.compSchedEnabled,
    ...
    sprintf(boxName, "GPU%dMemoryController", gpuId);
    memController = createMemoryController(gpuId, gpuPrefix,
		                           simP, (const char**) tuPrefixes, (const char**) suPrefix, (const char**) slPrefixes,
                                           boxName, 0);
    ...
    sprintf(boxName, "GPU%dStreamer", gpuId);
    ...
    if (!unifiedShader)
    {
        ...
        for(i = 0; i < simP.gpu.numVShaders; i++)
        {
            ...
            sprintf(boxName, "GPU%dVertSh%dEmulator", gpuId, i);
            ...
            //  Create Emulator name.
            sprintf(boxName, "GPU%dVertexShaderFetch%d", gpuId, i);
		...
		gpuPrefix,
            ...
            //  Create Emulator name.
            sprintf(boxName, "GPU%dVertexShaderDecExec%d", gpuId, i);
    ...
    sprintf(boxName, "GPU%dPrimitiveAssembly", gpuId);
    ...
    sprintf(boxName, "GPU%dClipper", gpuId);
    clipper = new Clipper(
	gpuId,
	gpuPrefix,
	simP.numGPUs,
	...
	simP.numGPUs,
        ...
        simP.ras.displayScanWidth,      //  Display tile width in scan tiles.
        simP.ras.displayScanHeight,     //  Display tile height in scan tiles.
    ...
    else
    {
        ...
        sprintf(boxName, "GPU%dRasterizer_Unified", gpuId);
        rast = new Rasterizer(
	    gpuId,
	    simP.numGPUs,
	    gpuPrefix,
	    ...
	    simP.compSchedEnabled,
	    simP.idealComp,
	    simP.interGPUBusWidth,
	    simP.interGPUBusLatency,
            *rastEmu,                       //  Rasterizer emulator.
            ...
            simP.ras.displayScanWidth,      //  Display tile width (scan tiles).
            simP.ras.displayScanHeight,     //  Display tile height (scan tiles).
	    ...
	    simP.ras.syncedStampQSize,
	    simP.ras.interGPUSyncedStampQSize,
            simP.gpu.numStampUnits,         //  Number of stamp units attached to the Rasterizer.
	    simP.zst.dummyFragsGenPossibility,
    ...
    for(i = 0; i < simP.gpu.numFShaders; i++)
    {
        ...
        //  Creat Emulator name.
        sprintf(boxName, "GPU%dFragmentShaderEmulator%d", gpuId, i);
		...
		i,
            ...
            //  Creat Emulator name.
            sprintf(boxName, "GPU%dVectorShaderFetch%d", gpuId, i);
		...
		i,
            ...
            //  Creat Emulator name.
            sprintf(boxName, "GPU%dVectorShaderDecExec%d", gpuId, i);
		...
		i,
            ...
            //  Creat Emulator name.
            sprintf(boxName, "GPU%dFragmentShaderFetch%d", gpuId, i);
		...
		gpuPrefix,
            ...
            //  Creat Emulator name.
            sprintf(boxName, "GPU%dFragmentShaderDecExec%d", gpuId, i);
        ...
        {
            //  Creat Emulator name.
            sprintf(boxName, "GPU%dTextureUnitFSh%d-%d", gpuId, i, j);
		...
		i,
		j,
	        i * simP.fsh.textureUnits + j,
		simP.gpu.numFShaders * simP.fsh.textureUnits,
    ...
    for(i = 0; i < simP.gpu.numStampUnits; i++)
    {
        ...
        //  Creat Emulator name.
        sprintf(boxName, "GPU%dZStencilTest%d", gpuId, i);
	    ...
	    i,
            ...
            simP.zst.inputQueueSize * simP.zst.stampsCycle,        //  Input stamps queue.
	    ...
	    simP.zst.syncCommQSz,
            simP.zst.syncInputQueueSize,
            simP.zst.syncFetchQueueSize,
            simP.zst.syncReadQueueSize,
	    simP.interGPUBusWidth,
	    simP.interGPUBusLatency,
	    simP.profileForSortLast,
	    simP.compSchedEnabled,
        ...
        // Creat Emulator name.
        sprintf(boxName, "GPU%dColorWrite%d", gpuId, i);
	    ...
	    i,
            ...
            simP.cwr.inputQueueSize * simP.cwr.stampsCycle,        //  Input stamps queue.
            ...
            *fragEmu[i],                    //  Reference to the Fragment Operation Emulator.
	    simP.cwr.syncCommQSz,
            simP.cwr.syncInputQueueSize,
            simP.cwr.syncFetchQueueSize,
            simP.cwr.syncReadQueueSize,
	    simP.interGPUBusWidth,
	    simP.interGPUBusLatency,
	    simP.profileForSortLast,
	    simP.compSchedEnabled,
    ...
    GPU_ASSERT(
        if (simP.zst.syncCommQSz != simP.cwr.syncCommQSz)
            panic("", "", "ColorWrite and ZStencilTest unit should have same sync command queue size");
    )

    //  Create DAC box.
    sprintf(boxName, "GPU%dDAC", gpuId);
    dac = new DAC(
        gpuId,                          //  GPU Id in multi-GPU architecture
	gpuPrefix,
	simP.numGPUs,
    ...
    // Create PrimitiveExchanger box.
    sprintf(boxName, "GPU%dPE", gpuId);
    primitiveExchanger = new PrimitiveExchanger(
        gpuId,
	gpuPrefix,
	simP.numGPUs,
	simP.multiGPUExecMode,
	...
	simP.interGPUBusWidth,
	simP.interGPUBusLatency,
	simP.pe.trianglesCycle,
	simP.pe.primitiveExchangerUnits,
	simP.pe.p2gMappingLatency,
	simP.pe.startLatency,
	simP.cpdDrawCommBatchSize,
	simP.pe.peTriangleInputBufferSize,
	simP.pe.mappedInfoReadBufferSize,
	simP.pe.exchangedIndexBufferSize,
        simP.ras.displayScanWidth,      //  Display tile width (scan tiles).
        simP.ras.displayScanHeight,     //  Display tile height (scan tiles).
        simP.ras.scanWidth,             //  Scan tile width (pixels).
        simP.ras.scanHeight,            //  Scan tile height (pixels).
	simP.pe.allResultsOnChip,
        boxName,
        NULL	
    );

    // Add PrimitiveExchanger box to the box array.
    boxArray.push_back(primitiveExchanger);
    if (multiClock)
	gpuDomainBoxes.push_back(primitiveExchanger);
    ...
    if (simP.statistics && simP.perGPUStatistics)
    {
        //  Set statistics rate 
        GPUStatistics::StatisticsManager::instance(gpuId).setDumpScheduling(0, simP.statsRate, false);

	char gpuStatsFile[128];
	sprintf(gpuStatsFile, "GPU%d.round%d.%s", gpuId, execRound, simP.statsFile);
        out.open(gpuStatsFile, ios::out | ios::binary);

	char msg[256];
	
	if (!out.is_open()) {
	    sprintf(msg, "Error opening statistics file in GPU%d", gpuId);
            panic("GPUSimulator", "GPUSimulator", msg);
	}
        
	GPUStatistics::StatisticsManager::instance(gpuId).setOutputStream(out);

        //  Check if per cycle statistics are enabled
        if (simP.perCycleStatistics)
        {
            //  Initialize the statistics manager.
	    sprintf(gpuStatsFile, "GPU%d.round%d.%s", gpuId, execRound, simP.statsFilePerCycle);
            outCycle.open(gpuStatsFile, ios::out | ios::binary);

	    if (!outCycle.is_open()) {
	        sprintf(msg, "Error opening per cycle statistics file in GPU%d", gpuId);
                panic("GPUSimulator", "GPUSimulator", msg);
	    }

            GPUStatistics::StatisticsManager::instance(gpuId).setPerCycleStream(outCycle);
        }

        //  Check if per frame statistics are enabled.
        if (simP.perFrameStatistics)
        {
	    sprintf(gpuStatsFile, "GPU%d.round%d.%s", gpuId, execRound, simP.statsFilePerFrame);
            outFrame.open(gpuStatsFile, ios::out | ios::binary);

	    if (!outFrame.is_open()) {
	        sprintf(msg, "Error opening per frame statistics file in GPU%d", gpuId);
                panic("GPUSimulator", "GPUSimulator", msg);
	    }

            GPUStatistics::StatisticsManager::instance(gpuId).setPerFrameStream(outFrame);
        }

        //  Check if per batch statistics are enabled.
        if (simP.perBatchStatistics)
        {
	    sprintf(gpuStatsFile, "GPU%d.round%d.%s", gpuId, execRound, simP.statsFilePerBatch);
            outBatch.open(gpuStatsFile, ios::out | ios::binary);

	    if (!outBatch.is_open()) {
	        sprintf(msg, "Error opening per batch statistics file in GPU%d", gpuId);
                panic("GPUSimulator", "GPUSimulator", msg);
	    }

            GPUStatistics::StatisticsManager::instance(gpuId).setPerBatchStream(outBatch);
        }
    }
...
void GPUSimulator::createSnapshot()
{
    ...
    {    
        ...
        {
            sprintf(directoryName, "GPU%dATTILAsnapshot%02d", current->gpuId, snapshotID);
...
void GPUSimulator::debugLoop(bool validate)
{
    ...
    if (validate)
    {
        ...
        gpuEmulator = new GPUEmulator(gpuId, simP, trDriver);
...
//emuVertexInputLog.size());
                ...
                {
                    printf("Validation => Number of read vertices is different: emulator = %ld simulator = %ld\n",
...
//emuShadedVertexLog.size());
                ...
                {
                    printf("Validation => Number of shaded vertices is different: emulator = %ld simulator = %ld\n",
...
//emuZStencilUpdateLog.size());
                ...
                {
                    printf("Validation => Number of updates to Z Stencil Buffer differs: emulator %ld simulator %ld\n",
...
//emuColorUpdateLog.size());
                    ...
                    {
                        printf("Validation => Number of updates to Color Write Buffer differs: emulator %ld simulator %ld\n",
        ...
        if (autoSnapshotEnable && !pendingSaveSnapshot)
        {
            ...
            if ((newTime - startTime) >= (snapshotFrequency * 60))
            {
                ...
                printf("Autosaving at time %lld.  Auto save frequency %d minutes.  Times since last save in seconds %lld\n",
...
void GPUSimulator::setPtrOfExchangedIndexesInGPUpd(exchangedIndexesInGPUpd_t *ptr)
{
    primitiveExchanger->setPtrOfExchangedIndexesInGPUpd(ptr);
}

void GPUSimulator::setPtrOfKDTreeNodeExtentsInSortLast(kdTreeNodeExtentsInSortLast_t *ptr)
{
    rast->setPtrOfKDTreeNodeExtentsInSortLast(ptr);
}
...
void GPUSimulator::simulationLoop()
{
    ...
    //  Simulation loop.
    GPU_DEBUG( printf(" Round%d GPU%d Cycle %lld ----------------------------\n", execRound, gpuId, cycle); )
    ...
    if (simP.statistics && simP.perGPUStatistics) {
        //  Update statistics.
        GPUStatistics::StatisticsManager::instance(gpuId).clock(cycle);

        //  Check if current batch has finished.
        if (commProc->endOfBatch() && simP.perBatchStatistics)
        {
            //  Update statistics.
            GPUStatistics::StatisticsManager::instance(gpuId).batch();
        }
        ...
        {
            //  Update statistics.
            GPUStatistics::StatisticsManager::instance(gpuId).frame(frameCounter);
        }
    }
    ...
    if (commProc->isSwap())
    {
        ...
        if (simP.simFrames != 0)
        {
            ...
            endSimLoop = endSimLoop || ((frameCounter - simP.startFrame) == simP.simFrames);
        }
    }
...
//OpzedDynamicMemory::usage();

        //  Check if trace has finished.
        endSimLoop = endSimLoop || commProc->isEndOfTrace();
    }

    //  Determine end of simulation.
    if (simP.simCycles != 0)
    {
        endSimLoop = endSimLoop || ((cycle + 1) == simP.simCycles);
    }
    ...
    if (simP.detectStalls && (GPU_MOD(cycle, 100000) == 99999))
    {
	    ...
	    char reportFileName[32];
	    sprintf(reportFileName, "GPU%dStallReport.txt", gpuId); 
        ...
        //  End the simulation if a stall was detected.                  
        endSimLoop = endSimLoop || gpuStalled;
    }
    ...
    if (endSimLoop) {
        ...
        if (simP.statistics && simP.perGPUStatistics)
            GPUStatistics::StatisticsManager::instance(gpuId).finish();
...
void GPUSimulator::multiClockSimLoop()
{
    ...
    {
        GPU_DEBUG(
            printf("GPU Domain in Round%d GPU%d. Cycle %lld ----------------------------\n", execRound, gpuId, gpuCycle);
        )
        ...
        if (simP.statistics && simP.perGPUStatistics) {
            //  Update statistics.
            GPUStatistics::StatisticsManager::instance(gpuId).clock(gpuCycle);
        ...
        if (commProc->isSwap())
        {
            ...
            {
                //  End if the number of requested frames has been rendered.
                endMultiClockSimLoop = endMultiClockSimLoop || ((frameCounter - simP.startFrame) == simP.simFrames);
            }
        }
...
//OpzedDynamicMemory::usage();
            ...
            //  Check if trace has finished.
            endMultiClockSimLoop = endMultiClockSimLoop || commProc->isEndOfTrace();
        }

        //  Determine end of simulation.
        if (simP.simCycles != 0)
        {
            endMultiClockSimLoop = endMultiClockSimLoop || ((gpuCycle + 1) == simP.simCycles);
        }
        ...
        if (simP.detectStalls && (GPU_MOD(gpuCycle, 100000) == 99999))
        {
            ...
            //  End the simulation if a stall was detected.                  
            endMultiClockSimLoop = endMultiClockSimLoop || gpuStalled;
        }
    ...
    if (nextShaderClock == 0)
    {
        ...
        {
            GPU_DEBUG(
                printf("Shader Domain in Round%d GPU%d. Cycle %lld ----------------------------\n", execRound, gpuId, shaderCycle);
            )
    ...
    if (nextMemoryClock == 0)
    {
        ...
        {
            GPU_DEBUG(
                printf("Memory Domain in Round%d GPU%d. Cycle %lld ----------------------------\n", execRound, gpuId, memoryCycle);
            )
    ...
    if (endMultiClockSimLoop) { 
        //GPU_DEBUG(
            printf("\n");
	    printf("Cycles in Round%d GPU%d\n", execRound, gpuId);
...
void GPUSimulator::dumpLatencyMap(u32bit w, u32bit h)
{
    ...
    sprintf(filename, "GPU%dlatencyMap%04d.ppm", gpuId, frameCounter);
