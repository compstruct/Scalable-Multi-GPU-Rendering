...
private:
    ...
    u32bit gpuId;                   /**<  GPU Id in multi-GPU architecture. */
    u32bit execRound;
    ...
    bool endSimLoop;            /**<  Flag that stores if the simulation loop end.  */
    bool endMultiClockSimLoop;  /**<  Flag that stores if the multi clock simulation loop end.  */
...
public:
    ...
    static u64bit globalGpuCycle;            /**<  Global cycle counter for GPU clock domain.  */
    static u64bit globalShaderCycle;         /**<  Global cycle counter for shader clock domain.  */
    static u64bit globalMemoryCycle;         /**<  Global cycle counter for memory clock domain.  */

    static struct GPUSync* gpuSync;
    ...
    GPUSimulator(u32bit gpuId, SimParameters simP, TraceDriverInterface *trDriver, bool unified,
		 bool d3d9Trace, bool oglTrace, bool agpTrace, u32bit execRound);
    ...
    bool simLoopEnd() { return endSimLoop; }
    bool multiClockSimLoopEnd() { return endMultiClockSimLoop; }

    bool isSwap() { return commProc->isSwap(); }
    ...
    void dumpZFightingInfo() { zStencilV2[0]->dumpZFightingInfo(); }
