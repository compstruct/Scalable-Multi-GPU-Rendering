...
Box* gpu3d::createMemoryController(u32bit gpuId,
		                   char *gpuPrefix,
		                   SimParameters& simP, 
                                   const char** tuPrefixes, 
                                   const char** suPrefix,
                                   const char** slPrefixes,
                                   char* memoryControllerName,
                                   Box* parentBox)
{
    using std::cout;
    using std::endl;
	...
	if (simP.multiGPUExecMode == GPUPD)
	    panic("gpu3d", "createMemoryController", "GPUpd only implemented with memoryControllerV2.");
	...
	params.peMapperBus = simP.mem.peMapperBus;
	params.peSyncerBus = simP.mem.peSyncerBus;
        ...
        params.readBuffers = simP.mem.readBufferLines * simP.gpu.numStampUnits;
        params.writeBuffers = simP.mem.writeBufferLines * simP.gpu.numStampUnits;
        params.requestQueueSize = simP.mem.reqQueueSz * simP.gpu.numStampUnits;
        params.serviceQueueSize = simP.mem.servQueueSz * simP.gpu.numStampUnits;
        ...
        memController = 
            new memorycontroller::MemoryController( gpuId,
			                            gpuPrefix,
			                            params,
                                                    (const char**)tuPrefixes, 
                                                    (const char**)suPrefix,
