...
//  Emulator.
std::vector<GPUEmulator *> gpuEmus;
...
int gpu3d::abortSignalHandler(int s)
{
	...
	for (u32bit gpuId = 0; gpuId < gpuEmus.size(); gpuId++) {
            gpuEmus[gpuId]->setAbortEmulation();
	}
...
void gpu3d::segFaultSignalHandler(int s)
{
    if (s == SIGSEGV)
    {
	    ...
	    for (u32bit gpuId = 0; gpuId < gpuEmus.size(); gpuId++) {
                printf("\n");
                printf("***! Segmentation Fault detected in GPU[%d] at frame %d batch %d triangle %d !***\n",
                       gpuId, gpuEmus[gpuId]->getFrameCounter(), gpuEmus[gpuId]->getBatchCounter(), gpuEmus[gpuId]->getTriangleCounter());
                fflush(stdout);
	    }
            signal(SIGSEGV, SIG_DFL);
        }
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        GPU_ASSERT(
	    if (simP.numGPUs != 1)
                panic("bGPU-emu", "main", "Only D3DTraceDriver supports multi-GPU architecture.");
	)
    ...
    //  Create GPU emulator.
    gpuEmus.resize(simP.numGPUs);
    for (u32bit gpuId = 0; gpuId < simP.numGPUs; gpuId++) {
        gpuEmus[gpuId] = new GPUEmulator(gpuId, simP, trDriver);
    }
	...
	for (u32bit gpuId = 0; gpuId < simP.numGPUs; gpuId++) {
	    ...
	    emuEnd = true;	
	    for (u32bit gpuId = 0; gpuId < simP.numGPUs; gpuId++) {
		if (gpuEmus[gpuId]->emulationLoopEnd() == false) {
                    gpuEmus[gpuId]->emulationLoop();
		    emuEnd = false;
		}
	    }
	}
