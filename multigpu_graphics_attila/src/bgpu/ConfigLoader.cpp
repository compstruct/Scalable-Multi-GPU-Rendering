...
bool ConfigLoader::parseSection(SimParameters *simP)
{
    ...
    if (!strcmp("PRIMITIVEEXCHANGER", sectionName))
    {
        section = SEC_PE;
    }
    ...
    while (!end)
    {
        ...
        if (getline(&str, &len, configFile) > 0)
        {
			...
			case SEC_PE:
			    ...
			    if (!parsePESectionParameter(&simP->pe))
				return FALSE;
			    break;
...
bool ConfigLoader::parseSimSectionParameter(SimParameters *simP)
{
    ...
    if (!parseStringParameter("StatsFilePerCycle", id, simP->statsFilePerCycle))
        return FALSE;
    ...
    if (!parseBooleanParameter("PerGPUStatistics", id, simP->perGPUStatistics))
        return FALSE;
    ...
    if (!parseDecimalParameter("NumGPUs", id, simP->numGPUs))
        return FALSE;

    if (!parseDecimalParameter("MultiGPUExecutionMode", id, simP->multiGPUExecMode))
        return FALSE;

    if (!parseDecimalParameter("InterGPUBusWidth", id, simP->interGPUBusWidth))
        return FALSE;

    if (!parseDecimalParameter("InterGPUBusLatency", id, simP->interGPUBusLatency))
        return FALSE;

    if (!parseDecimalParameter("CpdDrawCommandBatchSize", id, simP->cpdDrawCommBatchSize))
        return FALSE;

    if (!parseBooleanParameter("RunaheadEnabledInGPUpd", id, simP->runaheadInGPUpd))
        return FALSE;

    if (!parseBooleanParameter("ProfileForSortLast", id, simP->profileForSortLast))
        return FALSE;

    if (!parseDecimalParameter("MinKDTreeInSortLast", id, simP->minKDTreeInSortLast))
        return FALSE;

    if (!parseBooleanParameter("RoundRobinDispatcher", id, simP->roundRobinDispatcher))
        return FALSE;

    if (!parseBooleanParameter("IdealComposition", id, simP->idealComp))
        return FALSE;

    if (!parseBooleanParameter("CompositionSchedulerEnabled", id, simP->compSchedEnabled))
        return FALSE;

    if (!parseDecimalParameter("ProcessedTrianglesUpdateThreshold", id, simP->processedTrisUpdateThreshold))
        return FALSE;
...
bool ConfigLoader::parseMemSectionParameter(MemParameters *memP)
{
    ...
    if (!parseDecimalParameter("PrimitiveExchangerMapperBusWidth", id, memP->peMapperBus))
        return FALSE;

    if (!parseDecimalParameter("PrimitiveExchangerSyncerBusWidth", id, memP->peSyncerBus))
        return FALSE;
...
bool ConfigLoader::parseRasSectionParameter(RasParameters *rasP)
{
    ...
    if (!parseDecimalParameter("DisplayScanWidth", id, rasP->displayScanWidth))
        return FALSE;

    if (!parseDecimalParameter("DisplayScanHeight", id, rasP->displayScanHeight))
        return FALSE;
    ...
    if (!parseDecimalParameter("SyncedStampQueueSize", id, rasP->syncedStampQSize))
        return FALSE;

    if (!parseDecimalParameter("InterGPUSyncedStampQueueSize", id, rasP->interGPUSyncedStampQSize))
        return FALSE;
...
bool ConfigLoader::parseZSTSectionParameter(ZSTParameters *zstP)
{
    ...
    if (!parseDecimalParameter("SyncCommQueueSizePerGPU", id, zstP->syncCommQSz))
        return FALSE;

    if (!parseDecimalParameter("SyncInputQueueSize", id, zstP->syncInputQueueSize))
        return FALSE;

    if (!parseDecimalParameter("SyncFetchQueueSize", id, zstP->syncFetchQueueSize))
        return FALSE;

    if (!parseDecimalParameter("SyncReadQueueSize", id, zstP->syncReadQueueSize))
        return FALSE;

    if (!parseFloatParameter("DummyFragmentsGenerationPossibility", id, zstP->dummyFragsGenPossibility))
        return FALSE;
...
bool ConfigLoader::parseCWRSectionParameter(CWRParameters *cwrP)
{
    ...
    if (!parseDecimalParameter("SyncCommQueueSizePerGPU", id, cwrP->syncCommQSz))
        return FALSE;

    if (!parseDecimalParameter("SyncInputQueueSize", id, cwrP->syncInputQueueSize))
        return FALSE;

    if (!parseDecimalParameter("SyncFetchQueueSize", id, cwrP->syncFetchQueueSize))
        return FALSE;

    if (!parseDecimalParameter("SyncReadQueueSize", id, cwrP->syncReadQueueSize))
        return FALSE;
...
/*  Parse PE parameters.  */
bool ConfigLoader::parsePESectionParameter(PEParameters *peP)
{
    char id[100];
    ...
    paramsTracker.startParamSectionDefinition("PRIMITIVEEXCHANGER");

    /*  Parse parameters.  */

    if (!parseDecimalParameter("TrianglesCycle", id, peP->trianglesCycle))
        return FALSE;

    if (!parseDecimalParameter("PrimitiveExchangerUnits", id, peP->primitiveExchangerUnits))
        return FALSE;

    if (!parseDecimalParameter("PrimitiveToGPUMappingLatency", id, peP->p2gMappingLatency))
        return FALSE;

    if (!parseDecimalParameter("PrimitiveExchangerTriangleInputBufferSize", id, peP->peTriangleInputBufferSize))
        return FALSE;

    if (!parseDecimalParameter("PrimitiveExchangerStartLatency", id, peP->startLatency))
        return FALSE;

    if (!parseDecimalParameter("PrimitiveExchangerMappedInfoReadBufferSize", id, peP->mappedInfoReadBufferSize))
        return FALSE;

    if (!parseDecimalParameter("PrimitiveExchangerExchangedIndexBufferSize", id, peP->exchangedIndexBufferSize))
        return FALSE;

    if (!parseBooleanParameter("AllResultsOnChip", id, peP->allResultsOnChip))
        return FALSE;

    if (!parseBooleanParameter("PrimitiveExchangerGPUpdBatchEnabled", id, peP->gpupdBatchEnabled))
        return FALSE;

    if (!parseDecimalParameter("PrimitiveExchangerGPUpdBatchThreshold", id, peP->gpupdBatchThreshold))
        return FALSE;

    if ( !paramsTracker.wasAnyParamSectionDefined() ) {
        stringstream ss;
        ss << "Parameter '" << id << "' in section [PRIMITIVEEXCHANGER] is not supported";
        panic("ConfigLoader", "parsePESectionParameter", ss.str().c_str());
    }
...
bool ConfigLoader::parseDecimalParameter(std::string paramName, char *id, u32bit &val)
{
    s32bit aux;
...
bool ConfigLoader::parseDecimalParameter(std::string paramName, char *id, u64bit &val)
{
    s64bit aux;
...
bool ConfigLoader::parseFloatParameter(std::string paramName, char *id, f32bit &val)
{
    f32bit aux;
...
bool ConfigLoader::parseStringParameter(std::string paramName, char *id,
    char *&string)
{
    char *auxStr;
...
bool ConfigLoader::parseBooleanParameter(std::string paramName, char *id, bool &val)
{
    bool aux;
