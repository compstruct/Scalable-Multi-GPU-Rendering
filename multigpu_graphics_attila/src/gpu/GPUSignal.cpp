...
bool Signal::writeGenFast(u64bit cycle, DynamicObject* dataW)
{
    ...
    if (nextWrite == nextRead) {
	    ...
	    if (readsDone > 0) {
                char buffer[1024];
                sprintf(buffer, "Same cycle read happen in signal \"%s\" cycle %lld nextWrite %d nReads %d readsDone %d.",
    	            name, cycle, nextWrite, nReads[nextWrite], readsDone);
                panic("Signal", "writeGenFast", buffer);	
	    }		
	)
    }
...
bool Signal::writeGenFast(u64bit cycle, DynamicObject* dataW, u32bit latency)
{
    ...
    if (nextWrite == nextRead) {
	    ...
	    if (readsDone > 0) {
                ...
                sprintf(buffer, "Same cycle read happen in signal \"%s\" cycle %lld nextWrite %d nReads %d readsDone %d.",
    	            name, cycle, nextWrite, nReads[nextWrite], readsDone);
                panic("Signal", "writeGenFastLat", buffer);	
	    }		
	)
    }
...
//latency, nextWrite, nReads[nextWrite]);
    ...
    if (nReads[nextWrite] == bandwidth)
    {
        ...
        sprintf(buffer, "Error.  Max. BW exceeded (read conflict).  Signal \"%s\" cycle %lld. "
...
bool Signal::writeGen( u64bit cycle, DynamicObject* dataW )
{
    ...
    GPU_ASSERT(
        ...
        {
            sprintf(buffer, "Error in write.  Lost data in signal \"%s\" cycle %lld."
                            "DynamicObject info='%s'\n", name, cycle, (char *)dataW->getInfo());
        ...
        {
            sprintf(buffer, "Error. Max. BW (%d) exceeded (writes per cycle). Signal \"%s\" cycle %lld."
                            "DynamicObject info='%s'\n", nWrites, name, cycle,
                            (char *)dataW->getInfo());
    ...
    GPU_ASSERT(
        ...
        if ( lastCycle == cycle && nWrites == bandwidth )
        {
            sprintf(buffer, "Error. Max. BW exceeded (number of writes in same cycle ). "
                            "Signal \"%s\" cycle %lld. DynamicObject info='%s'\n",
                            name, cycle, (char *)dataW->getInfo());
