...
{
    sManager = &(StatisticsManager::instance());
    return *sManager;
}

StatisticsManager& Box::getSM(u32bit gpuId)
{
    return getSM().instance(gpuId);
}
...
Box::Box( u32bit gpuId, const char* nameBox, Box* parentBox ) :
gpuId(gpuId), parent(parentBox), binder(SignalBinder::getBinder()), debugMode(false)
{
    // register new box
